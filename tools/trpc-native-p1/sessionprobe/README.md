...
It requires a changed replay under a stable event ID to return an error and to
...
choice. A bootstrap user event is included solely because the SDK's session
read filtering removes assistant-only histories.
