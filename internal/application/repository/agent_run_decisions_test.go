...
func TestAgentRunDecisionSameIDIsIdempotent(t *testing.T) {
	...
	require.NoError(t, db.Exec("UPDATE agent_runs SET status='waiting_user', wait_reason='p1', revision=7 WHERE tenant_id=1 AND run_id='r1'").Error)
	d := agentruntime.Decision{PendingID: "p1", ToolCallID: "c1", ArgsHash: "ah1", DecisionID: "d1", Action: "provide_result", Reason: "user supplied", ExpectedRevision: 7, Result: json.RawMessage(`{"success":true,"output":"ok"}`)}
...
func TestAgentRunDecisionRetryPreflightParkRequeuesRun(t *testing.T) {
	...
	store := NewAgentRunStore(db)
	require.NoError(t, admitDecisionRun(t, store))
	require.NoError(t, db.Exec("INSERT INTO agent_tool_calls (tenant_id,run_id,call_id,call_seq,tool_name,tool_identity,args_hash,args,status) VALUES (1,'r1','c1',1,'fetch','fetch:v1','ah1','{}','planned')").Error)
	...
	require.NoError(t, db.Exec("UPDATE agent_runs SET status='running', lease_owner='w', epoch=1, lease_until=datetime('now','+1 hour') WHERE tenant_id=1 AND run_id='r1'").Error)
	require.NoError(t, store.ParkToolPreflightWait(context.Background(), fence, "c1", "mcp_oauth_x", "svc-1"))
	require.NoError(t, db.Exec("UPDATE agent_runs SET status='waiting_user', wait_reason='mcp_oauth_x', revision=7, lease_owner='', lease_until=NULL WHERE tenant_id=1 AND run_id='r1'").Error)
	run, err := store.ApplyDecision(context.Background(), agentruntime.RunKey{TenantID: 1, RunID: "r1"}, "u1", agentruntime.Decision{
	...
	require.NoError(t, db.Raw("SELECT status, unknown_reason FROM agent_tool_calls WHERE tenant_id=1 AND run_id='r1' AND call_id='c1'").Scan(&tool).Error)
	...
	var attempts int64
	require.NoError(t, db.Raw("SELECT COUNT(*) FROM agent_tool_attempts WHERE tenant_id=1 AND run_id='r1' AND call_id='c1'").Scan(&attempts).Error)
	require.Zero(t, attempts)
	var decision struct{ ResourceRef, ToolCallID string }
	require.NoError(t, db.Raw("SELECT resource_ref, tool_call_id FROM agent_run_decisions WHERE tenant_id=1 AND run_id='r1' AND decision_id='d1'").Scan(&decision).Error)
...
func TestAgentRunDecisionProvideResultLinksUnknownTool(t *testing.T) {
	...
	require.NoError(t, db.Exec("INSERT INTO agent_tool_calls (tenant_id,run_id,call_id,call_seq,tool_name,tool_identity,args_hash,args,status,unknown_reason) VALUES (1,'r1','c1',1,'write','write:v1','ah1','{}','unknown','p1')").Error)
	require.NoError(t, db.Exec("UPDATE agent_runs SET status='waiting_user', wait_reason='p1', revision=7 WHERE tenant_id=1 AND run_id='r1'").Error)
	d := agentruntime.Decision{PendingID: "p1", ToolCallID: "c1", ArgsHash: "ah1", DecisionID: "d1", Action: "provide_result", Reason: "user supplied", ExpectedRevision: 7, Result: json.RawMessage(`{"success":true,"output":"ok"}`), ResourceRef: "oauth://example"}
	...
	require.NoError(t, db.Raw("SELECT status,source,result FROM agent_tool_calls WHERE tenant_id=1 AND run_id='r1' AND call_id='c1'").Scan(&row).Error)
	...
	require.NoError(t, db.Raw("SELECT args_hash,resource_ref,tool_call_id FROM agent_run_decisions WHERE tenant_id=1 AND run_id='r1' AND decision_id='d1'").Scan(&decision).Error)
