...
func TestSemanticModelInvocationCompleteReplaysAndCountsObservedUsage(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, types.SemanticModelInvocationClaimedNew, claim.Disposition)
	require.Error(t, store.Complete(context.Background(), capability, types.SemanticModelInvocationResult{Result: []byte("saved"), InputTokens: 1, OutputTokens: 1}), "completion before the irreversible dispatch marker would release a quota slot without proving the outbound boundary")
	require.NoError(t, store.MarkDispatched(context.Background(), capability))
	require.Error(t, store.FailBeforeDispatch(context.Background(), capability), "a dispatched call must retain its quota for reconciliation")
