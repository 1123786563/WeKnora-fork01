...
func TestSemanticOutboxSQLiteClaimReturnsConcurrentDueFailureCode(t *testing.T) {
	ctx := context.Background()
	db, path := newSemanticSQLiteTestDBWithPath(t)
	require.NoError(t, db.Exec("PRAGMA journal_mode=WAL").Error)
	otherDB, err := gorm.Open(sqlite.Open(path+"?_foreign_keys=on&_busy_timeout=5000"), &gorm.Config{Logger: logger.Default.LogMode(logger.Silent)})
	...
	otherPool.SetMaxOpenConns(1)
	t.Cleanup(func() { _ = otherPool.Close() })
	repo, otherRepo := NewSemanticControlRepository(db), NewSemanticControlRepository(otherDB)
	_, err = repo.WithSemanticMutation(ctx, mutationFixture(0, false, []byte("stable-payload")), noBusinessWrite)
	require.NoError(t, err)
	first, err := repo.ClaimSemanticOutbox(ctx, "worker-a", 30)
	require.NoError(t, err)
	require.NotNil(t, first)
	...
	require.NoError(t, err)
	require.NotNil(t, staleCandidate)
	require.NoError(t, otherRepo.FailSemanticOutbox(ctx, first.EventID, "worker-a", first.LeaseToken, time.Unix(1, 0), "due-failure"))

	claimed, err := repo.claimSQLiteOutboxCandidate(ctx, staleCandidate, "worker-b", 30)
	require.NoError(t, err)
	require.NotNil(t, claimed)
	require.Equal(t, first.EventID, claimed.EventID)
	require.Equal(t, first.PayloadHash, claimed.PayloadHash)
	require.Equal(t, first.ConfigDigest, claimed.ConfigDigest)
	require.Equal(t, "due-failure", claimed.ErrorCode)
	require.Equal(t, "worker-b", claimed.LeaseOwner)
	require.Equal(t, first.LeaseToken+1, claimed.LeaseToken)
}
