...
func NativeSchemaManifest() []NativeSchemaTable {
	return []NativeSchemaTable{
		...
		nativeTable("native_memory_jobs", []string{"tenant_id", "subject_id", "job_id", "generation", "through_event_id", "status", "session_app_name", "session_user_id", "session_id", "retry_attempt", "max_attempts", "retry_status", "next_attempt_at", "last_error"}, []string{"tenant_id", "subject_id", "job_id"}),
		...
		nativeTable("native_agent_tool_plans", []string{"tenant_id", "run_id", "attempt_id", "call_id", "plan_version", "kind", "service_id", "installation_id", "name", "schema_hash", "config_version", "args", "args_hash", "policy", "idempotency_key"}, []string{"tenant_id", "run_id", "attempt_id", "call_id", "plan_version"}),
		...
		nativeTable("native_agent_pending_decisions", []string{"tenant_id", "run_id", "pending_id", "call_id", "plan_version", "args_hash", "wait_kind", "expected_revision", "decision_id", "decision_hash", "detail"}, []string{"tenant_id", "run_id", "pending_id"}),
