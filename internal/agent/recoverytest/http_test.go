...
func TestRecoveryAdmissionGate(t *testing.T) {
	cases := []struct {
		...
		{name: "worker only drains", cfg: &config.Config{Agent: &config.AgentConfig{Recovery: config.AgentRecoveryConfig{Enabled: boolPtr(true)}}}, want: false},
...
func TestRecoveryAdmissionGateRejectsInconsistentConfig(t *testing.T) {
	cfg := &config.Config{Agent: &config.AgentConfig{Recovery: config.AgentRecoveryConfig{AdmissionEnabled: boolPtr(true)}}}
