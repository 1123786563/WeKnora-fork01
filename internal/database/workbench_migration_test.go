...
func TestWorkbenchSQLiteURLUpgradeAndDownUpPreserveChildren(t *testing.T) {
	...
	t.Run("legacy-upgrade-down-up", func(t *testing.T) {
		...
		require.NoError(t, runWorkbenchSQLiteMigrationSteps(repoRoot, dbPath, -6))
		version, dirty = sqliteMigrationState(t, db)
		require.Equal(t, 15, version)
		require.False(t, dirty)
		assertWorkbenchChildSummary(t, db)
...
func copySQLiteMigrationsBeforeWorkbench(t *testing.T, repoRoot string) string {
	...
	for _, entry := range entries {
		if entry.IsDir() || strings.HasPrefix(entry.Name(), "000016_") || strings.HasPrefix(entry.Name(), "000017_") || strings.HasPrefix(entry.Name(), "000018_") || strings.HasPrefix(entry.Name(), "000019_") || strings.HasPrefix(entry.Name(), "000020_") {
			continue
		}
