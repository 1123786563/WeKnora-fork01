...
var semanticTables = []string{
	...
	"semantic_budget_reservations",
	"semantic_budgets",
	"semantic_task_operations",
	"semantic_attempt_counters",
}
...
func TestSemanticMigrationSQLite(t *testing.T) {
	...
	// Down removes exactly the semantic tables (000053-000056); Up
	// reinstates them.
...
func TestSemanticMigrationPostgres(t *testing.T) {
	...
	// Down drops the semantic tables (000133-000136), Up restores them.
	require.NoError(t, migrator.Steps(-4))
