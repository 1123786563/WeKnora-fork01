// Package insights 是 WeKnora 后端模块化改造（Pass A）的 insights 模块占位骨架。
//
// 职责（spec §5.18 / F0）：Analytics、Evaluation、运营指标、报表与可重建分析投影。
//
// 非职责：源记录归产生事实的业务模块；Insights 只拥有投影与报表。
//
// 搬迁清单：docs/architecture/moves/insights.yaml（本模块唯一 scope 事实源）。
//
