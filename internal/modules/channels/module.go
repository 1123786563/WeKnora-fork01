// Package channels 是 WeKnora 后端模块化改造（Pass A）的 channels 模块占位骨架。
//
// 职责（spec §5.18 / F0）：IM 适配器、Channel、Embed、Webhook、外部身份与会话映射、入站标准化与出站投递。
//
// 非职责：统一会话行为归 Conversation；身份主数据归 Identity。
//
// 搬迁清单：docs/architecture/moves/channels.yaml（本模块唯一 scope 事实源）。
//
...
//	    注册本模块 HTTP 路由（当前 4 项入口，见 manifest integration_points.routes）。
