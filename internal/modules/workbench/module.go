// Package workbench 是 WeKnora 后端模块化改造（Pass A）的 workbench 模块占位骨架。
//
// 职责（spec §5.18 / F0）：Task 视图、Timeline、Inbox/Attention、通用 Artifact 汇总、通知与移动任务投影。
//
// 非职责：Craft 专用源记录归 Craft；通用文件/HMAC grant 机制归 Platform。
//
// 搬迁清单：docs/architecture/moves/workbench.yaml（本模块唯一 scope 事实源）。
//
...
//	    注册本模块 HTTP 路由（当前 10 项入口，见 manifest integration_points.routes）。
