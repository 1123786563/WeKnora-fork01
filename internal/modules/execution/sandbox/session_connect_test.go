...
// restoredTokenFixture returns a unique per-run inbound token for connect
// pass-through assertions. It is generated (never a stored literal) because it
// occupies the TrafficAccessToken credential slot.
func restoredTokenFixture() string {
	return fmt.Sprintf("fixture-token-%d", time.Now().UnixNano())
}
