// Package agentruntime 是 WeKnora 后端模块化改造（Pass A）的 agentruntime 模块占位骨架。
//
// 职责（spec §5.18 / F0）：Agent 执行引擎、Run/Attempt 生命周期、工具调用、运行审批、Compaction、Memory 与 Model Context，native/tRPC/OpenCode 协议。
//
// 非职责：Agent 定义读取归 Agent Catalog；MCP 服务定义与凭据归 AI Resource。
//
// 搬迁清单：docs/architecture/moves/agentruntime.yaml（本模块唯一 scope 事实源）。
//
...
//	    注册本模块 HTTP 路由（当前 3 项入口，见 manifest integration_points.routes）。
//	(m *Module) RegisterWorkers(mux WorkerRegistrar)
//	    注册本模块 asynq/Lite 任务处理器（当前 1 项，见 integration_points.workers）。
