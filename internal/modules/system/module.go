// Package system 是 WeKnora 后端模块化改造（Pass A）的 system 模块占位骨架。
//
// 职责（spec §5.18 / F0）：初始化、System Setting、部署能力、系统管理与 housekeeping。
//
// 非职责：config 读取与进程启动机制归 Platform/bootstrap。
//
// 搬迁清单：docs/architecture/moves/system.yaml（本模块唯一 scope 事实源）。
//
