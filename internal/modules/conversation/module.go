// Package conversation 是 WeKnora 后端模块化改造（Pass A）的 conversation 模块占位骨架。
//
// 职责（spec §5.18 / F0）：Session、Message、chat pipeline、Feedback、分享、建议、Query History 与临时文档。
//
// 非职责：Agent 执行引擎与工具调用归 Agent Runtime；不在 Conversation 内复制运行引擎。
//
// 搬迁清单：docs/architecture/moves/conversation.yaml（本模块唯一 scope 事实源）。
//
