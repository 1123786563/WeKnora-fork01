// Package appconnector 是 WeKnora 后端模块化改造（Pass A）的 appconnector 模块占位骨架。
//
// 职责（spec §5.18 / F0）：应用目录、安装、OAuth、连接、授权、动作、同步与 Open Connector 适配/控制。
//
// 非职责：Agent 工具只调用本模块公开动作接口，不读取连接表；MCP 定义归 AI Resource。
//
// 搬迁清单：docs/architecture/moves/appconnector.yaml（本模块唯一 scope 事实源）。
//
...
//	    启动后台任务（当前 5 项生命周期挂点，见 integration_points.lifecycle_hooks）。
