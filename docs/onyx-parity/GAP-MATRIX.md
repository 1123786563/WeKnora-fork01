...
| # | 能力 | Onyx 实现要点 | WeKnora 现状 | 定性 | 备注 |
|---|------|--------------|--------------|------|------|
| P-1 | 用量日桶聚合 | `user_usage` 表（user×日×model×flow）upsert 累加 token/成本 `backend/onyx/db/models.py:6206`、tracing 采集 `tracing/processors/user_usage_processor.py` | ✅ `user_usage` 日桶表（migration 000160/000081）+ chat 写入点（completeAssistantMessage 一次性 gate 防双计）+ craft 事务内 fold + `WEKNORA_USAGE_RATES` env 微积分计价，见 SP12 验收证据 | ✅ | 交付物：`43b8be8a`/`54f37272`/`af33e632`+`1c9fd69b`(并发带入)/`afd25ea9`/`9548d95f`；[evidence](../migrations/react/evidence/onyx-parity/2026-09-19-sp12-usage-aggregation.md) |
| P-2 | 用户/admin 用量视图 | `/app/settings/usage`、`/admin/performance/usage` | ✅ 三端点（`/usage/me`、`/admin/usage/by-user`、`/admin/usage/export` CSV）+ settings 用量分区（UsagePanel）+ analytics 用量 tab（全员表+CSV 下载），见 SP12 验收证据 | ✅ | 交付物：`839d68b6`/`4d7616e9`/`1a3f1f1c`/`5e0f000d`；[evidence](../migrations/react/evidence/onyx-parity/2026-09-19-sp12-usage-aggregation.md) |
| P-3 | 套餐/计费页 | `/admin/billing` 两档卡片+Stripe Checkout | ✅ `/platform/billing` 三路由（billing/checkout/admin）+ general 分区套餐卡片与升级入口（`formatBillingSummary` 五语）+ guardRoute 收口，见 SP14 验收证据 | ✅ | **有意收紧**：RefundPage 未挂路由（待真实退款流程）；CheckoutPage plan_key 硬编码 pro（组件原状）；交付物：`6a8c8f1a`/`eb49392f`；[evidence](../migrations/react/evidence/onyx-parity/2026-09-20-sp14-lightweight.md) |
...
| P-10 | API 文档入口 | docs.onyx.app 外链 | ✅ 后端 `swagger_enabled` 透出（非 release 模式）+ integrations 页「OpenAPI /docs」条件渲染（`swaggerDocsUrl` 归一 `/swagger/index.html`，修悬空 `/docs`）+ API key 面板 API 文档外链 + capabilities 五语文案；顺带修 platform-api-keys 分区不可达（role 折叠吞 system-admin），见 SP14 验收证据 | ✅ | 老后端无字段→隐藏（向后兼容）；交付物：`eb49392f`+收尾 vite 别名修复；[evidence](../migrations/react/evidence/onyx-parity/2026-09-20-sp14-lightweight.md) |
| P-11 | per-user 默认模型偏好 | `User.default_model` 等+设置页 `web/src/views/SettingsPage.tsx` | ✅ `user.preferences.default_model`（JSONB 无迁移，PATCH 合并/空串=清除）+ 会话偏好分区（回显/保存/清除+优先级文案）+ 解析链 pick > agent 绑定 > 用户默认 > models[0]（芯片/下拉/发送链一致），见 SP14 验收证据 | ✅ | 解析链纯前端方案 A（后端零改动）；交付物：`c14c4501`/`f8ca453c`+`e72acff9`（收编）；[evidence](../migrations/react/evidence/onyx-parity/2026-09-20-sp14-lightweight.md) |
...
**第三部分收口（2026-09-20，六域线收官）**：P-1~P-11 十一项全部 ✅（SP11 P-4/P-5 → SP12 P-1/P-2 → SP13 P-6~P-9 → SP14 P-3/P-10/P-11，证据链见 [evidence/onyx-parity/](../migrations/react/evidence/onyx-parity/) 目录四个验收文档）。平台运营面六域（用量/套餐/分析/查询历史/OpenAPI/对话偏好）对齐线就此收官；已登记未追随/遗留项（匿名公开分享、商业运营端点 wiring、commercial summary 前后端形状错配等）见各 SP 证据"已知遗留"节与 ROADMAP 变更记录。
