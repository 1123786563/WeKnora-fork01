...
## 当前阶段：全部 36 项 verified（S00-S05 全闭）——网页里程碑双模式达标 + Office 三类型模拟环境过 + 安全/灰度/证据收敛完成
...
| CFT-S00-T004 | 共享组件与Craft宿主壳 | **verified（自审）** | shell.tsx 组合层（Shell/PanelHeader/Notice/Drawer，焦点委托共享 Sheet）；4 测试+82 shared；craft.css 增量全走语义别名。证据 evidence/CFT-S00-T004/ |
