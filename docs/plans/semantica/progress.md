...
状态：V01、V02、C01、C02 已按各自证据层和验收门槛标记为 verified；C03 的实现与测试已完成，最终整体 review 待记录；其余 19 个正式任务仍为 pending。先导 probe 另行留证，不替代 V03 或生产验收。总计划：[实施入口](../2026-09-11-semantica-implementation.md)，校准入口：[2026-09-20 rebaseline](../2026-09-20-semantica-rebaseline.md)。ADR-0002 与 rebaseline 优先于旧计划中的未验证建议。
...
| C03 | 事实与证据校验模型 | C01,V02 | implemented | 2026-09-21: plan `2026-09-21-semantic-c03-facts-evidence.md`; code commits `4377f3d4`, `4c8e2c73`, `0bcca37f`, `06c46968`; reviewed contract adjustments close malformed unanchored evidence types, Unicode normalization regression, and missing scopes on facts/derivations. Final implementation head `06c46968e303f7023f21e93ebb75253ab7c7a4ba`. Fresh C02+C03 semantic tests: `uv run --locked --project semantic python -m pytest semantic/tests -q` exit 0 (51 passed); `semantic/scripts/generate_proto.sh --check` exit 0; `git diff 5fb3525dc3aac1d3b58da711267399991f728cfc..HEAD --check` exit 0. Task review deltas: Task 1 had one Important + one Minor, both fixed with RED→GREEN and scoped re-review clean; Task 2 had one Important, fixed with RED→GREEN and scoped re-review clean. No proto/C01 contract changes, persistence, graph writes, model/provider, authorization issuance, or production wiring. Final whole-C03 review pending; status remains implemented. |
