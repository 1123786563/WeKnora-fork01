...
状态：V01、V02、C01 已按各自证据层和验收门槛标记为 verified；其余 21 个正式任务仍为 pending。先导 probe 另行留证，不替代 V03 或生产验收。总计划：[实施入口](../2026-09-11-semantica-implementation.md)，校准入口：[2026-09-20 rebaseline](../2026-09-20-semantica-rebaseline.md)。ADR-0002 与 rebaseline 优先于旧计划中的未验证建议。
...
| C01 | 版本化协议和跨语言领域类型 | V01 | verified | 2026-09-21: implementation reviewed through `cd4d0616b1eefa34f671bc55ba47bed5abc23948`; separate `weknora.semantic.v1` schema, seven RPCs, version-pinned Go/Python generation, and bidirectional DTO mappings. Fresh commands exit 0: `semantic/scripts/generate_proto.sh --check`; `go test ./semantic/proto ./internal/types -run 'TestSemantic' -count=1`; `uv run --locked --project semantic python -m pytest semantic/tests -q` (32 passed). Final independent review `review_c01_presence_close` found no Critical/Important findings after populated optional-field round trips were added. Scope remains wire/domain contract only: no DocReader extension, RPC server/client, model/provider, storage or authority issuance; owned Assertion/Derivation and IndexManifest/ReadLease models remain deferred to C03/I03. |
