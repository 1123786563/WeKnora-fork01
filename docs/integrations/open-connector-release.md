...
| provider_read | **本会话已在候选镜像上真实验证（只读）**，见 §6 | §6 记录（候选镜像 `4de6df4d…` + 真实 GitHub `get_current_user` 200；响应文件 sha256 见 §6 表） | (a) 证据 fixture 未入库（超出 T18 write set，待协调者落盘）；(b) 经 WeKnora 全链（Prepare→审批→Execute→预算结算）的真实 Provider 读未做：blocked-env，缺授权测试空间；(c) OAuth connected 态真实流程：blocked-env，无授权 OAuth app/账号 |
