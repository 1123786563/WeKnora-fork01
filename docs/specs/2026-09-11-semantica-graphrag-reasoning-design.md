> **历史规划参考，不是实施权威。** [ADR-0002](../adr/0002-semantica-independent-service.md) 与 [2026-09-20 Semantica rebaseline](../plans/2026-09-20-semantica-rebaseline.md) 在任何冲突中优先，尤其是未经验证的版本、协议、存储、拓扑、阈值和 promotion 决策。

# WeKnora Semantica 语义知识服务架构规格

## 2026-09-20 访谈确认

本次访谈确认 [ADR-0002](../adr/0002-semantica-independent-service.md)：Semantica 是独立 Python 服务，使用独立专用 gRPC 协议和 Go adapter；借鉴 DocReader 的通信、配置和部署模式，但不扩展 DocReader 服务。上传经 DocReader 解析后，由 Go 保存正文和标准分块；Semantica 接收标准分块，负责语义图谱、语义索引和推理产物。tenant、知识库、文档和权限由 Go 保持权威。独立服务避免解析接口承担构图及推理的不同生命周期，复用 Go 业务权威避免两套权限模型。最终范围为构图、GraphRAG 与知识推理，回答须区分原文事实与推理结论。

模型配置和调用经 WeKnora 授权入口，关联用量、预算和任务；Python 不配置独立长期密钥，该适配仍待验证。首期知识关联范围为单一知识库内的跨文档关联。

每个知识库选择一个图后端，保留 native 后端；选择 Semantica 的知识库先构建其索引并完成验收后才切换，每个知识库的正式查询只使用一个图后端。首期推理采用模型辅助推断和开发者注册的版本化规则推导，规则编辑器另议。知识库启用后自动处理存量文档，新增和修改持续异步更新；记录进度、失败重试和用量，解析状态与语义状态分开。普通知识问答可退回现有检索，并显式注明未使用语义图谱；明确请求推理但服务不可用、索引未就绪或推理执行失败时，显示推理未完成和重试入口，不能用普通检索结果冒充推理成功。
