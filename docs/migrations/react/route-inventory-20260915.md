...
当前 checkout 静态存在性检查仍找不到四个 `/platform/apps*` 行引用的 Vue 文件，但它们已在历史 Vue 实现提交 `9b0c11c4` 中解析到，详见 `artifacts/apps-vue-authority-20260916.md`。这解决了 authority 路径追溯问题，但不等于视觉、权限或 mutation 已验收。`embed.html` 行仍把两个路径合并在一个字段中；详细命令、路径和影响见 `artifacts/route-inventory-parity-ledger-audit-20260916.md`。
