...
- 新增 Vue 风格自定义组织选择器（头像、角色、成员/知识库/智能体计数、选中态、外部点击关闭和 Escape），并保留组织信息预览、权限提示/footer、共享行头像/权限色签和组织设置导航入口；专测 12/12。
- Web 全量回归 681/681；本切片专测 12/12。`typecheck:web` 被并行新增 `apps/web/src/faq/import-export.excel.test.ts` 的 xlsx/Blob 类型错误阻断，未将该外部失败归因于本切片。
