...
实现提交：`9e5f970`（`feat: add one-time legacy browser session import`）。
...
| 命令 | 结果 |
|---|---|
| `pnpm exec tsx --test apps/web/src/platform/legacy-session.test.ts apps/web/src/platform/credentials.test.ts` | 6/6 passed，exit 0 |
| `pnpm test:web` | 63/63 passed，exit 0 |
