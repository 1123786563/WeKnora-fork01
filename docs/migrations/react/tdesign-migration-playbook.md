...
| 16 | Input autoWidth 宽度测量取整 | `calcWidth = width < offsetWidth ? offsetWidth + 1 : width`——分数文本宽恒走 `offsetWidth+1` 上取整（input/Input.js updateInputWidth；pagination 页大小 select 实测 input 47px） | 使用分数文本宽（同文本 `20 条/页` input 45.84px），右侧箭头/后续元素不位移 | 库差异无 seam（测量分支内嵌组件）。传导：t-pagination 页大小 select 宽 +1.2px → total(flex) 与其后页码/跳至整体左移 ~1.2px → 文字 AA 差（settings-members 两表 pager 共 ~772px，task-12a fix-1 实证）。同家族未定位到库行的伴生差：Table 无边框行高整数化（成员行 69 vs Vue 68.19，内容 44.19+24 两端一致）、表格单元格内「svg+空格+CJK」空格 advance 3.67 vs 4.0（通字 glyph x 379.67 vs 380，两端字体栈/字号/charcode 全同）——均记扫描豁免 |
|   | （pilot 回填区） | | | |
