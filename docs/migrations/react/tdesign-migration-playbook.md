...
| 24 | 会话行「更多」菜单「分享」项 | `session-sidebar.tsx` 渲染 6 项（置顶/修改标题/分享/清空消息/批量管理/删除记录，onShareSession 能力开关=canShareSessions 常真） | `menu.vue` buildSessionMenuOptions 固定 5 项（无分享；分享类能力在 chat header 菜单=复制对话链接） | **SP13 spec 强制的 React-only 功能豁免（panel-matrix Phase II Batch-1 实证）**：spec docs/specs/2026-09-19-onyx-platform-parity-design.md §SP13 前端明文「会话侧栏 session-sidebar.tsx 菜单加‘分享’项+分享弹窗」（commit e22b1a659）；像素级：菜单同位 (81,272) 同宽 162，React 高 219 vs Vue 188=多一枚 32px「分享」行，px-shell-session-more 恒 1.166% 即此一项。实施 Plan 与批准 Spec 冲突以 Spec 为准（AGENTS.md 冲突规则），不改码，扫描解读引用本行。**【2026-09-25 第二轮全量复勘】豁免仍成立（run 05-52-41/06-54-20/08-58-05）**：像素级——三轮恒 1.166%，hot cells 逐字相同（3,0:24.8/3,1:23.7/4,0:16.9/4,1:5.7/5,0:1.7/5,1:1.8，全落侧栏下拉菜单+其下会话整体 32px 移位带，菜单带外 0）；代码级——分享项现为宿主能力开关制（packages/views/src/chat/session-sidebar.tsx:100-111 isShareActionAvailable），扫描宿主 apps/web/src/chat/ChatRoutePage.tsx:2006 传 `onShareSession={setShareSessionId}` → 菜单 6 项渲染；Vue menu.vue:610-630 复核仍固定 5 项（pin/rename/clearMessages/batchManage/delete，无 share 值）。**【2026-09-25 第三轮全量复勘】豁免仍成立（run 09-06-09，115 页 over_1pct 仅剩 settings-general 存量+本项）**：像素级——恒 1.166%，hot cells 逐字同前四轮；双端各自跨轮自比 0 差（06-54-20 vs 09-06-09 vue/react 均 0px）＝确定性结构差非扫描噪声；diff 带单带 y278-461 × x80-243（184 行）全落侧栏菜单+其下会话移位带，带外 0；DOM 探针（菜单盒 rect 级）——React [role=menu] [81,274,162,219] 六项（置顶/修改标题/分享/清空消息/批量管理/删除记录）vs Vue t-dropdown 内容盒 [86,279,152,176] 五项（无分享），同位同宽、高差 33px=一枚分享行（32px+1px 分隔线）；代码级——session-sidebar.tsx:328-329 分享项条件渲染复核 + ChatRoutePage.tsx:2006 宿主开关开启复核，均未变。**【2026-09-25 第四轮全量复勘】豁免仍成立（run 09-32-16）**：恒 1.166%，hot cells 逐字同前五轮；09-06-09 vs 09-32-16 双端各自跨轮自比 0px（渲染未变→菜单盒 rect 探针结论沿用：React 219 高六项 vs Vue 176 高五项，33px=分享行）；代码三锚复核——ChatRoutePage.tsx:2006 分享开关开启、menu.vue `value:'share'` 计 0 处、session-sidebar.tsx:328 条件渲染在位。**连续三轮全量恒定同值同 cells，判定为该豁免项稳态终值，后续轮次引用本行即可**。**【2026-09-25 第五轮全量复勘】豁免仍成立（run 09-53-01）**：恒 1.166%，hot cells 逐字同前；09-32-16 vs 09-53-01 双端各自跨轮自比 0px；代码三锚复核未变（ChatRoutePage.tsx:2006 开启/menu.vue share 计 0/session-sidebar.tsx:328 在位） |
...
| 27 | 交互项差异全量来自静态分区底差（弹层同构） | 弹层几何/文案逐字对齐 | 同左 | **静态底差转交判例（px-userprofile-change-password/px-ollama-redetect/px-members-rbac-hint 三项实证）**：判据=①交互项终值与对应 settings-{section} 静态基线同值；②像素级验证弹层 rect 内 diff=0/3/5px（AA 级）；③双端弹层 DOM 几何逐字段一致（popconfirm 566,228,320×132 / role-hint 620,118,360×120）。处置=交互面板项判同构，底差归静态页工作单池（与 settings-general 5.439/settings-integration-api 4.284 存量同性质），不在交互批次重复计欠账。**【2026-09-25 回退】三项实证后被逐项证伪（详见 baseline.md「豁免 #27 回退复勘」）：userprofile 内层排版规则锚定失配、ollama 失败提示 toast 异构、members 弹层 CSS/DOM 双缺口（10.919 本是交互项自身 diff 被错记为静态底差）——已全部回退豁免转工单修复，终值 0/0/0.043（7d351553a/da187f072/ddad150ae）。教训：判据①"终值与静态基线同值"不构成弹层同构证明（同值可能同为缺陷放大），豁免引用本判例前必须弹层 rect 级 DOM 探针实证 |
| 28 | packages/views 抽屉族换 Vue SettingDrawer（t-drawer chrome） | 手写 t-drawer DOM 同构层（SettingDrawerChrome：mask + content-wrapper + 自定义 header/footer + t-* 控件 TSelectLike/TInputLike/TRadioGroupLike/TCheckboxLike/TButtonLike） | SettingDrawer.vue（scoped+global 两块样式）+ IMChannelPanel.vue/AgentEmbedChannelPanel.vue/ApiIntegrationSettings.vue | **手写模拟层登记（#23 同因：本包不带 tdesign-react 依赖；#12 chat/message-face 同款口径）**：①tdesign open 态只置 visibility，滑入位移由组件内联 transform 驱动——同构层须自带 `transform: translateX(0)` 定格，否则内容 wrapper 停在 translateX(100%) 屏外（实测抽屉只有蒙层无内容）；②tdesign `.t-drawer--open > .t-drawer__content-wrapper` 是直接子选择器，wrapper 必须为 root 直接子节点；③t-dropdown/t-popup 弹层坐标被 popper 取整，React 绝对定位保留小数——终值收敛按扫描口径互相关取证后微调 left/top（1.05-1.25px 级），残差=边框亚像素+字形栅格化 AA（平台期 1.2-1.35px 渲染恒定），豁免归 #25 家族；④Vue scoped 样式锚抽屉 :class 家族（.im-channel-drawer/.embed-channel-drawer/.api-key-create-drawer）平移，防与 settings.td.css 全局同名规则串染；⑤测试 seam 保留（form 元素/.wk-form-actions/.wk-im-step.is-active/wk-check-row/role=radio+aria-checked）。实证：px-integrations-add-channel 59.292→0.078、px-integration-api-create-key 71.577→0.113、settings-integration-api 4.284→0。**【2026-09-25 判例④同名串染实例回退+修复】px-integration-api-create-key 0.113→2.374（第二轮全量 05-52-41 复现）**：根因=system/PlatformAPIKeys.vue:648-691 与 integrations/ApiIntegrationSettings.vue:2079-2180 两 Vue 源**共用 .api-key-create-drawer 抽屉类名但 scope-hint 两规则不同源**（platform 源行级 margin:0+capability-item `margin:2px 0 0 24px` 缩进；integration 源行级 8px 0 0+capability-item `margin-top:2px` 无缩进）。644d820fb（批3 platform 侧收编）把 24px 版平移进 settings.td.css §19，5fbde2205（integration 侧修复）把无缩进版写进 views-integrations-u.css:2892——抽屉 portal 到 body 后两套全局规则同页级联，**简写 margin:2px 0 0 24px（td.css）与长写 margin-top:2px（u.css）叠加必残留 24px 左距，与注入顺序无关**；0.113 期=当时该页仅注入 u.css 一套（settings.td.css §19 未进 integration-api 分区模块图），窗口内 7d351553a/ddad150ae 大改 settings.td.css 后两套同页注入即翻转暴露（DOM 探针实证：React capability hint x=762/宽500 vs Vue x=738/宽524，连锁换行多一行内容总高 +91px；对照 0.113 轮 PNG React hint x=741 与 Vue 一致）。修复=④判例落地：PlatformApiKeysPanel drawerClass 加 `pak-create-drawer` 变体，td.css §19 两条差异规则（.scope-hint 与 .api-key-capability-item .scope-hint）改锚变体类，共享规则（dialog/capability 家族两源逐字一致）保留共享锚；终值 2.374→0.113（hot cells 4,7:2.3/7,6:3.1 与平台期同款，归 #25 家族）、platform 侧 px-platform-api-keys-create 0% 无回退；SettingsPage.test/apiKeyCapabilityRender.test 31/31 过。**教训：两 Vue 源同名抽屉类平移双份全局 CSS 时，差异规则必须变体类分域锚定（幂等的共享规则方可共用锚），否则注入顺序一变即串染** |
| 29 | html zoom（字号档位）下文本行基线光栅 snapping 差 | 同一元素内个别文本行整体比对端低/高 1px（字形形状逐像素同构，仅行相位差）；zoom=1（normal 档）零差异 | 同左 | **引擎 relayout/paint 增量路径差豁免（px2-chat-sidebar-collapse 终值 0.238%≈2192px，#16/#18/#22/#25 栅格家族的 zoom 变体；#28 平台期 0.113% 同级先例）**。取证（大字号 zoom 1.125 折叠态）：①确定性——双端各自跨 run 自比 0px（2 次复跑），跨端恒 2192px/126 连通块（块尺寸=单字形盒 7-16×5-14px）；②DOM/CSS 可观测输入逐项全同——元素 rect/行 rects（Range getClientRects 439.391/468.641）/字符级 rect（156 字符逐字千分位）、32 项字体排版 computed 属性（含 fontOpticalSizing/variant 轴/kerning/synthesis/letter-spacing）、canvas measureText 同宽 272.781、markdown 规则书写逐字同源（line-height 1.625/margin 0.25em）、可见 DOM 树全同（pCount 差 5 个全为 display:none 弹层模板，不参与 paint）；③同元素内行为分裂——deterministic P 首行（行盒 fraction .391）双端 0 差、第二行（fraction .641）差 1px，元素级 clip 截图实证；④headed 系统 Chrome（GPU 光栅）下差异带换位（headless y471-486 → headed y228-247）而非消失——光栅路径敏感、非 layout 差；⑤灰阶对照为整行垂直平移模式（y475↔y476 内容互换），非字形形状差。结论：layout 输出（CSS px）完全一致而 paint 阶段文本基线 device-space snapping 分叉，锚定在两端 SPA 更新序列导致的 relayout 增量计算路径（float 累计次序），黑盒层不可观测、无页面 seam（几何/样式/内容/层化全部实证一致，#root 已补 Vue #app 同款 translateZ 合成层无效）。前置修复链（16.06%→0.238%）：React 字号 zoom 机制对齐（GeneralPreferencesPanel applyFontCssVariables 补 html zoom + main.tsx initFontPreferences 启动恢复，Vue useFont/main.ts:27 同款）、#root height:100% 高度链（.wk-shell-1 100vh 在 zoom 下溢出 90px）、.chat.is-sidebar-collapsed 类缺失（ChatSidebarCollapsedContext 通道，主区卡 1020px）、composer chip 盒模型（border-box 30px → Vue 同构 content-box 28px+2×.5px border，zoom 乘法 (28+1.778)×1.125=33.5 vs 30×1.125=33.75，0.25px 差经 clientHeight 取整放大成 scrollTop 1px 错位）。zoom=1 生产默认态零影响（chat/agents/login/settings-general 回归复扫 0/0/0.001/5.439 与存量一致，run 05-34-25） |
...
| 29 | Tabs 活动指示条（.t-tabs__bar）定位机制 | `transform: translateX(<offset>px)` + CSS left:0（TabBar.js:88 setBarStyle，tdesign-react 1.18.3）——transform 路径按真实分数几何光栅化（边缘 AA α≈45%/28%） | `left: <offset>px`（tab-nav-bar.mjs:55 getStyle，tdesign-vue-next 1.20.7）——left 路径把条边对齐到整数像素（边缘 AA α≈4%/96%） | **#16/#22 亚像素家族豁免（px2-settings-{models,sandbox,systemglobal}-tab 实证，残 12-14px/项）**。取证链：①DOM 级——双端 barRect 逐字段一致（models x=424.546875 vs 424.5469055，Δ3e-5px；y/w/h 全同），inline offset 数值同串（67.5469px），nav items 六项 x/w 全同；②代码级——两库 TabBar 均以 `getComputedStyle(item).width` 累加求 offset，差异仅定位属性（transform vs left），CSS `.t-tabs__bar` 规则文本逐字同（position:absolute/left:0/height:3px/radius small）；③决定性实验——React 端把 inline `transform: translateX(67.8906px)` 同值替换为 `left: 67.8906px` 后双端截图 diff **0px**（sandbox 项，/tmp 实验脚本，2026-09-26）；④无页面 seam——offset 由库运行时按文本测量动态计算（label 含计数如「全部(2)」），CSS !important 只能置 transform:none 无法注入动态 left，JS 后改写属脆弱 hack（闪烁/顺序风险）。首 tab 起点多为整数（x=357）故静态页 0 差；点击到第 2 tab 落在分数 offset（x=424.55）才显形。px2-settings-models-tab 0.005% = 本条 14px + #16 play-circle 28px |
|   | （pilot 回填区） | | | |
