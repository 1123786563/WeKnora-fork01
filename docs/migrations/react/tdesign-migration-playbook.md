...
| 13 | Textarea 计数器文本节点切分 | `.t-textarea__limit` 由 `renderLimitText` 渲染为**两个文本节点**（`"0"` + `"/500"`，`tdesign-react/es/textarea/Textarea.js:240-243`），跨节点字偶 kern 丢失 → 计数内后续字形步进比 vue-next 窄 1/64-3/64px | 单次插值单文本节点 `"0/500"`，整串 shaping | 页面代码无 seam（计数器 DOM 在库组件内部，无法合并文本节点）；当计数器左缘落在分数 x 时字形相位差被 AA 放大——orgs ix-orgs-create 实证 35px≈0.004%（单枚 12px 字形，bbox 7×9，两轮确定性复现，orgs R1/R2）。两端排版盒（x/width/字号/颜色）逐项相等，差异纯字形相位 → **扫描豁免**；后续页面若大面积复现可评估 unlayered 全局 kern 对齐方案（Phase 4） |
