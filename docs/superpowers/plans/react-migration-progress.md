...
| T15 Agent、模型、MCP与Skill配置 | review | `cbe5710` + `9b253b6` + `194aee4` + `77d431a` + `b75e452` + `63ce00d` + `3b70657` + `ce24c7b` + `2102a70` + `533e86e` | `pnpm test:shared` 0（176/176）；`pnpm test:web` 0（80/80）；T15 focused configuration 25/25；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm build:web` 0（124 modules）；`node scripts/check-react-boundaries.mjs` 0；`git diff --check` 0 | Strict shared configuration/credential/MCP/model-debug/Skill catalog/sandbox-skill contracts, Web editors and operation panels: static/Node/build evidence passed; live writes, role negatives, real model/MCP calls, OAuth callback, install progress/file contents and browser acceptance remain open | Added dedicated credential subresources with recursive secret rejection/redaction, strict 204 and credential metadata handling, MCP transport/test/tools/OAuth controls, legacy transport fallback, model provider/debug with multipart-without-file support, Skill catalog/installed-skill APIs, Agent selection/share/hide, model debug rendering, Skill installation status/stop and safe file panel, and server-confirmed remove controls. Model usage/occupancy is not claimed because no such Go route exists. |
