...
| T13 Markdown、引用、工具结果与产物 | review | `addbc48` + working tree Markdown/artifact/tool-result renderer | `pnpm test:shared` 0（215/215）；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm test:web` 0（109/109）；`pnpm build:web` 0；`git diff --check` 0 | Pure reference/artifact/tool contracts and React Web Markdown/tool-result source/build evidence passed; protected download/preview, browser XSS/performance, Mermaid hydration, citation navigation and native acceptance not completed | Added the shared React Web Markdown/KaTeX renderer, escaped raw HTML, allow-listed link/image destinations, citation buttons, fenced-code/Mermaid metadata, metadata-only artifact rows, authenticated artifact list/download seams, and a normalized tool-result view covering every known renderer with readable unknown fallback and recursive secret redaction; MessageList and live tool calls use the shared views. Protected live artifact UI/download and browser/native evidence remain. |
