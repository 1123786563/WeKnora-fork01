...
| T24 构建、回归矩阵与灰度发布 | review | working tree (`scripts/check-react-boundaries.mjs`, `scripts/build_react_web_bundle.sh`, root-context Docker/CI, Lite static fallback) | Focused static/protocol verification pending; React Web/Embed candidate bundle, mobile CI workflow, root-context Docker wiring, dedicated Embed fallback test, and release runbook added | Build and boundary automation is implemented; real Docker multi-arch, Nginx/Lite/Wails runtime, backend fixture, browser/OS matrix, native package, performance baseline, and rollback rehearsal are not completed | Keep Vue `frontend/` as production release input until T25; run the runbook against a pinned backend and retain Vue/React artifact checksums before expanding gray rollout |
...
- T24 remains `review`: the local static/typecheck/test/build and single-host Docker/Nginx static evidence is collected, but real backend fixture, multi-arch Docker, Wails installed package, browser/OS matrix, native package/runtime, performance baseline, and rollback rehearsal still need to be collected before T24 can be accepted.
