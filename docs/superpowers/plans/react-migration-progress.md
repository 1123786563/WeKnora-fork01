...
| T24 构建、回归矩阵与灰度发布 | review | `0e886be` + `docs/migrations/react/evidence/t24-lite-live-2026-09-11.md` + `docs/migrations/react/evidence/t24-wails-macos-2026-09-11.md` | Focused static/protocol verification, isolated Lite live backend/browser smoke, and macOS arm64 Wails package smoke passed; React Web/Embed candidate bundle, mobile CI workflow, root-context Docker wiring, dedicated Embed fallback test, and release runbook added | Live Lite health, React Web/deep-link/Embed serving, auto-setup, authenticated `auth/me`, capabilities and KB list passed on isolated SQLite; macOS arm64 Wails build/sign/runtime evidence passed; real Docker multi-arch, Nginx deployment, Windows/Linux Wails, full browser/OS and role matrix, native mobile package/runtime, performance baseline, and rollback rehearsal are not completed | Keep Vue `frontend/` as production release input until remaining T24 gates are evidenced; do not start T25 Vue deletion |
...
- T24 remains `review`: local static/typecheck/test/build, single-host Docker static smoke, isolated Lite backend and browser evidence are collected, but real multi-arch Docker, Nginx deployment, Wails installed package, complete browser/OS and role matrix, native package/runtime, performance baseline, and rollback rehearsal still need to be collected before T24 can be accepted.
