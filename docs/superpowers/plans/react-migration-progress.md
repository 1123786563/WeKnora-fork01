...
| T07 文档上传、列表、目录标签与预览 | review | `45901be` | `pnpm test:shared` 0（24/24）；`pnpm typecheck:shared` 0；`git diff --check` 0 | Contracts/API/domain focused tests and multipart seam: passed; real upload/processing/preview/browser: not completed | Added paginated document DTO validation, processing-state guard, document list API and multipart upload API. UI document view, folders/tags/preview, real backend upload and 413/browser proof remain. |
