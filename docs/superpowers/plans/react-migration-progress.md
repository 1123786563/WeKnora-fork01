...
| T10 聊天协议、状态机与恢复底座 | review | `cfb7d29` + `f2c2fd3` | `pnpm test:shared` 0（41/41）；`pnpm typecheck:shared` 0；`git diff --check` 0 | Existing backend response_type vocabulary, pure reducer and incremental SSE framing tests: passed; live SSE/reconnect/restore: not completed | Added response-type contract helper, deduplicating chat reducer, SSE frame parser, JSON event projection and resumable `Last-Event-ID` request builder. Live transport, persisted restoration and browser reconnection remain. |
| T11 会话、消息和问答主界面 | review | `5fcbae4` | `pnpm test:shared` 0（58/58）；`pnpm typecheck:web` 0；`pnpm build:web` 0；`git diff --check` 0 | Session/message contracts, scoped draft key, React sidebar/composer/message-list/page and Web route compile: passed; real RAG/Agent send, SSE binding, browser acceptance: not completed | Added typed session list/create/history APIs and explicit new-chat flow. `ChatRoutePage.send` intentionally remains a typed unavailable error until T10 SSE transport is wired; title/pin/delete/source filter/suggestion/stream retry are not yet integrated. |
