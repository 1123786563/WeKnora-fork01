...
| T07 文档上传、列表、目录标签与预览 | review | `4ecd4c1` + `dcff3fa` + `29c6d5d` (`feat: add React document migration slice`, `fix: keep unknown document statuses unavailable`, `fix: satisfy shared React boundary guard`) | `pnpm test:shared` 0（142/142）；`pnpm test:web` 0（37/37）；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm build:web` 0；`node scripts/check-react-boundaries.mjs` 0；`git diff --check` 0；2026-09-12 isolated Lite + React Web document route showed authenticated KB navigation, embedding-backed multipart upload/readback, `parse_status=completed`, and rendered `Root · txt Completed` | Contracts/API/domain focused tests, Web route tests, boundary gate and production bundle: passed; isolated Lite document parse and React Web status readback: passed; native chooser submission, progress telemetry, preview/download/search-reference and full renderer acceptance: not completed | Added strict document mutation seams for file/URL/manual sources, reparse/cancel/delete/batch-delete, folder move/rename, batch tag updates and protected preview/download paths; React list/detail routes now expose folder/status/tag filters, selection-preserving bulk actions, upload cancel/413 feedback, and authoritative processing-state display. Unknown backend status values now remain unavailable instead of crashing or being treated as complete. Live native browser upload, search→reference/download, protected binary response handling, full preview renderer inventory, and browser proof remain. Evidence: `docs/migrations/react/evidence/t07-web-upload-live-2026-09-12.md` |
...
| T10 聊天协议、状态机与恢复底座 | review | `cfb7d29` + `f2c2fd3` + `360ab98` | `pnpm test:shared` 0（59/59）；`pnpm typecheck:web` 0；`pnpm build:web` 0；`git diff --check` 0；2026-09-12 Web chat deep-link/error-state 通过；隔离 Lite + 本地流式模型的 React Web SSE 通过 | Existing backend response_type vocabulary, pure reducer, incremental transport reader and SSE framing tests: passed; authenticated Web route mounted; real backend SSE emitted `agent_query`/incremental `answer`/`complete` and the reducer completed the response | Added response-type contract helper, deduplicating chat reducer, browser ReadableStream transport path and resumable `Last-Event-ID` request builder. Continue-stream recovery, cancellation timeout, real third-party/RAG/Agent generation remain unverified. Evidence: `docs/migrations/react/evidence/t06-t17-web-live-2026-09-12.md`, `docs/migrations/react/evidence/t10-t11-web-sse-live-2026-09-12.md` |
| T11 会话、消息和问答主界面 | review | `5fcbae4` + `360ab98` | `pnpm test:shared` 0（59/59）；`pnpm typecheck:web` 0；`pnpm build:web` 0；`git diff --check` 0；2026-09-12 authenticated Web chat deep-link mounted and rendered server error；同一隔离 Lite 会话的 React Web 流式问答与历史回读通过 | Session/message contracts, scoped draft key, React sidebar/composer/message-list/page, shared SSE reducer binding and Web route compile: passed; browser created a session, sent a message, rendered the streamed assistant answer, and authenticated history returned both completed turns | Added typed session list/create/history APIs and explicit new-chat flow. Web submits knowledge chat through shared SSE parsing/reducer. Title/pin/delete/source filter/suggestion, stream restore, real third-party/RAG/Agent generation and native chat remain. Evidence: `docs/migrations/react/evidence/t06-t17-web-live-2026-09-12.md`, `docs/migrations/react/evidence/t10-t11-web-sse-live-2026-09-12.md` |
...
| T15 Agent、模型、MCP与Skill配置 | review | `cbe5710` + `9b253b6` + `194aee4` + `77d431a` + `b75e452` + `63ce00d` + `3b70657` + `ce24c7b` + `2102a70` + `533e86e` | `pnpm test:shared` 0（176/176）；`pnpm test:web` 0（80/80）；T15 focused configuration 34/34；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm build:web` 0（124 modules）；`node scripts/check-react-boundaries.mjs` 0；`git diff --check` 0；2026-09-12 隔离 Lite + Chrome 配置页加载、Agent hide 操作通过 | Strict shared configuration/credential/MCP/model-debug/Skill catalog/sandbox-skill contracts, Web editors and operation panels: static/Node/build evidence passed; isolated browser Agent render/hide and empty Model/MCP disabled-state passed; live writes beyond Agent hide, role negatives, real model/MCP calls, OAuth callback, install progress/file contents and production/provider acceptance remain open. Skill catalog returned 500 because isolated Lite lacks `tenant_skill_catalog`; UI preserved the failure. Evidence: `docs/migrations/react/evidence/t15-configuration-live-2026-09-12.md` | Added dedicated credential subresources with recursive secret rejection/redaction, strict 204 and credential metadata handling, MCP transport/test/tools/OAuth controls, legacy transport fallback, model provider/debug with multipart-without-file support, Skill catalog/installed-skill APIs, Agent selection/share/hide, model debug rendering, Skill installation status/stop and safe file panel, and server-confirmed remove controls. Model usage/occupancy is not claimed because no such Go route exists. |
