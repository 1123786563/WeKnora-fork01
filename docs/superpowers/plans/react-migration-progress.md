...
| T13 Markdown、引用、工具结果与产物 | review | `addbc48` + `04707b3` + `4d364f7` + `4c53589` + `0dba0c3` + `c5c2fa9` + `466ed8d` + `ad735a4` + `c0d3471` + `141ba7d` + working tree fixture follow-up | `pnpm test:shared` 0（221/221）；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm test:web` 0（111/111）；`pnpm test:desktop` 0（2/2）；`pnpm test:embed` 0（3/3）；`pnpm test:mobile` 0（77/77）；五端 typecheck 0；`pnpm build:web`/`build:desktop-renderer`/`build:embed` 0；iOS/Android Expo export 0；浏览器 fixture 通过 | Pure reference/artifact/tool contracts and React Web Markdown/tool-result/reference source/build evidence passed; browser fixture now proves one Mermaid SVG reaches the DOM with zero rendered scripts and a removed SVG event attribute, but protected download/preview, broad browser XSS/performance, live citation navigation and native acceptance are not completed | Added the shared React Web Markdown/KaTeX renderer, escaped raw HTML, allow-listed link/image destinations, citation buttons, fenced-code/Mermaid metadata plus lazy strict Mermaid/DOMPurify hydration, a real development fixture using the same DOM renderer, metadata-only artifact rows, authenticated artifact list/download seams, normalized tool-result views covering every known renderer with readable unknown fallback and recursive secret redaction, grouped stream/history references with safe web links and local chunk activation, and a Web host adapter for absolute HTTP(S) citation targets. Protected live artifact UI/download and native evidence remain. |
