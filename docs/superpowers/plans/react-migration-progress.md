...
| T13 Markdown、引用、工具结果与产物 | review | `addbc48` + working tree Markdown/artifact renderer | `pnpm test:shared` 0（212/212）；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm test:web` 0（107/107）；`pnpm build:web` 0；`git diff --check` 0 | Pure reference/artifact/tool contracts and React Web Markdown source/build evidence passed; protected download/preview, browser XSS/performance, Mermaid hydration, citation navigation and native acceptance not completed | Added the shared React Web Markdown/KaTeX renderer, escaped raw HTML, allow-listed link/image destinations, citation buttons, fenced-code/Mermaid metadata, metadata-only artifact rows, and authenticated artifact list/download seams; MessageList now uses the renderer. Protected live artifact UI/download and browser/native evidence remain. |
