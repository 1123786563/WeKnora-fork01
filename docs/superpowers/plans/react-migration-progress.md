...
| T13 Markdown、引用、工具结果与产物 | review | `addbc48` + working tree Markdown renderer | `pnpm test:shared` 0（208/208）；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm test:web` 0（107/107）；`pnpm build:web` 0；`git diff --check` 0 | Pure reference/artifact/tool contracts and React Web Markdown source/build evidence passed; protected download/preview, browser XSS/performance, KaTeX/Mermaid hydration, citation navigation and native acceptance not completed | Added the shared React Web Markdown renderer, escaped raw HTML, allow-listed link/image destinations, citation buttons, fenced-code/Mermaid metadata and readable TeX spans; MessageList now uses the renderer. Protected live artifact UI/download and browser/native evidence remain. |
...
- The current React Web increment adds `packages/views/src/chat/markdown.ts` and wires `MessageList` through a safe `marked` renderer. Fresh verification is `pnpm test:shared` 208/208, `pnpm typecheck:shared` 0, `pnpm typecheck:web` 0, `pnpm test:web` 107/107, `pnpm build:web` 0, and `git diff --check` 0. Evidence: `docs/migrations/react/evidence/t13-react-markdown-renderer-2026-09-12.md`.
- The renderer currently provides readable TeX spans and Mermaid code metadata, not KaTeX typesetting or Mermaid SVG hydration. Citation navigation, protected artifact UI/download, browser XSS/performance and native acceptance remain open; T13 stays `review`.
