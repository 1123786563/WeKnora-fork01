...
| T10 聊天协议、状态机与恢复底座 | review | `cfb7d29` | `pnpm test:shared` 0（38/38）；`pnpm typecheck:shared` 0；`git diff --check` 0 | Existing backend response_type vocabulary captured in contracts; pure reducer tests: passed; live SSE/reconnect/restore: not completed | Added response-type contract helper and deduplicating chat reducer for answer/thinking/tools/approvals/references/complete/stop/error. SSE transport, persisted restoration and browser reconnection remain. |
