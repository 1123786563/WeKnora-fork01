...
| T08 FAQ与Wiki编辑/版本 | review | `736eca4` + `3d8daa6` + `1d08229` + `e1070b1` | `pnpm test:shared` 0（144/144）；`pnpm test:web` 37/37；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm build:web` 0；`git diff --check` 0 | Shared Wiki/FAQ API and React Wiki editor route/build: passed; FAQ UI, live backend and browser conflict acceptance: not completed | Migrated pure line/revision diff and typed Wiki page list/get/create/update/delete/revision/revert paths with optimistic version payloads. Added React Wiki search/create/edit surface and explicit 409 conflict/reload-latest handling, plus a strict FAQ list/create/update/upsert/field/tag/batch-delete client seam. FAQ screen, revision viewer/revert UI, live backend and browser proof remain. |
...
- T08 remains `review`: FAQ list/create/update/batch/delete contracts and UI, revision viewer/revert UI, live backend conflict behavior, and browser acceptance are still missing. No accepted status is inferred from the Wiki-only slice.
