...
| T23 移动管理功能与能力矩阵补齐 | review | `aa35f4` + working tree mobile Wiki/FAQ editor slice | `pnpm test:shared` 0（157/157）；`pnpm test:mobile` 0（24/24）；`pnpm typecheck:mobile` 0；knowledge editor/reference focused 8/8；Expo iOS/Android exports 0；Web/boundary/diff 0 | Explicit mobile capability matrix, capability-gated management hub, role-gated Wiki/FAQ list/detail/edit/create surface, read-only configuration/identity surfaces, production iOS/Android JS bundles, and prior real Lite capability projection/identity/configuration screens: passed; role-specific live backend matrix and remaining management writes: not completed | Added native FAQ/Wiki editor route with shared `get/create/update` APIs, server-owned Wiki optimistic `version`, local required-field validation, explicit 403/error handling and 409 reload-latest action. Owner/admin workspaces see New/Edit/Save; other roles remain read-only and server permissions remain authoritative. The editor does not add offline writes or generic JSON fields. Unsupported sandbox/offline/embed admin surfaces remain visible with reasons. Evidence: `docs/migrations/react/evidence/t23-mobile-reference-editor-2026-09-11.md`, `docs/migrations/react/evidence/t23-mobile-lite-capabilities-live-2026-09-11.md` |
