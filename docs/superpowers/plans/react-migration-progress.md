...
| T13 Markdown、引用、工具结果与产物 | review | `addbc48` + working tree Markdown/artifact/tool-result/reference renderer | `pnpm test:shared` 0（217/217）；`pnpm typecheck:shared` 0；`pnpm typecheck:web` 0；`pnpm test:web` 0（109/109）；`pnpm build:web` 0（145 modules）；`git diff --check` 0 | Pure reference/artifact/tool contracts and React Web Markdown/tool-result/reference source/build evidence passed; protected download/preview, browser XSS/performance, Mermaid hydration and native acceptance not completed; citation activation is locally wired but live backend navigation remains unverified | Added the shared React Web Markdown/KaTeX renderer, escaped raw HTML, allow-listed link/image destinations, citation buttons, fenced-code/Mermaid metadata, metadata-only artifact rows, authenticated artifact list/download seams, normalized tool-result views covering every known renderer with readable unknown fallback and recursive secret redaction, and a grouped reference panel for stream/history references with safe web links and local chunk activation; MessageList and live tool calls use the shared views. Protected live artifact UI/download and browser/native evidence remain. |
...
- The current React Web increment adds `packages/views/src/chat/markdown.ts`, wires `MessageList` through a safe `marked`/KaTeX renderer, adds strict metadata-only artifact list/download seams in `@weknora/api-client`, connects the Web save-file action, maps normalized tool results into readable structured views with recursive secret redaction, and renders grouped stream/history references with safe web links and local chunk activation. Fresh verification is `pnpm test:shared` 217/217, `pnpm typecheck:shared` 0, `pnpm typecheck:web` 0, `pnpm test:web` 109/109, `pnpm build:web` 0 (145 modules), and `git diff --check` 0. Evidence: `docs/migrations/react/evidence/t13-react-markdown-renderer-2026-09-12.md`.
