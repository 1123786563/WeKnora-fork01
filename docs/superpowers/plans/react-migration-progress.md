...
| T03 登录、刷新、OIDC与凭证隔离 | review | `118d15d` + `2f8697a` + `6118321` (`fix: accept normalized refresh responses`) | `pnpm test:shared` 0（126/126）；`pnpm --filter @weknora/web test` 0（22/22）；`pnpm --filter @weknora/web exec tsc -p tsconfig.json --noEmit` 0；`pnpm --filter @weknora/web build` 0；boundary check 0；`git diff --check` 0；真实后端/OIDC/浏览器 0 | 静态/Node mock 与 Web 生产构建：通过；真实后端、OIDC浏览器回调、React登录页：未完成 | 已接入旧 `weknora_*` refresh token、浏览器 CredentialAdapter、401 单飞重试（含 stream）、refresh endpoint 排除、Bearer/Embed 隔离和持久化轮换；coordinator 同时接受 auth.refresh 已解析 token pair；真实后端/OIDC/浏览器回调仍待补 |
