...
| 5. checkpoint → Session | 现有 probe 使用 `noop.NewService()`；Task 2 的 `inmemory.NewSessionService` 仅为 SDK 探针，且 v1.10.0 race gate 失败。当前 durable path 从 run store/repository checkpoint 恢复，尚未证明与最终 Session Service 历史权威的协调。 | tRPC Session Service 是新会话执行历史的唯一权威；业务 Run/ToolCall/UserDecision 继续记录执行身份、审批、结果与接管资格，checkpoint 继续记录图执行位置和状态。恢复以稳定事件身份协调这三者，UI 读模型仅作可重建投影，不能维护第二份可写历史或跨 tenant/session 注入状态。 | P1 定义 Session、run/decision、checkpoint 的权威边界和迁移契约；P2 连接 SDK Session history 与工具/审批语义且不扩大权限；P3 在恢复时协调 Session 写入、run/decision 日志和 checkpoint scope；P7 验证 race、跨进程、租户隔离、升级、回滚和投影重建。 | 先选定 SDK/Session 修复或版本并让 native race probe 通过；再在 SQLite/PostgreSQL 以同一稳定事件 ID 做跨进程/双租户重开，确认 Session history 可恢复、run/tool-decision 与 checkpoint 保留各自职责，且 UI 投影可由 Session/事件重建。 |
