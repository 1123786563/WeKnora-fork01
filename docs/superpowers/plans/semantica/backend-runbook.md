...
active_generation=expected）→ semantic。并发 Promote 恰一胜；失败返回
`ErrPromotionRejected`。切换后旧请求沿既有 read lease 完成，且仍受最新 deny 约束。

## 回滚（Rollback）

**先补齐再切换**：native 检查点必须 ≥ 当前源最高 revision，否则
`ErrNativeCatchupRequired`（绝不静默回旧图）。追赶内容：切换期间的文档更新/删除
（同一 outbox 输入）；追赶完成后核对源 revision 清单一致，CAS semantic→native。
