...
- `SemanticModelCapabilityIssuer.Issue(ctx, scopeRef, invocationKey) (SemanticModelIssuedCapability, error)` verifies the live A01 scope with `Resolve`, loads the active owner-KB policy, derives run/call identity in Go, obtains the versioned commercial price/rate estimate, persists/reuses the owner task budget, and signs a short-TTL HS256 token. `SemanticModelCapabilityIssuer.Verify(ctx, token) (SemanticModelCapability, error)` verifies signature/version/audience/expiry, resolves the embedded A01 scope live, reloads owner model/policy/funding/rates, and rejects a changed policy/model/price before any quota or budget mutation. Python cannot request another model, funding source, upper, deadline, owner, or tenant.
- `SemanticModelIssuedCapability` is only the issue-route response `{token, max_input_bytes, max_output_tokens, expires_at}`; `SemanticModelCapability` is Go-only. Implement issuer+verifier in `internal/application/service/semantic_model_capability.go`; inject the A01 scope resolver, active `SemanticModelPolicyService`, same `SemanticModelPricingResolver`/immutable rates used by policy validation and final settlement, and invocation repository. No provider data is accepted from Python.
...
- Modify: `internal/application/repository/semantic_model_invocation.go` and `internal/application/repository/semantic_model_invocation_test.go` to persist the irreversible dispatch boundary
- Modify: `internal/application/repository/commercial/usage.go` and `internal/application/repository/commercial/usage_test.go` to persist final BYOK model usage without a credit-settlement outbox event
- Modify: `internal/types/interfaces/semantic_model.go` for the `MarkDispatched` transition
- Modify: `internal/container/container.go`
...
- `SemanticModelInvocationStore.MarkDispatched(ctx, capability)` atomically changes only a `claimed` invocation to `dispatched` immediately before provider I/O. `FailBeforeDispatch` can change only `claimed` to terminal `failed_before_dispatch` and release quota; `Complete` can change only `dispatched` to `completed`; `MarkUnknown` can change `claimed` or `dispatched` to `unknown` so an uncertain database result around either dispatch marker is conservatively reconciled. Both `claimed` and `dispatched` count against owner task quota, and `Claim` returns either as `in_flight`; only `claimed_new` can cross the boundary. If persisting either dispatch marker fails after the other succeeds (or its commit result is uncertain), do not call the provider, do not release the possibly-dispatched budget hold, and conservatively retain quota/hold for reconciliation.
...
- `UsageStore.RecordRawModelUsage(ctx, fact)` is a narrow non-settling path for a final BYOK `ServiceModel` fact. It persists the physical raw usage and current revision under the source owner, with zero `ChargeMicro`, but emits no `usage_settlement` outbox event. It rejects any funding/service/status other than BYOK/final/model. Identical concurrent/replayed revisions converge idempotently; same-key different-content revisions conflict; an out-of-order lower revision may be stored for audit but never moves the current pointer backward. This is distinct from generic `Record`, whose final facts intentionally emit settlement events for billable service dimensions.
- The dispatch order is: validate capability/owner policy/A01 scope and request bounds → atomically claim invocation and reserve owner task call/token quota → only `claimed_new` may continue, `completed_replay` returns saved bytes without another dispatch, while `in_flight`/`unknown` deny replay → for platform funding reserve an unstarted owner-tenant hold → resolve the owner-tenant `GetChatModel` → persist invocation `dispatched` state and mark any platform reservation dispatched immediately before `chat.Chat` → invoke with cancellation and max-output cap → record provider-observed usage under the owner → finish a platform reservation exactly once with the exact returned ID. BYOK makes no platform model-credit reservation but still atomically consumes the owner policy's per-task call/token quota. On known pre-dispatch failure, `BudgetStore.ReleaseReservation` releases the still-held platform reservation and the invocation ledger returns its call/token quota; after the dispatched boundary, store `unknown` and retain the full quota plus any platform hold for reconciliation.

- [ ] **Step 1: Write failing gateway ordering and accounting tests**

Required RED cases: platform budget denial performs zero `GetChatModel` calls and zero provider calls; owner policy disabled/ACL invalid/quota exhausted are denied before model resolution; model resolution failure releases only an undispatched platform hold and call/token quota; either dispatch-marker persistence failure never invokes the provider and never releases a possibly-dispatched hold; dispatched calls cannot use `FailBeforeDispatch`; platform success finishes exactly once with the owner reservation and produces one owner-tenant final usage fact carrying server-derived funding and exact immutable rate version; BYOK final raw usage is persisted under the owner with no platform Credits reservation/finish AND no `usage_settlement` outbox event while task call/token quotas remain consumed; duplicate concurrent raw BYOK facts converge without a unique-conflict failure and stale revisions cannot roll back the current pointer; missing/inconsistent/negative/over-cap usage is `unknown`, not a final or zero fact, and retains the full token quota; cancellation after dispatch is unknown; completed identical replay returns stored bytes without a second claim-owned dispatch marker, reserve, model/provider call, or finish.
