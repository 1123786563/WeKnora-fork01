...
## Cross-Tenant Model and Billing Decision (Must Resolve Before Implementation)

A01 carries both the KB source-owner tenant and the authenticated requester tenant. `CONTEXT.md` defines tenants as independent billing units and says organization sharing does not merge their payment responsibility. Current KB-share permissions authorize reading KB content; they do not define a general owner-model delegation or a Semantica payer. Native shared-KB retrieval resolves the source tenant's embedding model, but that path does not establish chat-model authorization or commercial billing. Shared custom Agents also resolve the source Agent owner's model, yet use a distinct AgentShare authorization path and have no production commercial-gate binding for classic QA; their requester-scoped aggregate chat usage does not settle provider charges. Therefore neither neighboring path determines Semantica's model credential or payer.

Before Task 1, decide the model-credential tenant and commercial budget/usage tenant for a shared-KB request. The current draft assumes owner-configured model policy but leaves credential sharing and payer binding unresolved. If the requester supplies the model, Task 1 must move model selection into requester-side policy; if the KB owner supplies it, the owner must explicitly grant model-credential use and the budget payer must be defined. Capability claims must carry separate `model_tenant_id` and `billing_tenant_id`, both derived by Go from the chosen authorization policy, never from Python wire input. Do not implement the policy schema, capability issuer, or budget debit until this decision is recorded in an approved spec/ADR or explicit product ruling. Evidence pointers: `internal/application/service/knowledgebase_search.go` source-tenant embedding model path; `internal/handler/session/qa.go` effective tenant for shared-Agent model/KB/MCP; `internal/agent/commercial_adapter.go` requires a distinct trusted budget binding; `CONTEXT.md` defines independent tenant billing and BYOK cost bearing.
...
**Interfaces:**
- `SemanticModelPolicy` is keyed by source-owner `(tenant_id, kb_id)` and contains selected tenant-owned chat `model_id`, immutable commercial `price_version`, maximum input/output tokens, task credit upper, monotonically increasing `policy_version`, and update audit identity/time. The owner KB must exist and be active; the selected model must resolve within the owner tenant.
...
- `SemanticModelPolicyService.Get(ctx, ownerScope)` returns the live policy or a not-configured error. `Put(ctx, ownerScope, input)` requires existing KB-write/manage permission, validates the model and finite token/credit limits, resolves the immutable price version, and writes a new version transactionally. It never accepts or edits price rates. The caller cannot write `policy_version`.
...
Test that an owner KB manager can persist and read a valid model policy; another tenant, a viewer, a missing/deleted KB, a model outside the owner tenant, an unknown price version, an unresolved immutable rate version, or an unbounded/invalid token and credit limit are rejected without writes. Replacing a policy must increment `policy_version` and preserve the previous version for audit. Use formal PostgreSQL/SQLite migrations, not GORM-only schema creation.
...
- `SemanticModelCapability` is Go-only and binds the resolved owner scope, A01 subject/requester/purpose/scope hash/expiry, model-policy version, model tenant, billing tenant, model ID, model funding, task run ID, call ID, maximum output, per-call upper, price version, and deadline.
...
- Derive `runID` as `semantic-run:` plus SHA-256 of the exact A01 `scope_ref`, and derive `callID` as a namespaced SHA-256 of `scope_ref` plus `invocationKey`; call `BudgetStore.EnsureTaskBudget(ctx, billingTenant, runID, policy.TaskUpper, scopeExpiry)` idempotently only after the cross-tenant model/billing ruling above. The invocation key is only a stable deduplication input inside that authenticated scope, never an authorization field.
- `SemanticModelInvocationStore.Claim(ctx, tenant, callID, requestHash)` atomically creates `in_flight` or returns `completed` with the stored result for an identical request. A different request hash conflicts. States include `in_flight`, `completed`, `unknown`, and `failed_before_dispatch`; transition and reservation ID are durable.

- [ ] **Step 1: Write failing capability and concurrency tests**

Test expired/wrong-audience/tampered tokens, mismatched A01 scope, deleted owner, revoked scope, policy-version change, and unknown rate version; assert no ledger claim or budget call on rejection. Race two identical claims and assert one new claim. Reuse the key with a different payload hash and assert conflict. Assert stored response is returned on completed replay without a second dispatch.
...
Use HS256 only and a new secret distinct from both the A01 scope signer and internal service bearer. Hash messages and parameters for invocation identity; never persist capability tokens or provider credentials. Persist the model result under the owner tenant and only expose it through the authenticated internal route. Make token expiry no later than both A01 scope expiry and model-policy deadline.
...
- The semantic-budget adapter performs `BudgetStore.Reserve`, `MarkReservationDispatched`, and `ReleaseReservation` as distinct transitions. It uses the separately authorized `billing_tenant_id`, while model lookup uses `model_tenant_id`; usage facts, reservations and budget rows must all agree on the billing tenant. The decision section above must settle whether an owner-provided model is authorized for the requester and who bears its model costs.
- The dispatch order is: validate capability/policy/A01 scope and request bounds → atomically claim invocation → reserve an unstarted call hold → resolve `GetChatModel` → persist dispatched state and mark the reservation dispatched immediately before `chat.Chat` → invoke with cancellation and max-output cap → record provider-observed usage → finish once with the exact returned reservation ID. On a known pre-dispatch failure, `BudgetStore.ReleaseReservation` releases the still-held reservation and the ledger records `failed_before_dispatch`; after the dispatched boundary, store `unknown` and retain the hold for reconciliation.

- [ ] **Step 1: Write failing gateway ordering and accounting tests**

Required RED cases: budget denial performs zero `GetChatModel` calls and zero provider calls; model resolution failure releases only an undispatched hold; a successful call finishes exactly once with the reservation returned by `Begin`; token usage becomes one final `commercial.UsageFact` with server-derived funding and rate version; BYOK raw usage is retained but does not charge model credits; missing/partial usage is `unknown`, not a zero fact; cancellation after dispatch is unknown; completed identical replay returns stored bytes with no second `Begin`/model call/`Finish`.
