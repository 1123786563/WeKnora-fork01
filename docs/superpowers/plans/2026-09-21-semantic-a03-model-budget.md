...
- The dispatch order is: validate capability/owner policy/A01 scope and request bounds → atomically claim invocation and owner task quotas → for platform funding reserve an unstarted owner-tenant hold → resolve the owner-tenant `GetChatModel` → persist dispatched state and mark any platform reservation dispatched immediately before `chat.Chat` → invoke with cancellation and max-output cap → record provider-observed usage under the owner → finish a platform reservation exactly once with the exact returned ID. BYOK makes no platform model-credit reservation but still atomically consumes the owner policy's per-task call/token quotas. On a known pre-dispatch platform failure, `BudgetStore.ReleaseReservation` releases the still-held reservation and the ledger records `failed_before_dispatch`; after the dispatched boundary, store `unknown` and retain any platform hold for reconciliation.

- [ ] **Step 1: Write failing gateway ordering and accounting tests**

Required RED cases: platform budget denial performs zero `GetChatModel` calls and zero provider calls; owner policy disabled/ACL invalid/quota exhausted are denied before model resolution; model resolution failure releases only an undispatched platform hold; a successful platform call finishes exactly once with the owner reservation; token usage becomes one owner-tenant final `commercial.UsageFact` with server-derived funding and rate version; BYOK raw usage is retained, consumes token/call quotas, and creates no platform model-credit settlement; missing/partial usage is `unknown`, not a zero fact; cancellation after dispatch is unknown; completed identical replay returns stored bytes with no second `Begin`/model call/`Finish`.
