...
- `SemanticModelCapability` is Go-only and binds the resolved owner scope, A01 subject/requester/purpose/scope hash/expiry, model-policy version, model ID, model funding, task run ID, call ID, maximum output, per-call upper, price version, and deadline.
...
- The semantic-budget adapter performs `BudgetStore.Reserve`, `MarkReservationDispatched`, and `ReleaseReservation` as distinct transitions. It uses the source-owner tenant because the persisted KB model policy and selected model belong to that tenant; policy, model resolution, budget and usage therefore share one commercial tenant. If product policy instead charges a shared-KB requester tenant, change this binding before implementation rather than infer it from Python input.
