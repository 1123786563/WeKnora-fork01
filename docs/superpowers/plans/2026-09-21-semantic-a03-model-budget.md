...
- `UsageStore.RecordRawModelUsage(ctx, fact)` is a narrow non-settling path for a final BYOK `ServiceModel` fact. It persists the physical raw usage and current revision under the source owner, with zero `ChargeMicro`, but emits no `usage_settlement` outbox event. It rejects any funding/service/status other than BYOK/final/model. This is distinct from generic `Record`, whose final facts intentionally emit settlement events for billable service dimensions.
