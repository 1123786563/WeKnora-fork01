...
- WX-02、ALI-02：unit 层 pass（C02 e269415 TestWechat 9/9、C03 187570b TestAlipay 14/14，回调验签／解密／重放）。**ALI-01/03/04：2026-09-12 官方沙箱 runtime pass**（环境门控测试 c472408 + 修复提交；precreate 真实下单+QR、签名查单真实 ACQ.TRADE_NOT_EXIST、未付退款同键一致拒绝；沙箱联验发现并修复 C03 两处真实缺陷：请求签名需含 sign_type（官方 SDK 实证）、Close 业务码吞噬；渠道行为发现：未支付预下单对 query/close 不可见，完整生命周期归 ALI-05 交互流程；环境注记：官方沙箱非生产渠道）。WX-01/03/04 pending；WX-05、ALI-05 blocked-env（ALI-05 待沙箱买家钱包扫码交互）。
