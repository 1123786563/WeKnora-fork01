/**
 * Message timestamp formatting and conversation date separators.
 *
 * Ported from frontend/src/utils/messageTimestamp.ts (Vue implementation is
 * authoritative). Pure functions only so they run in Node tests and any
 * client runtime.
 */
...
export function shouldShowConversationTimestamp(
  messages: readonly ConversationTimestampMessage[],
  index: number,
  gapMs = CONVERSATION_TIMESTAMP_GAP_MS,
): boolean {
...
/**
 * Label for a date separator. Labels are injected so client packages own the
 * user-facing strings (TODO(migration): move into @weknora/i18n once chat
 * migrates off local maps).
 */
export interface ConversationTimestampLabels {
  today: string
  yesterday: string
  /** Formatter for a day inside the current year, e.g. "3/5". */
  thisYear: (model: ConversationTimestampModel) => string
  /** Formatter for a day in an earlier year, e.g. "2024/3/5". */
  otherYear: (model: ConversationTimestampModel) => string
}

export function formatConversationTimestampLabel(
  value: unknown,
  labels: ConversationTimestampLabels,
  now = new Date(),
): string {
  const model = getConversationTimestampModel(value, now)
  if (!model) return ''
  if (model.kind === 'today') return `${labels.today} ${model.time}`
  if (model.kind === 'yesterday') return `${labels.yesterday} ${model.time}`
  if (model.kind === 'thisYear') return `${labels.thisYear(model)} ${model.time}`
  return `${labels.otherYear(model)} ${model.time}`
}