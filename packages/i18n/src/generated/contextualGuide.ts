// Ported BYTE-EXACT from frontend/src/i18n/locales/{zh-CN,en-US,ja-JP,
// ko-KR,ru-RU}.ts -> contextualGuide block (zh-CN.ts:6689, en-US.ts:97,
// ja-JP.ts:97, ko-KR.ts:6687, ru-RU.ts:6687 = 89 keys x 5 locales = 445
// values; key order follows the Vue locale files). The block is the full
// copy surface of the Vue contextual guides: SpotlightGuide chrome
// (stepOf/skip/prev/next/done/interactHint) plus step title/desc pairs
// for the seven tours in frontend/src/config/contextualGuides.ts (kbList,
// kbCreate, kbDetail, chat, tenantModels, agentList, agentCreate —
// tenantModels with steps + stepsAgent variants and needChatModelFirst).
// vue-i18n message syntax survives verbatim: named params ({current},
// {total}) and the literal escapes ({'@'} in chat.steps.kb.desc /
// kbDetail.steps.done.desc) are preserved exactly like the locale source.
// Pinned by packages/i18n/test/contextualGuide.test.ts, which also
// byte-pins the views local mirror (packages/views/src/guides/
// contextual-guide-messages.ts) to this table — views cannot depend on
// the @weknora/i18n package name (guides/steps.ts +
// chat/messages.ts precedent).
export const contextualGuideMessages: Record<string, Record<string, string>> = {
  'zh-CN': {
