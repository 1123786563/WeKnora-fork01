...
export interface DialogProps {
  ...
  onClose: () => void;
  closeLabel?: string;
  /** Optional extra class on the dialog panel (e.g. width variants). */
  className?: string;
}
...
export function Dialog({ open, title, children, onClose, closeLabel = 'Close', className }: DialogProps) {
  ...
  return <div className="wk-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className={className ? `wk-dialog ${className}` : 'wk-dialog'} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} ref={dialogRef}>
