...
export function Textarea({ className, ...props }: TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea className={cn('w-full rounded-[3px] border border-line-input bg-surface px-2 py-1.5 text-[13px] text-black/90 outline-none transition-colors placeholder:text-placeholder hover:border-accent focus:border-accent focus:ring-2 focus:ring-accent/20 disabled:cursor-not-allowed disabled:border-line-input disabled:bg-disabled-bg disabled:text-black/25', className)} {...props} />;
}
