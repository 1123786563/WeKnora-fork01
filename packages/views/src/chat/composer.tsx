 ...
 */
export function ChatComposer({ draft, disabled = false, onDraftChange, onSubmit, agents, selectedAgentId, onAgentChange, modelLabel, streaming = false, onStop }: ChatComposerProps) {
  ...
  return <form className="wk-chat-composer" onSubmit={submit}>
    <label className="wk-chat-visually-hidden" htmlFor="wk-chat-draft">直接向模型提问</label>
      ...
      <div className="wk-chat-control-bar">
        <div className="wk-chat-control-left">
          {agents && onAgentChange ? <span className="wk-chat-agent-chip">
            ...
            >
              <option value="">快速问答</option>
              {agents.map((agent) => <option key={agent.id} value={agent.id} disabled={agent.disabled}>{agent.name}{agent.disabled ? ' · 不可用' : ''}</option>)}
            </select>
          ...
          <button type="button" className="wk-chat-control-icon" aria-label="上传附件" disabled={disabled} title="上传附件">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
              <path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48" />
            </svg>
          </button>
          <button type="button" className="wk-chat-control-icon" aria-label="知识库" disabled={disabled} title="知识库">
        ...
        <div className="wk-chat-control-right">
          <span className="wk-chat-model-chip" role="note" aria-label="对话模型" title="对话模型">
            <span className="wk-chat-model-name">{modelLabel ?? '对话模型'}</span>
          ...
          {showStop && onStop ? <button type="button" className="wk-chat-stop wk-chat-send" aria-label="停止生成" title="停止生成" onClick={onStop}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor" aria-hidden="true"><rect x="2.5" y="2.5" width="9" height="9" rx="1.5" /></svg>
          </button> : <button type="submit" className="wk-chat-send" disabled={disabled || !draft.trim()} aria-label="发送" title="发送 · Enter">
