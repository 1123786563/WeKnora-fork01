...
function parseEntry(value: unknown): FAQEntry { const row = object(value, 'Invalid FAQ entry'); if (typeof row.id !== 'number' || !Number.isSafeInteger(row.id) || row.id < 0) throw new Error('Invalid FAQ entry id'); if (typeof row.standard_question !== 'string' || typeof row.is_enabled !== 'boolean' || typeof row.is_recommended !== 'boolean') throw new Error('Invalid FAQ entry fields'); return { ...row, id: row.id, standard_question: row.standard_question, similar_questions: stringArray(row.similar_questions, 'similar_questions'), negative_questions: stringArray(row.negative_questions, 'negative_questions'), answers: stringArray(row.answers, 'answers'), is_enabled: row.is_enabled, is_recommended: row.is_recommended } as FAQEntry; }
