...
test('uses exact Tenant Release routes, methods, and request bodies', async () => {
  ...
  const metadata = { semantic_version: '1.0.0', display_name: 'Helper', summary: 'Help', supported_languages: ['en'], use_cases: ['support'], license_id: 'MIT' };
  await api.submit('version-1', metadata);
  await api.listReviewQueue();
  await api.review('submission/1', { expected_digest: submission.bundle_digest, decision: 'approve' });
  ...
  assert.deepEqual(calls, [
    ...
    { method: 'POST', path: '/api/v1/marketplace/tenant/release-submissions/submission%2F1/review', body: { expected_digest: submission.bundle_digest, decision: 'approve' } },
...
test('preserves rejected review response with no Release and rejects malformed response', async () => {
  const api = createTenantReleaseApi(async () => ({ success: true, data: { review: { ...review, decision: 'reject' }, release: null } }));
  const result = await api.review('submission-1', { expected_digest: submission.bundle_digest, decision: 'reject', reason: 'Needs changes' });
  assert.equal(result.release, null);
  const malformed = createTenantReleaseApi(async () => ({ success: true, data: { review, release: null } }));
  await assert.rejects(malformed.review('submission-1', { expected_digest: submission.bundle_digest, decision: 'approve' }));
});
