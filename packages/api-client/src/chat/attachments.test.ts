...
test('uploads native chat attachments through the session-scoped multipart route', async () => {
  ...
  const result = await api.upload('session/a', { file: { uri: 'content://notes', name: 'notes.md', type: 'text/markdown' }, agentId: 'agent-7' }, controller.signal);
  ...
  // Native sources must route through the local file port (sendMultipartFile)
  // so content:// and file:// URIs become multipart bytes instead of a
  // device-local path forwarded to the backend.
  assert.deepEqual(captured?.nativeFile, { uri: 'content://notes', name: 'notes.md', type: 'text/markdown' });
  assert.equal(captured?.multipartFields?.agent_id, 'agent-7');
});
