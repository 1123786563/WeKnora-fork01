...
def write_lab_env(path, env=None):
    ...
    if os.path.exists(path):
        raise LabError(f"refusing to overwrite existing {path} (delete it first to regenerate)")
    flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL
    try:
        fd = os.open(path, flags, 0o600)
    except FileExistsError:
        raise LabError(f"refusing to overwrite existing {path} (delete it first to regenerate)") from None
    try:
