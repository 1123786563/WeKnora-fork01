...
除了手工填写服务地址，插件开发者还可以发布**版本化插件清单**，由空间管理员粘贴清单地址审阅后安装——插件由此获得版本治理（固定版本、差异升级、漂移阻断）与写操作默认关闭等能力，而手工 MCP 服务保持原有行为不变。

### 管理员安装流程

1. 开发者自行部署远程 MCP 服务（WeKnora 作为 MCP 客户端调用，不托管插件代码），并发布 `weknora.plugin/1` 清单地址，声明插件标识、版本、执行端点、工具快照（含读写分类、个人授权需求与 scope）。
2. 管理员在「设置 → 插件」粘贴清单 URL。WeKnora 抓取清单并**实地核验**远端执行端点的真实工具目录（清单声明与远端目录不一致、schema 摘要不符、清单 URL 指向内网/环回/保留地址时直接拒绝），然后展示版本、端点、工具、读写分类与授权要求供审阅。
3. 管理员确认后，空间安装**固定在该版本**：已接受版本的工具列表与 schema 摘要构成运行时核验基准；开发者发布新版不会自动改变本空间能力。

预览与安装是管理员操作（普通成员调用被拒绝）；安装后所有空间成员都能在「集成 → 插件」中发现已安装插件及自己的授权状态。

### 版本升级与漂移

- **升级**：管理员发起升级预览时，WeKnora 重新抓取长期清单来源，对比已接受快照与候选版本在工具新增/移除、schema、scope、端点与读写分类五个维度的差异；管理员**手动接受**后才切换空间版本。清单或候选服务不可达时接受失败，旧版本原样保留。
- **漂移**：运行时以已接受快照为核验基准。远端执行端点悄悄增加工具、移除工具或变更 schema 时，成员调用被阻断，管理员核验后可复审重定基——未接受的能力扩张无法绕过版本决定。一个空间的漂移不影响其他空间。

### 写工具默认关闭与成员审批

安装或升级接受时，快照内**每个**写工具默认关闭（只读工具默认启用）。管理员可逐项启用写工具，并设置调用时是否要求成员审批；审批卡片展示确定的服务、工具与参数原文，成员批准才派发，拒绝或超时不会发生任何外部写入。成员完成个人授权**不等于**启用写工具。

### 成员个人授权

需要个人连接的工具（如 Jira 待办查询）由成员各自授权自己的外部账号；凭据按（空间、成员、插件）隔离，成员之间不能借用连接。撤销或过期只影响本人，重新授权后可继续使用。

### 自托管信任边界（明示）

版本端点稳定是插件开发者的交付约定。WeKnora 能核验**可观察的**工具目录、schema 摘要与权限快照，并在发现能力漂移时阻断未经接受的扩张；但平台**不能保证**开发者未在相同端点改变内部代码——无法验证的行为变化属于自托管插件的信任边界，由空间管理员在选择安装时自行判断。

两条明示口径：

1. **工具 schema 以平台摘要核验，界面不渲染 schema 原文**——预览与差异审阅展示的是「摘要已核验」的事实与读写分类等治理要点，不逐字展示 JSON Schema（展示粒度取舍）。
2. **本版没有插件卸载**——停用即治理终点：停用后成员不可见、调用被拒，成员个人连接与审计记录保留。

### 示例插件服务

仓库提供首个纵向案例 `examples/plugins/jira-todo-mcp`（Go，独立部署）：成员在对话中查询本人本周 Jira 待办并收到带 `/browse/<KEY>` 来源链接的结果。MCP 端点目录公开、执行鉴权，OAuth 授权码 + PKCE + 动态客户端注册，JQL 由服务端固定模板构造（不接受模型传入账号令牌或任意查询）。环境变量、启动命令与生产加固建议（如 `PLUGIN_ALLOWED_REDIRECT_HOSTS` 注册白名单）见该目录 README。
