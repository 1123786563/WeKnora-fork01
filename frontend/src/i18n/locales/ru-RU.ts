export default {
  apps: {
    common: {
      ...
      no: 'Нет',
      stateActive: 'Активно',
      stateRevoked: 'Отключено',
      stateDisabled: 'Неактивно',
      stateOther: 'Состояние: {state}',
    },
    risk: {
      read: 'Чтение',
      write: 'Запись',
    ...
    catalog: {
      ...
      description: 'Проверенные и опубликованные для этого рабочего пространства действия и их области доступа; ниже перечислены установленные версии приложений. Сетевые адресы среды, ссылки на секреты и внутренние псевдонимы никогда не отображаются.',
      refresh: 'Обновить',
      loadFailed: 'Не удалось загрузить каталог приложений',
      empty: 'Нет доступных действий',
      installedTitle: 'Установленные приложения',
      installedEmpty: 'Нет установленных приложений',
      colAction: 'Действие',
      colApp: 'Приложение',
      colVersion: 'Версия',
      colState: 'Состояние',
      ...
      colRisk: 'Риск',
      colPermissions: 'Необходимые права',
      colSchemaDigest: 'Отпечаток схемы',
    ...
    connections: {
      ...
      kindPersonal: 'Личное',
      kindSpace: 'Общее',
      accountSpace: 'Общее для пространства',
      accountUnknown: '—',
      startAuthorization: 'Авторизовать',
      startAuthorizationFailed: 'Не удалось начать авторизацию',
      revoke: 'Отключить',
      revokeConfirmContent: 'После отключения рабочее пространство сразу теряет авторизацию этого подключения; удаление на стороне провайдера может ещё выполняться в фоне. Отключить?',
      revokeSuccess: 'Подключение отключено (локально вступило в силу; очистка на стороне провайдера продолжается в фоне)',
      revokeConflict: 'Версия авторизации подключения изменилась — обновите и повторите',
      revokeFailed: 'Не удалось отключить подключение',
    ...
    authorization: {
      ...
      noUrlGuidance: 'Это развертывание не возвращает ссылку на внешнюю авторизацию: завершите авторизацию через уведомление от контрольной плоскости open-connector; после завершения эта страница обновится автоматически.',
      pollingHint: 'Ожидание завершения внешней авторизации; статус обновится автоматически…',
      completedHint: 'Авторизация завершена; подключение активно.',
      back: 'Вернуться к подключениям',
      status: {
        pending: 'Ожидает',
    ...
    actions: {
      ...
      targetLabel: 'Цель',
      riskLabel: 'Риск',
      riskUnknownHint: 'Текущий API не возвращает это поле; риск зафиксирован на стороне сервера',
      stateLabel: 'Состояние',
      digestLabel: 'Отпечаток содержимого',
      fenceLabel: 'Барьер версии',
      argsLabel: 'Аргументы',
      approve: 'Согласовать',
      execute: 'Выполнить',
      unknown: 'Результат уточняется',
      noResendHint: 'Пока результат уточняется, повторная отправка недоступна; дождитесь результата запроса к провайдеру.',
      memberCannotApprove: 'Ваша роль не позволяет согласовывать или выполнять действия (нужен владелец или администратор рабочего пространства).',
      approveFailed: 'Не удалось согласовать',
      executeFailed: 'Не удалось выполнить',
      state: {
        awaiting_approval: 'Ожидает согласования',
        authorized: 'Согласовано',
        queued: 'В очереди',
  ...
  datasource: {
    ...
    deletePanelPurgeLabel: 'Также удалить эти {count} синхронизированных документов',
    deletePanelPurgeLabelUnknown: 'Также удалить все синхронизированные документы',
    deletePanelPurgeWarning: 'Из базы знаний будут безвозвратно удалены {count} документов вместе с их векторами и индексами.',
    deletePanelPurgeWarningUnknown: 'Из базы знаний будут безвозвратно удалены все синхронизированные документы вместе с их векторами и индексами.',
    deleteAndPurge: 'Удалить источник и документы',
    deleteSuccessPurged: 'Источник данных и его синхронизированные документы удалены',
