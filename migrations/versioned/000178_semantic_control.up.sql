...
CREATE TABLE semantic_outbox (event_id UUID PRIMARY KEY, tenant_id NUMERIC(20,0) NOT NULL CHECK(tenant_id >= 0), kb_id TEXT NOT NULL, document_id TEXT NOT NULL, revision NUMERIC(20,0) NOT NULL CHECK(revision >= 0), content_hash TEXT NOT NULL, config_digest TEXT NOT NULL, deleted BOOLEAN NOT NULL, payload BYTEA NOT NULL, payload_hash TEXT NOT NULL, attempt_count NUMERIC(20,0) NOT NULL DEFAULT 0 CHECK(attempt_count >= 0), lease_token NUMERIC(20,0) NOT NULL DEFAULT 0 CHECK(lease_token >= 0), lease_owner TEXT NOT NULL DEFAULT '', lease_expires_at TIMESTAMPTZ, retry_at TIMESTAMPTZ NOT NULL, error_code TEXT NOT NULL DEFAULT '');
