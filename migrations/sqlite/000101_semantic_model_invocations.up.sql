...
CREATE TABLE semantic_model_invocations (
  ...
  state TEXT NOT NULL CHECK(state IN ('claimed','completed','failed_before_dispatch','unknown')), reserved_input_tokens INTEGER NOT NULL CHECK(reserved_input_tokens >= 0), reserved_output_tokens INTEGER NOT NULL CHECK(reserved_output_tokens >= 0),
