...
def _validate_cleanup_capture(capture: str, created_captures: dict[str, object] | set[str], captures: dict[str, object] | None = None) -> None:
    ...
    if captures is None:
        return
    if capture not in captures or captures[capture] != created_captures[capture]:
        raise ValueError(f"cleanup capture was overwritten: {capture}")
...
def run_case(case: dict, base_url: str, namespace: str, allow_test_writes: bool, schema: Path, artifact_root: Path) -> None:
    ...
    created_captures: dict[str, object] = {}
    records = []
    ...
    try:
      for step in _steps(case):
        ...
        for name, pointer in step.get("captures", {}).items():
            ...
            if method in WRITE_METHODS:
                created_captures[name] = captures[name]
        if "expected" in step:
      ...
      for item in case.get("cleanup", []):
        ...
        _validate_cleanup_capture(item["capture"], created_captures, captures)
