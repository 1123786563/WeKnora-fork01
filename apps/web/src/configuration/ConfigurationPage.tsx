...
export function ConfigurationPage({ client }: { client: WeKnoraClient }) {
  ...
  return <main className="wk-page max-w-[1180px]!">
    ...
    <div className="grid grid-cols-2 gap-4 max-[720px]:grid-cols-1">{configurationSections.map((section) => { const items = records[section.key]; return <Card key={section.key} className="min-w-0">
      <div className="mb-3 flex items-start justify-between gap-4"><div><h2 className="m-0 mb-1">{section.title}</h2><p className="wk-muted m-0">{section.description}</p></div><div className="wk-list-actions"><span className="wk-role-badge rounded-[999px] px-[0.55rem] py-[0.2rem] text-[0.8rem] text-[#6941c6] bg-[#f4f3ff]">{section.writeSupport}</span>{section.writeSupport === 'supported' ? <Button type="button" onClick={() => openEditor(section.key)}>Add</Button> : null}</div></div>
