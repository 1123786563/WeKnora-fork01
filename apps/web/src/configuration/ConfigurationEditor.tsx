...
export function ConfigurationEditor({ client, section, record, onSaved, onCancel }: {
  ...
  return <Card className="wk-configuration-editor"><div className="wk-settings-panel-heading flex items-start justify-between gap-4 border-b border-[#eef1f5] pb-4 mb-4 max-[720px]:flex-col"><div><p className="wk-eyebrow m-0 text-[0.78rem] font-bold uppercase tracking-[0.08em] text-primary">{record ? 'Edit' : 'Create'} {section}</p><h3>{record ? draft.name : `New ${section}`}</h3><p className="wk-muted text-muted m-0">Secrets are write-only and use dedicated credential endpoints.</p></div><Button type="button" theme="default" variant="outline" disabled={busy} onClick={onCancel}>Close</Button></div>{error ? <Status tone="error">{error}</Status> : null}{notice ? <Status tone="success">{notice}</Status> : null}<form className="wk-settings-editor my-4 grid gap-[.8rem] max-w-[620px] [&_label]:grid [&_label]:gap-[.35rem] [&_label]:font-semibold" onSubmit={(event) => void save(event)}><label>Name<Input className={fieldClass} value={draft.name} onChange={(value) => setField('name', String(value))} /></label><label>Description<Textarea className={fieldClass} autosize={{ minRows: 2, maxRows: 2 }} value={draft.description ?? ''} onChange={(value) => setField('description', String(value))} /></label>{section === 'agents' ? <label>Avatar<Input className={fieldClass} value={draft.avatar ?? ''} placeholder="https://…" onChange={(value) => setField('avatar', String(value))} /></label> : null}{section === 'models' ? <><label>Type<Input className={fieldClass} value={draft.type ?? ''} onChange={(value) => setField('type', String(value))} /></label><label>Source<Input className={fieldClass} value={draft.source ?? ''} onChange={(value) => setField('source', String(value))} /></label></> : null}{section === 'mcp' ? <><label>URL<Input className={fieldClass} type="url" value={draft.url ?? ''} onChange={(value) => setField('url', String(value))} /></label><label>Transport<Select className={fieldClass} value={draft.transportType ?? 'sse'} onChange={(value) => setField('transportType', String(value))}><Select.Option value="sse" label="SSE">SSE</Select.Option><Select.Option value="http-streamable" label="HTTP streamable">HTTP streamable</Select.Option><Select.Option value="stdio" label="stdio">stdio</Select.Option></Select></label><Checkbox className="flex! grid-cols-[auto_1fr] items-center gap-2" checked={draft.enabled !== false} onChange={(checked) => setField('enabled', checked)} label="Enabled" /></> : null}{section === 'agents' ? <><label>System prompt<Textarea className={fieldClass} autosize={{ minRows: 6, maxRows: 6 }} value={draft.systemPrompt ?? ''} onChange={(value) => setField('systemPrompt', String(value))} /></label><Checkbox className="flex! grid-cols-[auto_1fr] items-center gap-2" checked={draft.memoryEnabled === true} onChange={(checked) => setField('memoryEnabled', checked)} label="Memory enabled" /></> : null}{section !== 'agents' ? <label>Safe configuration JSON<Textarea className={fieldClass} autosize={{ minRows: 6, maxRows: 6 }} value={draft.details} onChange={(value) => setField('details', String(value))} /></label> : null}{section === 'models' || section === 'mcp' ? <div className="wk-credential-fields"><label>API key<Input className={fieldClass} type="password" autocomplete="new-password" value={draft.apiKey ?? ''} placeholder={configured('api_key') ? 'Configured; leave blank to keep' : 'Enter a new key'} onChange={(value) => setField('apiKey', String(value))} /></label>{section === 'models' ? <label>App secret<Input className={fieldClass} type="password" autocomplete="new-password" value={draft.appSecret ?? ''} placeholder={configured('app_secret') ? 'Configured; leave blank to keep' : 'Enter a new secret'} onChange={(value) => setField('appSecret', String(value))} /></label> : <label>Token<Input className={fieldClass} type="password" autocomplete="new-password" value={draft.token ?? ''} placeholder={configured('token') ? 'Configured; leave blank to keep' : 'Enter a token'} onChange={(value) => setField('token', String(value))} /></label>}<div className="wk-list-actions mb-[0.75rem] flex items-center justify-end gap-[0.5rem]">{draft.id && configured('api_key') ? <Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void clearCredential('api_key')}>Clear API key</Button> : null}{section === 'models' && draft.id && configured('app_secret') ? <Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void clearCredential('app_secret')}>Clear app secret</Button> : null}{section === 'mcp' && draft.id && configured('token') ? <Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void clearCredential('token')}>Clear token</Button> : null}</div></div> : null}<div className="wk-list-actions mb-[0.75rem] flex items-center justify-end gap-[0.5rem]"><Button type="submit" theme="default" variant="outline" loading={busy}>{record ? 'Save changes' : 'Create configuration'}</Button><Button type="button" theme="default" variant="outline" disabled={busy} onClick={onCancel}>Cancel</Button>{section === 'mcp' && draft.id ? <><Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void testMcp()}>Test connection</Button><Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void loadTools()}>Load tools</Button><Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void authorizeMcp()}>Authorize OAuth</Button></> : null}</div></form>{section === 'mcp' && mcpTools.length > 0 ? <div><h4>Server tools</h4><ul className="wk-list m-0 list-none p-0">{mcpTools.map((tool) => <li key={tool.name} className="flex items-baseline justify-between gap-4 border-b border-line-soft py-[0.9rem]"><div className="wk-list-item-copy grid gap-[0.2rem] min-w-0"><strong>{tool.name}</strong><small>{tool.description ?? 'No description returned.'}</small></div></li>)}</ul></div> : null}</Card>;
}
