...
export function ConfigurationEditor({ client, section, record, onSaved, onCancel }: {
  ...
  return <Card className="wk-configuration-editor"><div className="wk-settings-panel-heading"><div><p className="wk-eyebrow">{record ? 'Edit' : 'Create'} {section}</p><h3>{record ? draft.name : `New ${section}`}</h3><p className="wk-muted">Secrets are write-only and use dedicated credential endpoints.</p></div><Button type="button" disabled={busy} onClick={onCancel}>Close</Button></div>{error ? <Status tone="error">{error}</Status> : null}{notice ? <Status tone="success">{notice}</Status> : null}<form className="wk-settings-editor" onSubmit={(event) => void save(event)}><label>Name<input required value={draft.name} onChange={(event) => setField('name', event.target.value)} /></label><label>Description<textarea rows={2} value={draft.description ?? ''} onChange={(event) => setField('description', event.target.value)} /></label>{section === 'agents' ? <label>Avatar<input value={draft.avatar ?? ''} placeholder="https://…" onChange={(event) => setField('avatar', event.target.value)} /></label> : null}{section === 'models' ? <><label>Type<input required value={draft.type ?? ''} onChange={(event) => setField('type', event.target.value)} /></label><label>Source<input required value={draft.source ?? ''} onChange={(event) => setField('source', event.target.value)} /></label></> : null}{section === 'mcp' ? <><label>URL<input type="url" value={draft.url ?? ''} onChange={(event) => setField('url', event.target.value)} /></label><label>Transport<select value={draft.transportType ?? 'sse'} onChange={(event) => setField('transportType', event.target.value)}><option value="sse">SSE</option><option value="http-streamable">HTTP streamable</option><option value="stdio">stdio</option></select></label><label><input type="checkbox" checked={draft.enabled !== false} onChange={(event) => setField('enabled', event.target.checked)} /> Enabled</label></> : null}{section === 'agents' ? <><label>System prompt<textarea rows={6} value={draft.systemPrompt ?? ''} onChange={(event) => setField('systemPrompt', event.target.value)} /></label><label><input type="checkbox" checked={draft.memoryEnabled === true} onChange={(event) => setField('memoryEnabled', event.target.checked)} /> Memory enabled</label></> : null}{section !== 'agents' ? <label>Safe configuration JSON<textarea rows={6} value={draft.details} onChange={(event) => setField('details', event.target.value)} /></label> : null}{section === 'models' || section === 'mcp' ? <div className="wk-credential-fields"><label>API key<input type="password" autoComplete="new-password" value={draft.apiKey ?? ''} placeholder={configured('api_key') ? 'Configured; leave blank to keep' : 'Enter a new key'} onChange={(event) => setField('apiKey', event.target.value)} /></label>{section === 'models' ? <label>App secret<input type="password" autoComplete="new-password" value={draft.appSecret ?? ''} placeholder={configured('app_secret') ? 'Configured; leave blank to keep' : 'Enter a new secret'} onChange={(event) => setField('appSecret', event.target.value)} /></label> : <label>Token<input type="password" autoComplete="new-password" value={draft.token ?? ''} placeholder={configured('token') ? 'Configured; leave blank to keep' : 'Enter a token'} onChange={(event) => setField('token', event.target.value)} /></label>}<div className="wk-list-actions">{draft.id && configured('api_key') ? <Button type="button" disabled={busy} onClick={() => void clearCredential('api_key')}>Clear API key</Button> : null}{section === 'models' && draft.id && configured('app_secret') ? <Button type="button" disabled={busy} onClick={() => void clearCredential('app_secret')}>Clear app secret</Button> : null}{section === 'mcp' && draft.id && configured('token') ? <Button type="button" disabled={busy} onClick={() => void clearCredential('token')}>Clear token</Button> : null}</div></div> : null}<div className="wk-list-actions"><Button type="submit" loading={busy}>{record ? 'Save changes' : 'Create configuration'}</Button><Button type="button" disabled={busy} onClick={onCancel}>Cancel</Button>{section === 'mcp' && draft.id ? <><Button type="button" disabled={busy} onClick={() => void testMcp()}>Test connection</Button><Button type="button" disabled={busy} onClick={() => void loadTools()}>Load tools</Button><Button type="button" disabled={busy} onClick={() => void authorizeMcp()}>Authorize OAuth</Button></> : null}</div></form>{section === 'mcp' && mcpTools.length > 0 ? <div><h4>Server tools</h4><ul className="wk-list">{mcpTools.map((tool) => <li key={tool.name}><div className="wk-list-item-copy"><strong>{tool.name}</strong><small>{tool.description ?? 'No description returned.'}</small></div></li>)}</ul></div> : null}</Card>;
}
