...
export function ModelDebugPanel({ client, models }: { client: WeKnoraClient; models: ModelConfiguration[] }) {
  ...
  return <Card className="wk-configuration-operations"><h2>Model debug</h2><p className="wk-muted wk-cfg-ops-1">The provider call runs on the server; previews and observations are redacted before rendering. VLLM requires an image and ASR requires an audio file. No usage/occupancy endpoint is available in the current backend contract.</p>{error ? <Status tone="error">{error}</Status> : null}<form className="wk-settings-editor wk-cfg-ops-4" onSubmit={(event) => void debug(event)}><label>Model<Select value={modelId} onChange={(value) => { setModelId(String(value)); setFile(null); }}><Select.Option value="" label="No model">No model</Select.Option>{models.map((model) => <Select.Option key={model.id} value={model.id} label={model.name}>{model.name}</Select.Option>)}</Select></label><label>Input<Textarea autosize={{ minRows: 3, maxRows: 3 }} value={input} onChange={(value) => setInput(String(value))} /></label><label>Documents, one per line<Textarea autosize={{ minRows: 3, maxRows: 3 }} value={documents} onChange={(value) => setDocuments(String(value))} /></label>{fileKind ? <label>{fileKind === 'image' ? 'Image file' : 'Audio file'}<input type="file" accept={`${fileKind}/*`} required onChange={(event) => setFile(event.currentTarget.files?.[0] ?? null)} /></label> : null}<Button type="submit" theme="default" variant="outline" loading={busy} disabled={!modelId}>Run debug</Button></form>{result ? <pre className="wk-debug wk-cfg-ops-5">{JSON.stringify(redactDebugValue(result), null, 2)}</pre> : null}</Card>;
}
...
export function SkillOperations({ client, initialCatalog }: { client: WeKnoraClient; initialCatalog?: SkillCatalog[] }) {
  ...
  return <Card className="wk-configuration-operations"><div className="wk-settings-panel-heading wk-cfg-ops-6"><div><h2 className="wk-cfg-ops-7">Skill catalog and files</h2><p className="wk-muted wk-cfg-ops-8">Installation is asynchronous and server-owned; status is polled until ready, failed, or removed. File paths are validated before reads.</p></div><Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void reload()}>Reload</Button></div>{error ? <Status tone="error">{error}</Status> : null}{notice ? <Status tone={notice.tone}>{notice.message}</Status> : null}<form className="wk-settings-editor wk-cfg-ops-4" onSubmit={(event) => void register(event)}><label>Catalog source<Input value={source} onChange={(value) => setSource(String(value))} placeholder="@owner/skill or https://…" /></label><Button type="submit" theme="default" variant="outline" loading={busy}>Register skill source</Button></form><label>Sandbox configuration IDs<Input value={sandboxIds} onChange={(value) => setSandboxIds(String(value))} placeholder="cfg-a, cfg-b" /></label><p className="wk-debug wk-cfg-ops-5">installations: {summary.complete} complete · {summary.active} active · {summary.failed} failed</p>{catalog.length === 0 ? <Status>No catalog entries returned.</Status> : <ul className="wk-list wk-cfg-ops-9">{catalog.map((item) => <li key={item.id} className="wk-cfg-ops-10"><div className="wk-list-item-copy wk-cfg-ops-11"><strong>{item.name}</strong><span className="wk-cfg-ops-12">{item.version ?? 'unversioned'} · {item.installations?.length ?? 0} installation(s)</span><small>{item.description ?? 'No description returned.'}</small>{item.installations?.map((installation) => <small key={installation.sandboxConfigId}>{installation.sandboxConfigName ?? installation.sandboxConfigId}: {installation.status}{installation.error ? ` · ${installation.error}` : ''}{canStopSkillInstallation(installation.status) ? <Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void stop(installation.sandboxConfigId, installation.skillId)}>Stop</Button> : null}</small>)}</div><div className="wk-list-actions wk-cfg-ops-3"><Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void install(item.id)}>Install</Button><Button type="button" theme="default" variant="outline" disabled={busy} onClick={() => void openFiles(item.id)}>Files</Button></div></li>)}</ul>}{fileRows.length > 0 ? <div><h3>Files</h3><ul className="wk-list wk-cfg-ops-9">{fileRows.map((file) => <li key={file.path} className="wk-cfg-ops-10"><button type="button" disabled={file.isDir} onClick={() => void openFile(file.path)} style={{ paddingLeft: `${file.depth * 16 + 8}px` }}>{file.isDir ? '▸ ' : '· '}{file.name}{file.isDir ? '' : ` · ${file.size ?? 0} bytes`}</button></li>)}</ul></div> : null}{fileContent !== null ? <div><h3>{selectedFile?.path ?? 'File preview'}</h3>{selectedFile?.truncated ? <Status tone="warning">File preview was truncated by the server.</Status> : null}<pre className="wk-debug wk-cfg-ops-5">{fileContent}</pre></div> : null}</Card>;
}
