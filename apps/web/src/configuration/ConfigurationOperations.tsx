...
export function AgentOperations({ client, agents, disabledIds }: { client: WeKnoraClient; agents: AgentConfiguration[]; disabledIds: string[] }) {
  ...
  return <Card className="wk-configuration-operations"><h2>Agent sharing and visibility</h2><p className="wk-muted">Select an agent here for sharing or hide/show operations. To choose an agent for a conversation, use the Agent selector in the chat entry. Shared agents are read-only in the receiving workspace, so every share uses Viewer permission. Sharing and hide/show preferences remain server-authorized.</p>{error ? <Status tone="error">{error}</Status> : null}{notice ? <Status tone="success">{notice}</Status> : null}<div className="wk-form-grid wk-form-grid--two"><label>Selected agent<select value={selected} onChange={(event) => setSelected(event.target.value)}><option value="">No agent</option>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}{disabledIds.includes(agent.id) ? ' · disabled' : ''}</option>)}</select></label><label>Organization ID<input value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} placeholder="org_…" /></label><label>Share permission<input value="Viewer (read-only)" readOnly /></label></div><div className="wk-list-actions"><Button type="button" disabled={busy || !selected} onClick={() => void setDisabled(!selectedDisabled)}>{selectedDisabled ? 'Show selected agent' : 'Hide selected agent'}</Button><Button type="button" loading={busy} disabled={!selected || !organizationId.trim()} onClick={() => void share()}>Share agent</Button></div></Card>;
}
