...
// --- R484 B1-5: empty-graph toolbar visibility + parser-engine banner ---
//
// Vue WikiBrowser.vue gates every graph overlay on graphReady: the flag flips
// true only after renderGraph() draws actual nodes (renderGraph returns early
// for !data.nodes?.length, L3759-3764), so a 0-node graph keeps the search
// shell, the ?-help trigger, the legend chips (摘要/实体/概念/综合/对比), the
// 适应屏幕/隐藏箭头 actions and the node-count status card ALL hidden, with
// only the 暂无图谱数据 empty state visible (v-if="!graphReady", L120).
...
test('graph empty state hides the whole toolbar until nodes render (Vue graphReady)', async () => {
  ...
  // The Vue empty state stays: 暂无图谱数据 with the bubble icon.
  assert.match(container.querySelector('.wiki-graph-empty')?.textContent ?? '', /暂无图谱数据/);
  // Search overlay (search + ? help) — hidden like v-if="graphReady".
  ...
  // Legend overlay (type chips / 适应屏幕 / 隐藏箭头 / status card) — hidden.
  ...
  // No type filter chips leak either.
...
test('graph toolbar reappears once nodes exist (graphReady flips with data)', async () => {
  const container = await mount(); // graphClient fixture carries 3 nodes
...
// included) renders the .parser-hint warning line under the subtitle with a
// 前往配置 → link into the KB settings parser section.
...
test('graph header renders the parser-engine warning banner with 前往配置', async () => {
  ...
  // The subtitle stays above the banner (Vue renders document-subtitle first).
...
test('graph parser banner 前往配置 opens KB settings on the parser section (Vue openKBSettings parser)', async () => {
  const container = await mountWith(partialEngineClient());
  const hint = container.querySelector<HTMLParagraphElement>('.parser-hint');
  assert.ok(hint);
  // Vue uiStore.openKBSettings(kbId, 'parser'): the React graph page hosts
  // the settings Dialog in place and must seed it at the parser section. The
  // wiring is asserted on the component source + a click-through that stops
  // at the Dialog shell (the KnowledgeSettingsPage internals belong to the
  // settings-surface parity work, not the graph surface).
  ...
  // esbuild lowers `initialSection={settingsSection}` to a plain object prop.
  ...
  // The banner element is wired to that handler (the click-through into the
  // hosted KnowledgeSettingsPage is covered by the settings-surface suite;
  // mounting it here would couple this graph test to that surface's state).
...
test('graph parser banner stays hidden when every advertised type has an engine', async () => {
  const container = await mount(); // graphClient engines: builtin pdf/md all available
