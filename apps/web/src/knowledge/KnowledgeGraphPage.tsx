...
export function KnowledgeGraphPage({ client, knowledgeBaseId, slug }: { client: WeKnoraClient; knowledgeBaseId: string; slug?: string }) {
  ...
  return (
    <main className="wk-page mx-auto box-border max-w-[960px] px-[1.25rem] py-12">
      ...
      <Card>
        <div className="wk-toolbar mb-4 flex flex-wrap items-end gap-3" role="search">
      ...
      {drawerNode ? <aside className="mt-4 rounded-[8px] border border-[#d8e0eb] bg-white p-4 shadow-[0_12px_32px_rgba(31,52,84,.14)]" aria-label={drawerNode.title} role="dialog" aria-modal="true"><div className="wk-header mb-[.75rem]! flex items-start justify-between gap-4"><div><h2>{drawerNode.title}</h2><p className="wk-muted text-muted">{drawerNode.page_type} · {drawerNode.link_count} {t('knowledgeBase.graph.links')}</p></div><Button type="button" onClick={() => { setDrawerNode(null); setDrawerPage(null); }}>{t('common.close')}</Button></div>{drawerStatus === 'loading' ? <Status>{t('wikiBrowser.loading')}</Status> : null}{drawerStatus === 'error' ? <Status tone="error">{t('wikiBrowser.revisionLoadFailed')}</Status> : null}{drawerPage ? <><p className="wk-muted text-muted">{drawerPage.summary || '—'} · {t('wikiBrowser.version', { ver: drawerPage.version })}</p><pre className="box-border rounded-[6px] border border-[#d8e0eb] bg-[#f8fafc] [font:inherit] leading-[1.6] max-h-[24rem] overflow-auto p-[.85rem] whitespace-pre-wrap">{drawerPage.content}</pre><div className="wk-list-actions mb-[0.75rem] flex items-center justify-end gap-[0.5rem]"><Button type="button" onClick={() => void bloomNeighbors(drawerNode.slug)}>{t('wikiBrowser.bloomNeighbors')}</Button><Button type="button" onClick={() => void load('ego', drawerNode.slug)}>{t('wikiBrowser.expandNeighbors')}</Button></div></> : null}</aside> : null}
    </main>
  );
}
