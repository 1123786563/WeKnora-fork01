...
test('KB mention picker supports search, Enter selection, Escape, and chip removal', async () => {
  ...
  await act(async () => root?.render(<ChatComposer copy={resolveChatCopy('zh-CN')} draft="" onDraftChange={() => undefined} onSubmit={() => undefined} mentionOptions={[{ id: 'kb-1', name: '产品文档', type: 'kb' }, { id: 'kb-2', name: 'FAQ 库', type: 'kb' }]} onMentionSelect={(item) => selected.push(item.id)} mentionedItems={[{ id: 'kb-1', name: '产品文档', type: 'kb' }]} onMentionRemove={(id) => removed.push(id)} />));
  ...
  await act(async () => trigger?.click());
  await act(async () => container.querySelector('[role="listbox"] input')?.dispatchEvent(new dom.window.KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true })));
  assert.equal(container.querySelector('[role="listbox"]'), null);
