...
test('builds an explicit connector payload without serializing a generic JSON editor', () => {
  assert.deepEqual(buildDataSourceInput({ name: 'Docs', type: 'notion', schedule: '0 0 */6 * * *', mode: 'incremental', conflict: 'overwrite', deletions: true, credentialsText: 'token = abc', settingsText: 'workspace_id = ws-1' }), {
    name: 'Docs', type: 'notion', sync_schedule: '0 0 */6 * * *', sync_mode: 'incremental', conflict_strategy: 'overwrite', sync_deletions: true,
    config: { credentials: { token: 'abc' }, settings: { workspace_id: 'ws-1' } },
  });
});
