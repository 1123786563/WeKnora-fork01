...
function statusToneClass(status: unknown): string {
  return status === 'active' ? 'wk-ds-tone-success' : status === 'paused' ? 'wk-ds-tone-warning' : status === 'error' ? 'wk-ds-tone-danger' : '';
}
...
function syncResultToneClass(status: string): string {
  return status === 'success' ? 'wk-ds-tone-success' : status === 'failed' ? 'wk-ds-tone-danger' : status === 'running' ? 'wk-ds-tone-primary' : status === 'partial' ? 'wk-ds-tone-warning' : '';
}
...
export function DataSourcesPage({ client, knowledgeBaseId, canManage = false, embedded = false }: { client: WeKnoraClient; knowledgeBaseId: string; canManage?: boolean; embedded?: boolean }) {
  ...
  const editorSurface = editing === undefined ? null : <Drawer visible header={editorTitle} onClose={() => setEditing(undefined)} size="640px" placement="right" className="wk-data-source-drawer">
    {editing === null && createStep === 'type' ? <div className="wk-ds-1">
      {createTypes.map((type) => <button key={type.type} type="button" className="focus-visible:outline-primary focus-visible:outline-offset-2 wk-ds-2" onClick={() => chooseCreateType(type.type)}><strong>{t(`dataSource.connector.${type.type}`)}</strong><span className="wk-ds-3">{t(`dataSource.connectorDesc.${type.type}`)}</span></button>)}
    </div> : <form className="wk-wiki-editor wk-ds-4" onSubmit={(event) => void save(event)}>
      {guide && guide.requiredPermissions.length > 0 ? <div className="wk-data-source-prereq wk-ds-5" data-kind="setup-guide">
        <button type="button" aria-expanded={prereqExpanded} className="wk-ds-6" onClick={() => setPrereqExpanded((current) => !current)}><span aria-hidden="true">ⓘ</span><span className="wk-ds-7">{prereqCopy(t, `dataSource.prereqBarText_${form.type}`, 'dataSource.prereqBarText')}</span><span aria-hidden="true">{prereqExpanded ? '▴' : '▾'}</span></button>
        {prereqExpanded ? <div className="wk-ds-8" data-kind="setup-guide-body">
          <ol className="wk-ds-9">
            <li className="wk-ds-10" data-step="1"><span className="wk-ds-11">{prereqCopy(t, `dataSource.prereqStep1Brief_${form.type}`, 'dataSource.prereqBotBrief')}</span><span className="wk-ds-12">{prereqCopy(t, `dataSource.prereqStep1Desc_${form.type}`, 'dataSource.prereqBotDesc')}</span></li>
            <li className="wk-ds-10" data-step="2"><span className="wk-ds-11">{prereqCopy(t, `dataSource.prereqStep2Brief_${form.type}`, 'dataSource.prereqPermBrief')}</span>{hasKey(t, `dataSource.prereqStep2Desc_${form.type}`) ? <span className="wk-ds-12">{t(`dataSource.prereqStep2Desc_${form.type}`)}</span> : <span className="wk-ds-13">{guide.requiredPermissions.map((perm) => <code key={perm} className="wk-ds-14">{perm}</code>)}</span>}</li>
            <li className="wk-ds-10" data-step="3"><span className="wk-ds-11">{prereqCopy(t, `dataSource.prereqStep3Brief_${form.type}`, 'dataSource.prereqMemberBrief')}</span><span className="wk-ds-12">{prereqCopy(t, `dataSource.prereqStep3Desc_${form.type}`, 'dataSource.prereqMemberDesc')}</span></li>
          </ol>
      ...
      {guide?.docUrl ? <div className="wk-data-source-doc-hint wk-ds-16" data-kind="doc-hint"><span>{t('dataSource.docHint')}</span><a className="wk-ds-17" href={guide.docUrl} target="_blank" rel="noopener">{t('dataSource.openDoc')}</a></div> : null}
      <p className="wk-muted wk-ds-18">{t('dataSource.credentialsLabel')}</p>
      ...
      {kind === 'configured' ? (credentialStep.pendingRemove ? <div className="wk-ds-19" data-kind="confirm-remove"><span className="wk-ds-20">{t('dataSource.credential.confirmRemovePrompt')}</span><div className="wk-ds-21"><Button type="button" theme="default" variant="text" size="small" onClick={() => setCredentialStep((current) => credentialStepReducer(current, 'cancel-remove'))}>{t('common.cancel')}</Button><Button type="button" theme="danger" variant="text" size="small" loading={action === 'remove-credentials'} onClick={() => void confirmRemoveCredentials()}>{t('dataSource.credential.confirmRemove')}</Button></div></div> : <div className="wk-ds-22" data-kind="configured"><span className="wk-ds-23" aria-hidden="true">✓</span><span>{t('dataSource.credential.configured')}</span><div className="wk-ds-24"><Button type="button" theme="default" variant="text" size="small" onClick={() => setCredentialStep((current) => credentialStepReducer(current, 'enter-replace'))}>{t('dataSource.credential.update')}</Button><Button type="button" theme="danger" variant="text" size="small" onClick={() => setCredentialStep((current) => credentialStepReducer(current, 'request-remove'))}>{t('dataSource.credential.remove')}</Button></div></div>) : null}
      {kind === 'unconfigured' ? <div className="wk-ds-25" data-kind="unconfigured"><span>{t('dataSource.credential.unconfigured')}</span><Button type="button" theme="default" variant="text" size="small" className="wk-ds-26" onClick={() => setCredentialStep((current) => credentialStepReducer(current, 'enter-replace'))}>{t('dataSource.credential.configure')}</Button></div> : null}
      {kind === 'inputs' ? <fieldset className="wk-ds-27"><legend className="wk-ds-28">{t('dataSource.credentialsLabel')}</legend>{VUE_CREDENTIAL_FIELDS[form.type] ? VUE_CREDENTIAL_FIELDS[form.type].map((field) => <label key={field.key} className="wk-ds-29"><span>{t(field.label)}</span><Input type={field.secret ? 'password' : 'text'} placeholder={field.placeholder || t('dataSource.credential.inputPlaceholder')} value={credentialValue(form.credentialsText, field.key)} onChange={(value) => updateForm('credentialsText', setCredentialValue(form.credentialsText, field.key, String(value)))} />{field.hint ? <small className="wk-ds-12">{t(field.hint)}</small> : null}</label>) : form.type === 'rss' ? <div className="wk-ds-8" data-kind="rss-auth-headers"><div className="wk-ds-30"><span className="wk-ds-31">{t('dataSource.field.authHeaders')}</span><Button type="button" theme="default" variant="text" size="small" onClick={() => updateForm('authHeaders', [...(form.authHeaders ?? []), { key: '', value: '' }])}>{t('dataSource.credential.headerAdd')}</Button></div><small className="wk-ds-12">{t('dataSource.field.authHeadersHint')}</small>{(form.authHeaders ?? []).map((row, index) => <div key={index} className="wk-ds-21"><Input aria-label={t('dataSource.credential.headerKeyPlaceholder')} autocomplete="off" placeholder={t('dataSource.credential.headerKeyPlaceholder')} value={row.key} onChange={(value) => updateForm('authHeaders', (form.authHeaders ?? []).map((item, at) => at === index ? { ...item, key: String(value) } : item))} /><Input aria-label={t('dataSource.credential.headerValuePlaceholder')} autocomplete="off" placeholder={t('dataSource.credential.headerValuePlaceholder')} value={row.value} onChange={(value) => updateForm('authHeaders', (form.authHeaders ?? []).map((item, at) => at === index ? { ...item, value: String(value) } : item))} /><Button type="button" theme="default" variant="text" size="small" aria-label={t('common.delete')} onClick={() => updateForm('authHeaders', (form.authHeaders ?? []).filter((_, at) => at !== index))}>×</Button></div>)}</div> : <Textarea autosize={{ minRows: 4, maxRows: 4 }} value={form.credentialsText} onChange={(value) => updateForm('credentialsText', String(value))} placeholder={t('dataSource.credentialsPlaceholder')} />}</fieldset> : null}
  ...
  >
    <div className="wk-ds-35" data-kind="delete-panel">
      <p className="wk-ds-36" data-kind={deletePurge ? 'delete-purge-warning' : 'delete-keep'}>{deletePurge ? <span className="wk-ds-37">{purgeWarningText}</span> : t('dataSource.deletePanelKeep')}</p>
      {deleteCountLoading ? <p className="wk-ds-18" data-kind="delete-count-loading">{t('common.loading')}</p> : deleteCount !== null ? <p className="wk-ds-18" data-kind="delete-count">{t('dataSource.deletePanelCount', { count: deleteCount })}</p> : null}
  ...
  // classes drop away.
  return <main data-can-manage={canManage ? 'true' : 'false'} data-embedded={embedded ? 'true' : undefined} className={embedded ? 'wk-data-source-root' : 'wk-data-source-root wk-page wk-ds-83'}>{embedded ? null : <header className="wk-header wk-ds-38"><div><p className="wk-eyebrow wk-ds-39">{t('common.knowledgeBases')} · {knowledgeBaseId}</p><h1 className="wk-ds-40">{t('dataSource.title')}</h1><p className="wk-muted wk-ds-12">{t('dataSource.description')}</p></div></header>}
    <Card>{message ? <Status tone={message.tone}>{message.text}</Status> : null}{loading ? <Status>{t('common.loading')}</Status> : accessDenied ? <Status tone="error">{t('dataSource.permissionRequired')}</Status> : sources.length === 0 && !canManage ? <Status>{t('dataSource.empty')}</Status> : sources.length === 0 ? <div className="wk-ds-41"><button type="button" className="wk-data-source-create wk-ds-42 wk-ds-create-focus" onClick={openCreate}><span className="wk-ds-43" aria-hidden="true"><KbIcon name="add" size={16} /></span><span>{t('dataSource.add')}</span></button></div> : <div className="wk-data-source-grid wk-ds-44">{sources.map((source) => { const log = source.latest_sync_log; const fullTime = source.last_sync_at ? new Date(source.last_sync_at).toLocaleString() : ''; return <div key={source.id} className={`wk-data-source-card wk-ds-77 ${canManage ? 'wk-ds-78 wk-ds-create-focus' : ''}`} onClick={canManage ? () => openEdit(source) : undefined} role={canManage ? 'button' : undefined} tabIndex={canManage ? 0 : undefined}><span aria-hidden="true" className="wk-data-source-badge wk-ds-45">{(source.type.slice(0, 1) || '?').toUpperCase()}</span><div className="wk-ds-46"><div className="wk-ds-47"><strong title={source.name} className="wk-ds-48">{source.name}</strong><div className="wk-data-source-actions wk-ds-49" onClick={(event) => event.stopPropagation()}>{canManage ? <><Button type="button" theme="default" variant="outline" onClick={() => openEdit(source)}>{t('dataSource.edit')}</Button><Button type="button" theme="default" variant="outline" disabled={action !== null} onClick={() => void run(source, 'validate')}>{t('dataSource.testConnection')}</Button><Button type="button" theme="default" variant="outline" disabled={action !== null || isSyncRunning(source)} onClick={() => void run(source, 'sync')}>{isSyncRunning(source) ? t('dataSource.status.running') : t('dataSource.syncNow')}</Button>{isSyncRunning(source) ? <Button type="button" theme="default" variant="outline" disabled={action !== null} onClick={() => void cancelSync(source)}>{t('dataSource.cancelSync')}</Button> : null}{source.status === 'paused' ? <Button type="button" theme="default" variant="outline" disabled={action !== null} onClick={() => void run(source, 'resume')}>{t('dataSource.resume')}</Button> : source.status === 'active' ? <Button type="button" theme="default" variant="outline" disabled={action !== null} onClick={() => void run(source, 'pause')}>{t('dataSource.pause')}</Button> : null}<Button type="button" theme="default" variant="outline" disabled={action !== null} onClick={() => void remove(source)}>{t('dataSource.delete')}</Button></> : null}<Button type="button" theme="default" variant="outline" onClick={() => void showLogs(source)}>{t('dataSource.logs')}</Button></div></div><p className="wk-data-source-card-subtitle wk-ds-50"><span>{t(`dataSource.connector.${source.type}`)} · {source.sync_mode ? t(`dataSource.syncMode.${source.sync_mode}`) : t('dataSource.syncMode.incremental')}</span><span className={`wk-ds-79 ${statusToneClass(source.status)}`}><span aria-hidden="true" className="wk-ds-51" />{localizedSyncStatus(t, source.status)}</span></p><p className="wk-data-source-card-detail wk-ds-52"><span>{humanizeCron(source.sync_schedule, t)}</span><span aria-hidden="true">·</span><span title={fullTime || undefined}>{relativeTime(source.last_sync_at, t)}</span>{log ? <><span aria-hidden="true">·</span><span className={`wk-ds-11 ${syncResultToneClass(log.status)}`}>{localizedSyncStatus(t, log.status)}</span>{syncResultPills(source, t).map((pill) => <span key={pill.kind} className="wk-data-source-metric tabular-nums wk-ds-53">{pill.text}</span>)}</> : null}</p>{source.error_message ? <div className="wk-ds-54">{source.error_message}</div> : null}</div></div>; })}{canManage ? <button type="button" className="wk-data-source-create wk-ds-42 wk-ds-create-focus" onClick={openCreate}><span className="wk-ds-43" aria-hidden="true"><KbIcon name="add" size={16} /></span><span>{t('dataSource.add')}</span></button> : null}</div>}</Card>
    {logsSource ? <Card className="wk-ds-55"><div className="wk-header wk-ds-56"><div><h2 className="wk-ds-36">{t('dataSource.syncHistory')} · {logsSource.name}</h2><p className="wk-muted wk-ds-12">{t('dataSource.updateSuccessSyncHint')}</p></div><div className="wk-ds-21">{canManage && retryableFailedCount > 0 ? <Button type="button" theme="default" variant="outline" disabled={retrySelection.size === 0} loading={action === `reindex:${logsSource.id}`} onClick={() => void retrySelected()}>{t('dataSource.retrySelected')}</Button> : null}<Button type="button" theme="default" variant="outline" onClick={() => setLogsSource(null)}>{t('dataSource.close')}</Button></div></div>{logsLoading ? <Status>{t('common.loading')}</Status> : logsError ? <><Status tone="error">{logsError}</Status><Button type="button" theme="default" variant="outline" onClick={() => void loadLogs(logsSource, logsOffset)}>{t('common.retry')}</Button></> : logs.length === 0 ? <Status>{t('dataSource.noLogs')}</Status> : <ul className="wk-list wk-ds-57">{logs.map((log) => { const samples = failedItemSamples(log); const failedCount = typeof log.items_failed === 'number' ? log.items_failed : samples.length; const overflow = Math.max(0, failedCount - samples.length); return <li key={log.id} className="wk-ds-58" data-kind="sync-log"><div className="wk-ds-59"><span className="wk-ds-60">{localizedSyncStatus(t, log.status)}</span><small>{typeof log.started_at === 'string' ? log.started_at : ''}{typeof log.finished_at === 'string' ? ` → ${log.finished_at}` : ''}{typeof log.error_message === 'string' ? ` · ${log.error_message}` : ''}</small><span className="wk-ds-61">{t('dataSource.logMetric.total')} {log.items_total ?? 0} · +{log.items_created ?? 0} · ↻{log.items_updated ?? 0} · −{log.items_deleted ?? 0} · {t('dataSource.logMetric.skipped')} {log.items_skipped ?? 0} · {t('dataSource.logMetric.failed')} {log.items_failed ?? 0}</span></div>{samples.length > 0 ? <div className="wk-ds-62" data-kind="failed-items"><div className="wk-ds-63">{t('dataSource.logDetail.failedItems')} ({failedCount})</div>{samples.map((sample, index) => { const id = typeof sample.external_id === 'string' ? sample.external_id : ''; const text = formatSyncError(t, sample); return <div key={index} className="wk-ds-64" data-kind="failed-item">{canManage && id !== '' ? <Checkbox checked={retrySelection.has(id)} onChange={() => toggleRetrySelection(id)} /> : null}<span title={text} className="wk-ds-65">{text}</span></div>; })}{overflow > 0 ? <div className="wk-ds-66" data-kind="failed-items-more">{t('dataSource.logDetail.failedItemsMore', { n: overflow })}</div> : null}</div> : null}</li>; })}</ul>}{logs.length > 0 ? <nav className="wk-pagination" aria-label={t('dataSource.syncHistory')}><Button type="button" theme="default" variant="outline" disabled={logsOffset === 0 || logsLoadingMore} onClick={() => void loadLogs(logsSource!, Math.max(0, logsOffset - LOG_PAGE_SIZE))}>{t('dataSource.back')}</Button><span>{Math.floor(logsOffset / LOG_PAGE_SIZE) + 1}</span><Button type="button" theme="default" variant="outline" disabled={!logsHasNext || logsLoadingMore} loading={logsLoadingMore} onClick={() => logsSource ? void loadLogs(logsSource, logsOffset + LOG_PAGE_SIZE) : undefined}>{t('dataSource.loadMore')}</Button></nav> : null}</Card> : null}
    {resourceSource ? <Card className="wk-ds-55"><div className="wk-header wk-ds-56"><div><h2>{t('dataSource.resourceHint')} · {resourceSource.name}</h2><p className="wk-muted wk-ds-12">{resourceTrail.length > 0 ? resourceTrail.map((item) => item.name).join(" / ") : t("dataSource.resourceHint")}</p></div><Button type="button" theme="default" variant="outline" onClick={() => setResourceSource(null)}>{t('dataSource.close')}</Button></div>{resourceSource && isDriveConnector(resourceSource.type) ? <div className="wk-ds-67" data-kind="drive-folder-input"><label className="wk-ds-31">{t('dataSource.drive.folderTokenLabel')}</label><div className="wk-ds-21"><Input aria-label={t('dataSource.drive.folderTokenLabel')} value={driveToken} placeholder={t('dataSource.drive.folderTokenPlaceholder')} onChange={(value) => { setDriveToken(String(value)); setDriveTokenError(false); }} onEnter={() => void loadDriveRoot(resourceSource)} /><Button type="button" theme="default" variant="outline" loading={resourceLoading} onClick={() => void loadDriveRoot(resourceSource)}>{t('dataSource.drive.load')}</Button></div>{driveTokenError ? <small className="wk-ds-20">{t('dataSource.drive.folderTokenRequired')}</small> : <small className="wk-ds-12">{t('dataSource.drive.shareHint')}</small>}</div> : null}{resourceTrail.length > 0 ? <Button type="button" theme="default" variant="outline" onClick={() => { const previous = resourceTrail.slice(0, -1); void loadResources(resourceSource, previous.length > 0 ? previous[previous.length - 1]!.id : undefined, previous); }}>{t('dataSource.back')}</Button> : null}<Button type="button" theme="default" variant="outline" onClick={() => void expandAllResources()} disabled={resourceLoading || !resources.some((resource) => resource.has_children)}>{t('dataSource.loadMore')}</Button>{resourceLoading ? <Status>{t('common.loading')}</Status> : null}{resourceError ? <Status tone="error">{resourceError}</Status> : null}{!resourceLoading && !resourceError && visibleResources.length === 0 ? (resourceSource && isDriveConnector(resourceSource.type) && !driveRootLoaded ? <div className="wk-ds-68" data-kind="drive-placeholder"><p className="wk-ds-69">{t('dataSource.drive.placeholderTitle')}</p><p className="wk-ds-70">{t('dataSource.drive.placeholderDesc')}</p></div> : <Status>{t('dataSource.noResources')}</Status>) : null}{!resourceLoading && !resourceError ? <ul className="wk-list wk-ds-57" role="tree">{visibleResources.map((resource) => <li key={resource.external_id} className="wk-ds-71" role="treeitem"><div className="wk-ds-72"><label className="wk-ds-73"><input type="checkbox" className="peer wk-ds-74" checked={resourceStates.get(resource.external_id) === "checked"} aria-checked={resourceStates.get(resource.external_id) === "indeterminate" ? "mixed" : resourceStates.get(resource.external_id) === "checked"} data-state={resourceStates.get(resource.external_id)} onChange={() => setForm((current) => ({ ...current, resourceIds: toggleResourceSelection(resources, current.resourceIds, resource.external_id) }))} /><span className={`wk-ds-80 ${(resourceStates.get(resource.external_id) ?? "unchecked") === "unchecked" ? "wk-ds-81" : "wk-ds-82"} peer-focus-visible:outline-2 peer-focus-visible:outline-[#2e6de6] peer-focus-visible:outline-offset-2`} aria-hidden="true">{resourceStates.get(resource.external_id) === "checked" ? "✓" : resourceStates.get(resource.external_id) === "indeterminate" ? "−" : ""}</span> <strong title={resource.name} className="wk-ds-75">{resource.name}</strong></label><small className="wk-ds-76">{resource.type}</small></div>{resource.has_children ? <Button type="button" theme="default" variant="outline" onClick={() => void loadResources(resourceSource, resource.external_id, [...resourceTrail, { id: resource.external_id, name: resource.name }])}>{t('dataSource.next')}</Button> : null}</li>)}</ul> : null}</Card> : null}
