...
export function DataSourcesPage({ client, knowledgeBaseId }: { client: WeKnoraClient; knowledgeBaseId: string }) {
  ...
  return <main className="wk-page max-w-[1180px]!"><header className="wk-header"><div><p className="wk-eyebrow">Knowledge base · {knowledgeBaseId}</p><h1>{t('dataSource.title')}</h1><p className="wk-muted">{t('dataSource.description')}</p></div><Button type="button" onClick={openCreate}>{t('dataSource.add')}</Button></header>
    <Card>{message ? <Status tone={message.tone}>{message.text}</Status> : null}{loading ? <Status>{t('common.loading')}</Status> : sources.length === 0 ? <Status>{t('dataSource.empty')}</Status> : <ul className="wk-list">{sources.map((source) => <li key={source.id} className="items-start!"><div className="wk-list-item-copy"><strong>{source.name}</strong><span>{source.type} · {source.sync_mode ? t(`dataSource.syncMode.${source.sync_mode}`) : t('dataSource.syncMode.incremental')} · {source.status ?? 'unknown'}</span><small className="text-muted">{source.last_sync_at ? new Date(source.last_sync_at).toLocaleString() : t('dataSource.neverSynced')}{source.error_message ? ` · ${source.error_message}` : ''}</small></div><div className="wk-list-actions flex-wrap"><Button type="button" onClick={() => openEdit(source)}>{t('dataSource.edit')}</Button><Button type="button" disabled={action !== null} onClick={() => void run(source, 'validate')}>{t('dataSource.testConnection')}</Button><Button type="button" disabled={action !== null} onClick={() => void run(source, 'sync')}>{t('dataSource.syncNow')}</Button>{source.status === 'paused' ? <Button type="button" disabled={action !== null} onClick={() => void run(source, 'resume')}>{t('dataSource.resume')}</Button> : <Button type="button" disabled={action !== null} onClick={() => void run(source, 'pause')}>{t('dataSource.pause')}</Button>}<Button type="button" onClick={() => void showLogs(source)}>{t('dataSource.logs')}</Button><Button type="button" onClick={() => showResources(source)}>{t('dataSource.resourceHint')}</Button><Button type="button" disabled={action !== null} onClick={() => void remove(source)}>{t('dataSource.delete')}</Button></div></li>)}</ul>}</Card>
    {logsSource ? <Card className="mt-4"><div className="wk-header"><div><h2 className="m-0">{t('dataSource.syncHistory')} · {logsSource.name}</h2><p className="wk-muted">{t('dataSource.updateSuccessSyncHint')}</p></div><Button type="button" onClick={() => setLogsSource(null)}>{t('dataSource.close')}</Button></div>{logs.length === 0 ? <Status>{t('dataSource.noLogs')}</Status> : <ul className="wk-list">{logs.map((log) => <li key={log.id}><span>{log.status}</span><small>{typeof log.started_at === 'string' ? log.started_at : ''}{typeof log.finished_at === 'string' ? ` → ${log.finished_at}` : ''}{typeof log.error_message === 'string' ? ` · ${log.error_message}` : ''}</small><span className="text-[#718096]! text-[0.75rem]!">Total {log.items_total ?? 0} · +{log.items_created ?? 0} · ↻{log.items_updated ?? 0} · −{log.items_deleted ?? 0} · skipped {log.items_skipped ?? 0} · failed {log.items_failed ?? 0}</span></li>)}</ul>}{logs.length > 0 ? <nav className="wk-pagination" aria-label={t('dataSource.syncHistory')}><Button type="button" disabled={logsOffset === 0} onClick={() => void loadLogs(logsSource!, Math.max(0, logsOffset - LOG_PAGE_SIZE))}>{t('dataSource.back')}</Button><span>{Math.floor(logsOffset / LOG_PAGE_SIZE) + 1}</span><Button type="button" disabled={!logsHasNext} onClick={() => logsSource ? void loadLogs(logsSource, logsOffset + LOG_PAGE_SIZE) : undefined}>{t('dataSource.loadMore')}</Button></nav> : null}</Card> : null}
    {resourceSource ? <Card className="mt-4"><div className="wk-header"><div><h2>{t('dataSource.resourceHint')} · {resourceSource.name}</h2><p className="wk-muted">{resourceTrail.length > 0 ? resourceTrail.map((item) => item.name).join(" / ") : t("dataSource.resourceHint")}</p></div><Button type="button" onClick={() => setResourceSource(null)}>{t('dataSource.close')}</Button></div>{resourceTrail.length > 0 ? <Button type="button" onClick={() => { const previous = resourceTrail.slice(0, -1); void loadResources(resourceSource, previous.length > 0 ? previous[previous.length - 1]!.id : undefined, previous); }}>{t('dataSource.back')}</Button> : null}<Button type="button" onClick={() => void expandAllResources()} disabled={resourceLoading || !resources.some((resource) => resource.has_children)}>{t('dataSource.loadMore')}</Button>{resourceLoading ? <Status>{t('common.loading')}</Status> : null}{resourceError ? <Status tone="error">{resourceError}</Status> : null}{!resourceLoading && !resourceError && visibleResources.length === 0 ? <Status>{t('dataSource.noResources')}</Status> : null}{!resourceLoading && !resourceError ? <ul className="wk-list" role="tree">{visibleResources.map((resource) => <li key={resource.external_id} className="items-center!" role="treeitem"><div className="grid gap-[0.2rem] min-w-0"><label className="inline-flex min-w-0 cursor-pointer items-center gap-[0.45rem]"><input className="wk-resource-checkbox" type="checkbox" checked={resourceStates.get(resource.external_id) === "checked"} aria-checked={resourceStates.get(resource.external_id) === "indeterminate" ? "mixed" : resourceStates.get(resource.external_id) === "checked"} data-state={resourceStates.get(resource.external_id)} onChange={() => setForm((current) => ({ ...current, resourceIds: toggleResourceSelection(resources, current.resourceIds, resource.external_id) }))} /><span className={`wk-resource-check is-${resourceStates.get(resource.external_id) ?? "unchecked"}`} aria-hidden="true">{resourceStates.get(resource.external_id) === "checked" ? "✓" : resourceStates.get(resource.external_id) === "indeterminate" ? "−" : ""}</span> <strong title={resource.name} className="overflow-hidden text-ellipsis whitespace-nowrap">{resource.name}</strong></label><small className="text-[#718096]">{resource.type}</small></div>{resource.has_children ? <Button type="button" onClick={() => void loadResources(resourceSource, resource.external_id, [...resourceTrail, { id: resource.external_id, name: resource.name }])}>{t('dataSource.next')}</Button> : null}</li>)}</ul> : null}</Card> : null}
    {editing !== undefined ? <Card className="mt-4"><div className="wk-header"><div><h2 className="m-0">{editing ? t('dataSource.editTitle') : t('dataSource.createTitle')}</h2><p className="wk-muted">{t('dataSource.credentialsLabel')}</p></div><Button type="button" onClick={() => setEditing(undefined)}>{t('dataSource.close')}</Button></div><form className="wk-wiki-editor" onSubmit={save}><label>{t('dataSource.nameLabel')} <input required value={form.name} onChange={(event) => updateForm('name', event.target.value)} /></label><label>{t('dataSource.connectorTypeLabel')} <select required value={form.type} onChange={(event) => updateForm('type', event.target.value)}>{types.map((type) => <option key={type.type} value={type.type}>{type.name} ({type.type})</option>)}</select></label><label>{t('dataSource.credentialsLabel')} <textarea rows={4} value={form.credentialsText} onChange={(event) => updateForm('credentialsText', event.target.value)} placeholder="token = secret\nOne key=value per line" /></label><label>{t('dataSource.connectorSettingsLabel')} <textarea rows={4} value={form.settingsText} onChange={(event) => updateForm('settingsText', event.target.value)} placeholder="workspace_id = example\nOne key=value per line" /></label><label>{t('dataSource.syncScheduleLabel')} <input required value={form.schedule} onChange={(event) => updateForm('schedule', event.target.value)} /></label><label>{t('dataSource.syncModeLabel')} <select value={form.mode} onChange={(event) => updateForm('mode', event.target.value as DataSourceFormValues['mode'])}><option value="incremental">{t('dataSource.syncMode.incremental')}</option><option value="full">{t('dataSource.syncMode.full')}</option></select></label><label>{t('dataSource.conflictLabel')} <select value={form.conflict} onChange={(event) => updateForm('conflict', event.target.value as DataSourceFormValues['conflict'])}><option value="overwrite">{t('dataSource.conflict.overwrite')}</option><option value="skip">{t('dataSource.conflict.skip')}</option></select></label><label><input type="checkbox" checked={form.deletions} onChange={(event) => updateForm('deletions', event.target.checked)} /> {t('dataSource.syncDeletions')}</label><Button type="submit" loading={saving}>{t('dataSource.save')}</Button></form></Card> : null}
  </main>;
}
