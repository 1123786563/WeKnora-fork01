...
export function DataSourcesPage({ client, knowledgeBaseId, canManage = false, embedded = false }: { client: WeKnoraClient; knowledgeBaseId: string; canManage?: boolean; embedded?: boolean }) {
  ...
  const editorSurface = editing === undefined ? null : <Sheet open title={editorTitle} onClose={() => setEditing(undefined)} closeLabel={t('dataSource.close')} width="640px" className="wk-data-source-drawer">
    {editing === null && createStep === 'type' ? <div className="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-3">
      ...
      {kind === 'inputs' ? <fieldset className="grid gap-3 rounded-lg border border-line-soft p-3"><legend className="px-1 text-[13px] font-semibold">{t('dataSource.credentialsLabel')}</legend>{VUE_CREDENTIAL_FIELDS[form.type] ? VUE_CREDENTIAL_FIELDS[form.type].map((field) => <label key={field.key} className="grid gap-1 text-[13px]"><span>{t(field.label)}</span><Input type={field.secret ? 'password' : 'text'} required={!field.optional} placeholder={field.placeholder || t('dataSource.credential.inputPlaceholder')} value={credentialValue(form.credentialsText, field.key)} onChange={(event) => updateForm('credentialsText', setCredentialValue(form.credentialsText, field.key, event.target.value))} />{field.hint ? <small className="text-muted">{t(field.hint)}</small> : null}</label>) : form.type === 'rss' ? <div className="grid gap-2" data-kind="rss-auth-headers"><div className="flex items-center justify-between gap-2"><span className="text-[13px] font-semibold">{t('dataSource.field.authHeaders')}</span><Button type="button" variant="text" size="small" onClick={() => updateForm('authHeaders', [...(form.authHeaders ?? []), { key: '', value: '' }])}>{t('dataSource.credential.headerAdd')}</Button></div><small className="text-muted">{t('dataSource.field.authHeadersHint')}</small>{(form.authHeaders ?? []).map((row, index) => <div key={index} className="flex items-center gap-2"><Input aria-label={t('dataSource.credential.headerKeyPlaceholder')} autoComplete="off" placeholder={t('dataSource.credential.headerKeyPlaceholder')} value={row.key} onChange={(event) => updateForm('authHeaders', (form.authHeaders ?? []).map((item, at) => at === index ? { ...item, key: event.target.value } : item))} /><Input aria-label={t('dataSource.credential.headerValuePlaceholder')} autoComplete="off" placeholder={t('dataSource.credential.headerValuePlaceholder')} value={row.value} onChange={(event) => updateForm('authHeaders', (form.authHeaders ?? []).map((item, at) => at === index ? { ...item, value: event.target.value } : item))} /><Button type="button" variant="text" size="small" aria-label={t('common.delete')} onClick={() => updateForm('authHeaders', (form.authHeaders ?? []).filter((_, at) => at !== index))}>×</Button></div>)}</div> : <Textarea rows={4} value={form.credentialsText} onChange={(event) => updateForm('credentialsText', event.target.value)} placeholder={t('dataSource.credentialsPlaceholder')} />}</fieldset> : null}
      {editing && credentialStep.replaceMode ? <Button type="button" variant="text" size="small" className="justify-self-start" onClick={() => { setCredentialStep((current) => credentialStepReducer(current, 'cancel-replace')); setForm((current) => ({ ...current, credentialsText: '', authHeaders: [] })); }}>{t('common.cancel')}</Button> : null}
      <fieldset className="grid gap-3 rounded-lg border border-line-soft p-3"><legend className="px-1 text-[13px] font-semibold">{t('dataSource.connectorSettingsLabel')}</legend>{form.type === 'gitlab' ? <div className="grid gap-2" data-kind="gitlab-projects"><div className="flex items-center justify-between gap-2"><span className="text-[13px] font-semibold">{t('dataSource.gitlab.projects')}</span><Button type="button" variant="text" size="small" onClick={() => updateForm('gitlabProjects', [...(form.gitlabProjects ?? []), { ...emptyGitLabProject }])}>{t('dataSource.gitlab.addProject')}</Button></div><small className="text-muted">{t('dataSource.gitlab.projectsHint')}</small>{(form.gitlabProjects ?? []).map((project, index) => <div key={index} className="grid gap-2 rounded-lg border border-line-soft p-3" data-kind="gitlab-project-row"><div className="flex items-center justify-between"><strong className="text-[13px]">{t('dataSource.gitlab.project')} {index + 1}</strong><Button type="button" variant="danger" size="small" aria-label={t('common.delete')} onClick={() => updateForm('gitlabProjects', (form.gitlabProjects ?? []).filter((_, at) => at !== index))}>×</Button></div><label className="grid gap-1 text-[13px]"><span>{t('dataSource.gitlab.projectId')} *</span><Input required placeholder={t('dataSource.gitlab.projectIdPlaceholder')} value={project.project_id} onChange={(event) => updateForm('gitlabProjects', (form.gitlabProjects ?? []).map((item, at) => at === index ? { ...item, project_id: event.target.value } : item))} /></label><label className="grid gap-1 text-[13px]"><span>{t('dataSource.gitlab.ref')}</span><Input placeholder={t('dataSource.gitlab.refPlaceholder')} value={project.ref} onChange={(event) => updateForm('gitlabProjects', (form.gitlabProjects ?? []).map((item, at) => at === index ? { ...item, ref: event.target.value } : item))} /></label><label className="grid gap-1 text-[13px]"><span>{t('dataSource.gitlab.paths')}</span><Textarea rows={2} placeholder={t('dataSource.gitlab.pathsPlaceholder')} value={project.pathsText} onChange={(event) => updateForm('gitlabProjects', (form.gitlabProjects ?? []).map((item, at) => at === index ? { ...item, pathsText: event.target.value } : item))} /></label></div>)}</div> : VUE_SETTINGS_FIELDS[form.type] ? VUE_SETTINGS_FIELDS[form.type].map((field) => <label key={field.key} className="grid gap-1 text-[13px]"><span>{t(field.label)}</span><Textarea rows={3} required={!field.optional} placeholder={field.placeholder || t('dataSource.credential.inputPlaceholder')} value={credentialValue(form.settingsText, field.key)} onChange={(event) => updateForm('settingsText', setCredentialValue(form.settingsText, field.key, event.target.value))} />{field.hint ? <small className="text-muted">{t(field.hint)}</small> : null}</label>) : <Textarea rows={4} value={form.settingsText} onChange={(event) => updateForm('settingsText', event.target.value)} placeholder={t('dataSource.settingsPlaceholder')} />}</fieldset>
  ...
  const deleteSurface = deleteSource === null ? null : <Sheet open title={t('dataSource.deletePanelTitle', { name: deleteSource.name })} onClose={() => setDeleteSource(null)} closeLabel={t('dataSource.close')} className="wk-data-source-delete-drawer">
    <div className="grid gap-3 text-[13px]" data-kind="delete-panel">
      ...
      <label className="flex items-center gap-2"><Checkbox checked={deletePurge} onChange={(event) => setDeletePurge(event.target.checked)} /> {purgeLabelText}</label>
      <div className="flex items-center justify-end gap-2">
        <Button type="button" variant="text" onClick={() => setDeleteSource(null)}>{t('common.cancel')}</Button>
        <Button type="button" variant="danger" loading={action === `delete:${deleteSource.id}`} onClick={() => void confirmDelete()}>{deletePurge ? t('dataSource.deleteAndPurge') : t('dataSource.delete')}</Button>
      </div>
    </div>
  </Sheet>;
