...
export function DataSourcesPage({ client, knowledgeBaseId, canManage = false, embedded = false }: { client: WeKnoraClient; knowledgeBaseId: string; canManage?: boolean; embedded?: boolean }) {
  ...
  return <main data-can-manage={canManage ? 'true' : 'false'} data-embedded={embedded ? 'true' : undefined} className={embedded ? 'wk-data-source-root' : 'wk-data-source-root wk-page wk-ds-83'}>{embedded ? null : <header className="wk-header wk-ds-38"><div><p className="wk-eyebrow wk-ds-39">{t('common.knowledgeBases')} · {knowledgeBaseId}</p><h1 className="wk-ds-40">{t('dataSource.title')}</h1><p className="wk-muted wk-ds-12">{t('dataSource.description')}</p></div></header>}
    ...
    {resourceSource ? <Card className="wk-ds-55"><div className="wk-header wk-ds-56"><div><h2>{t('dataSource.resourceHint')} · {resourceSource.name}</h2><p className="wk-muted wk-ds-12">{resourceTrail.length > 0 ? resourceTrail.map((item) => item.name).join(" / ") : t("dataSource.resourceHint")}</p></div><Button type="button" theme="default" variant="outline" onClick={() => setResourceSource(null)}>{t('dataSource.close')}</Button></div>{resourceSource && isDriveConnector(resourceSource.type) ? <div className="wk-ds-67" data-kind="drive-folder-input"><label className="wk-ds-31">{t('dataSource.drive.folderTokenLabel')}</label><div className="wk-ds-21"><Input aria-label={t('dataSource.drive.folderTokenLabel')} value={driveToken} placeholder={t('dataSource.drive.folderTokenPlaceholder')} onChange={(value) => { setDriveToken(String(value)); setDriveTokenError(false); }} onEnter={() => void loadDriveRoot(resourceSource)} /><Button type="button" theme="default" variant="outline" loading={resourceLoading} onClick={() => void loadDriveRoot(resourceSource)}>{t('dataSource.drive.load')}</Button></div>{driveTokenError ? <small className="wk-ds-20">{t('dataSource.drive.folderTokenRequired')}</small> : <small className="wk-ds-12">{t('dataSource.drive.shareHint')}</small>}</div> : null}{resourceTrail.length > 0 ? <Button type="button" theme="default" variant="outline" onClick={() => { const previous = resourceTrail.slice(0, -1); void loadResources(resourceSource, previous.length > 0 ? previous[previous.length - 1]!.id : undefined, previous); }}>{t('dataSource.back')}</Button> : null}<Button type="button" theme="default" variant="outline" onClick={() => void expandAllResources()} disabled={resourceLoading || !resources.some((resource) => resource.has_children)}>{t('dataSource.loadMore')}</Button>{resourceLoading ? <Status>{t('common.loading')}</Status> : null}{resourceError ? <Status tone="error">{resourceError}</Status> : null}{!resourceLoading && !resourceError && visibleResources.length === 0 ? (resourceSource && isDriveConnector(resourceSource.type) && !driveRootLoaded ? <div className="wk-ds-68" data-kind="drive-placeholder"><p className="wk-ds-69">{t('dataSource.drive.placeholderTitle')}</p><p className="wk-ds-70">{t('dataSource.drive.placeholderDesc')}</p></div> : <Status>{t('dataSource.noResources')}</Status>) : null}{!resourceLoading && !resourceError ? <ul className="wk-list wk-ds-57" role="tree">{visibleResources.map((resource) => <li key={resource.external_id} className="wk-ds-71" role="treeitem"><div className="wk-ds-72"><label className="wk-ds-73"><input type="checkbox" className="wk-ds-74" checked={resourceStates.get(resource.external_id) === "checked"} aria-checked={resourceStates.get(resource.external_id) === "indeterminate" ? "mixed" : resourceStates.get(resource.external_id) === "checked"} data-state={resourceStates.get(resource.external_id)} onChange={() => setForm((current) => ({ ...current, resourceIds: toggleResourceSelection(resources, current.resourceIds, resource.external_id) }))} /><span className={`wk-ds-80 ${(resourceStates.get(resource.external_id) ?? "unchecked") === "unchecked" ? "wk-ds-81" : "wk-ds-82"}`} aria-hidden="true">{resourceStates.get(resource.external_id) === "checked" ? "✓" : resourceStates.get(resource.external_id) === "indeterminate" ? "−" : ""}</span> <strong title={resource.name} className="wk-ds-75">{resource.name}</strong></label><small className="wk-ds-76">{resource.type}</small></div>{resource.has_children ? <Button type="button" theme="default" variant="outline" onClick={() => void loadResources(resourceSource, resource.external_id, [...resourceTrail, { id: resource.external_id, name: resource.name }])}>{t('dataSource.next')}</Button> : null}</li>)}</ul> : null}</Card> : null}
