...
export function AdministrationPage({ client, tenantId, systemAdmin = false }: { client: WeKnoraClient; tenantId: number; systemAdmin?: boolean }) {
  ...
  async function load() {
    ...
    const results = await Promise.allSettled([identity.tenants.members.list(tenantId, { page: 1, pageSize: 100, q: appliedMemberSearch || undefined }), client.auth.me()]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try { setMembers((await identity.tenants.members.list(tenantId, { page: 1, pageSize: 100, q: appliedMemberSearch || undefined })).items); }
      catch (reason) { setError(errorText(reason, t('mobileAdministration.loadFailed'))); }
    })();
  }, [appliedMemberSearch, client, tenantId]);
