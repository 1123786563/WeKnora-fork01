...
export function AdministrationPage({ client, tenantId, systemAdmin = false }: { client: WeKnoraClient; tenantId: number; systemAdmin?: boolean }) {
  ...
  return <main className="wk-page max-w-[1180px]!"><header className="wk-header"><div><p className="wk-eyebrow">Workspace administration · tenant {tenantId}</p><h1>{systemAdmin ? 'Administration' : 'Members and audit'}</h1><p className="wk-muted">Server-confirmed membership and audit state. Failed mutations never remove local rows optimistically.</p></div><Button type="button" onClick={() => void load()} disabled={loading}>Reload</Button></header>{error ? <Status tone="error">{error}</Status> : null}<div className="grid grid-cols-2 gap-4 max-[720px]:grid-cols-1"><Card><h2 className="mt-0 mb-3">Members</h2>{loading ? <Status>Loading…</Status> : members.length === 0 ? <Status>No members returned.</Status> : <ul className="wk-list">{members.map((member) => <li key={member.user_id}><div className="wk-list-item-copy"><strong>{member.username}</strong><span>{member.email} · {roleLabel(member.role)} · {member.status}</span></div><div className="wk-list-actions"><select value={member.role === 'owner' ? 'owner' : member.role} disabled={member.role === 'owner'} onChange={(event) => void updateRole(member, event.target.value as TenantMember['role'])}><option value="owner">Owner</option><option value="admin">Admin</option><option value="contributor">Contributor</option><option value="viewer">Viewer</option></select><Button type="button" disabled={member.role === 'owner'} onClick={() => void removeMember(member)}>Remove</Button></div></li>)}</ul>}</Card><Card><h2 className="mt-0 mb-3">Invite member</h2><form className="grid gap-[0.8rem]" onSubmit={invite}><label className="grid gap-[0.35rem] font-semibold">Email<input className="border border-[#cbd5e1] rounded-control bg-surface text-ink [font:inherit] px-[0.65rem] py-[0.55rem]" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} /></label><label className="grid gap-[0.35rem] font-semibold">Role<select className="border border-[#cbd5e1] rounded-control bg-surface text-ink [font:inherit] px-[0.65rem] py-[0.55rem]" value={role} onChange={(event) => setRole(event.target.value as typeof role)}><option value="admin">Admin</option><option value="contributor">Contributor</option><option value="viewer">Viewer</option></select></label><Button type="submit" loading={saving}>Send invitation</Button></form></Card><Card><h2 className="mt-0 mb-3">Open invitations</h2>{invitations.filter((item) => invitationIsOpen(item.status)).length === 0 ? <Status>No pending invitations.</Status> : <ul className="wk-list">{invitations.filter((item) => invitationIsOpen(item.status)).map((item) => <li key={item.id}><div className="wk-list-item-copy"><strong>{item.invitee_email ?? item.invitee_user_id}</strong><span>{roleLabel(item.role)} · expires {item.expires_at}</span></div><Button type="button" onClick={() => void revokeInvitation(item)}>Revoke</Button></li>)}</ul>}</Card><Card><h2 className="mt-0 mb-3">Audit log</h2>{audit.length === 0 ? <Status>No audit entries returned.</Status> : <ul className="wk-list">{audit.map((item) => <li key={item.id}><div className="wk-list-item-copy"><strong>{item.action}</strong><span>{item.outcome} · {item.actor_role}</span><small>{item.created_at} · {item.request_method} {item.request_path}</small></div></li>)}</ul>}</Card>{systemAdmin ? <><Card><h2 className="mt-0 mb-3">System administrators</h2><ul className="wk-list">{admins.map((item, index) => <li key={String(item.id ?? index)}><strong>{String(item.username ?? item.email ?? item.id)}</strong><span>{item.is_active === false ? 'inactive' : 'active'}</span></li>)}</ul></Card><Card><h2 className="mt-0 mb-3">System settings</h2><ul className="wk-list">{settings.map((item, index) => <li key={String(item.id ?? index)}><div className="wk-list-item-copy"><strong>{String(item.key ?? 'setting')}</strong><span>{item.is_secret === true ? 'secret' : String(item.value ?? '')}</span></div></li>)}</ul><p className="wk-muted">System settings are listed here; writes remain behind the explicit system-admin configuration flow.</p></Card><Card><h2 className="mt-0 mb-3">Runtime queues</h2><Status>{queues ? `Runtime endpoint available: ${queues.available === true ? 'yes' : 'no'}` : 'No queue state returned.'}</Status></Card></> : null}</div></main>;
}
