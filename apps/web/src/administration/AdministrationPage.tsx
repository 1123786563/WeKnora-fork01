...
import { invitationIsOpen } from './summary.ts';
...
export function AdministrationPage({ client, tenantId, systemAdmin = false }: { client: WeKnoraClient; tenantId: number; systemAdmin?: boolean }) {
  ...
  async function load() {
    ...
    if (invitationResult.status === 'fulfilled') setInvitations(invitationResult.value.items); else setError((current) => current ?? errorText(invitationResult.reason, t('mobileAdministration.loadFailed')));
    if (auditResult.status === 'fulfilled') setAudit(auditResult.value.items); else setError((current) => current ?? errorText(auditResult.reason, t('mobileAdministration.loadFailed')));
    if (systemAdmin) {
  ...
  async function updateRole(member: TenantMember, nextRole: TenantMember['role']) { setError(null); try { await identity.tenants.members.updateRole(tenantId, member.user_id, nextRole); await load(); } catch (reason) { setError(errorText(reason, t('mobileAdministration.unableToUpdateRole'))); } }
  async function removeMember(member: TenantMember) { if (!window.confirm(`${t('mobileAdministration.removeTitle')} ${t('mobileAdministration.removeMessage', { name: member.username })}`)) return; setError(null); try { await identity.tenants.members.remove(tenantId, member.user_id); await load(); } catch (reason) { setError(errorText(reason, t('mobileAdministration.unableToRemove'))); } }
  async function revokeInvitation(invitation: TenantInvitation) { if (!window.confirm(`${t('mobileAdministration.revoke')}: ${invitation.invitee_email ?? invitation.id}?`)) return; setError(null); try { await identity.tenants.invitations.revoke(tenantId, invitation.id); await load(); } catch (reason) { setError(errorText(reason, t('mobileAdministration.unableToRevoke'))); } }
