...
export function AdministrationPage({ client, tenantId, systemAdmin = false }: { client: WeKnoraClient; tenantId: number; systemAdmin?: boolean }) {
  ...
  if (!manageTenant) return <main className="wk-page wk-admin-1"><header className="wk-header wk-admin-2"><div><p className="wk-eyebrow wk-admin-3">{t('mobileAdministration.tenant', { tenant: tenantId })}</p><h1 className="wk-admin-4">{t('mobileAdministration.title')}</h1><p className="wk-muted wk-admin-5">{t('mobileAdministration.readOnly')}</p></div><Button type="button" onClick={() => void load()} disabled={loading}>{t('mobileAdministration.refresh')}</Button></header>{error ? <Status tone="error">{error}</Status> : null}<Card><h2 className="wk-admin-6">{t('mobileAdministration.members', { count: membersTotal })}</h2><Input type="search" value={memberSearch} placeholder={t('mobileAdministration.searchPlaceholder')} aria-label={t('mobileAdministration.searchPlaceholder')} onChange={(event) => setMemberSearch(event.target.value)} className="wk-admin-7" />{loading ? <Status>{t('mobileAdministration.loading')}</Status> : members.length === 0 ? <Status>{appliedMemberSearch ? t('mobileAdministration.noMembersForQuery', { q: appliedMemberSearch }) : t('mobileAdministration.noMembers')}</Status> : <ul className="wk-list wk-admin-8">{members.map((member) => <li key={member.user_id} className="wk-admin-9"><div className="wk-list-item-copy wk-admin-10"><strong>{member.username}</strong><span className="wk-admin-11">{member.email} · {roleText(member.role)} · {member.status}</span></div></li>)}</ul>}{membersTotal > 0 ? <MembersPager total={membersTotal} page={membersPage} pageSize={membersPageSize} onPage={onMembersPage} onPageSize={onMembersPageSize} t={t} /> : null}</Card></main>;

  return <main className="wk-page wk-admin-1"><header className="wk-header wk-admin-2"><div><p className="wk-eyebrow wk-admin-3">{t('mobileAdministration.tenant', { tenant: tenantId })}</p><h1 className="wk-admin-4">{systemAdmin ? t('settings.navGroups.systemAdministration') : t('mobileAdministration.title')}</h1><p className="wk-muted wk-admin-5">{t('mobileAdministration.readOnly')}</p></div><Button type="button" onClick={() => void load()} disabled={loading}>{t('mobileAdministration.refresh')}</Button></header>{error ? <Status tone="error">{error}</Status> : null}<div className="wk-admin-12"><Card><h2 className="wk-admin-6">{t('mobileAdministration.members', { count: membersTotal })}</h2><Input type="search" value={memberSearch} placeholder={t('mobileAdministration.searchPlaceholder')} aria-label={t('mobileAdministration.searchPlaceholder')} onChange={(event) => setMemberSearch(event.target.value)} className="wk-admin-7" />{loading ? <Status>{t('mobileAdministration.loading')}</Status> : members.length === 0 ? <Status>{appliedMemberSearch ? t('mobileAdministration.noMembersForQuery', { q: appliedMemberSearch }) : t('mobileAdministration.noMembers')}</Status> : <ul className="wk-list wk-admin-8">{members.map((member) => <li key={member.user_id} className="wk-admin-9"><div className="wk-list-item-copy wk-admin-10"><strong>{member.username}</strong><span className="wk-admin-11">{member.email} · {roleText(member.role)} · {member.status}</span></div><div className="wk-list-actions wk-admin-13"><Select value={member.role === 'owner' ? 'owner' : member.role} disabled={member.role === 'owner'} onChange={(event) => void updateRole(member, event.target.value as TenantMember['role'])} className="wk-admin-14"><option value="owner">{roleText('owner')}</option><option value="admin">{roleText('admin')}</option><option value="contributor">{roleText('contributor')}</option><option value="viewer">{roleText('viewer')}</option></Select><Button type="button" disabled={member.role === 'owner'} onClick={() => void removeMember(member)}>{t('mobileAdministration.remove')}</Button></div></li>)}</ul>}{membersTotal > 0 ? <MembersPager total={membersTotal} page={membersPage} pageSize={membersPageSize} onPage={onMembersPage} onPageSize={onMembersPageSize} t={t} /> : null}</Card><Card><h2 className="wk-admin-6">{inviteStep === 'confirm' ? t('mobileAdministration.confirmInviteTitle') : t('mobileAdministration.invite')}</h2>{inviteStep === 'confirm' ? <div className="wk-admin-15"><p className="wk-muted wk-admin-16">{t('mobileAdministration.confirmInviteBody', { email: email.trim(), role: roleText(role) })}</p><div className="wk-admin-17"><Button type="button" disabled={saving} onClick={() => setInviteStep('form')}>{t('mobileAdministration.back')}</Button><Button type="button" loading={saving} onClick={() => void sendInvitation()}>{t('mobileAdministration.confirmSend')}</Button></div></div> : <form className="wk-admin-15" onSubmit={onInviteFormSubmit}><label className="wk-admin-18">{t('mobileAdministration.inviteEmail')}<Input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} /></label><label className="wk-admin-18">{t('mobileAdministration.role', { role: '' }).replace(/: $/, '')}<Select value={role} onChange={(event) => setRole(event.target.value as typeof role)}><option value="admin">{roleText('admin')}</option><option value="contributor">{roleText('contributor')}</option><option value="viewer">{roleText('viewer')}</option></Select></label><Button type="submit" loading={saving}>{t('mobileAdministration.sendInvitation')}</Button></form>}</Card><Card><h2 className="wk-admin-6">{t('mobileAdministration.openInvitations')}</h2>{invitations.filter((item) => invitationIsOpen(item.status)).length === 0 ? <Status>{t('mobileAdministration.noPendingInvitations')}</Status> : <ul className="wk-list wk-admin-8">{invitations.filter((item) => invitationIsOpen(item.status)).map((item) => <li key={item.id} className="wk-admin-9"><div className="wk-list-item-copy wk-admin-10"><strong>{item.invitee_email ?? item.invitee_user_id}</strong><span className="wk-admin-11">{roleText(item.role)} · {t('mobileAdministration.expires', { date: item.expires_at })}</span></div><Button type="button" onClick={() => void revokeInvitation(item)}>{t('mobileAdministration.revoke')}</Button></li>)}</ul>}</Card><Card><h2 className="wk-admin-6">{t('mobileAdministration.auditLog')}</h2>{audit.length === 0 ? <Status>{t('mobileAdministration.noAuditEntries')}</Status> : <ul className="wk-list wk-admin-8">{audit.map((item) => <li key={item.id} className="wk-admin-9"><div className="wk-list-item-copy wk-admin-10"><strong>{item.action}</strong><span className="wk-admin-11">{item.outcome} · {item.actor_role}</span><small>{item.created_at} · {item.request_method} {item.request_path}</small></div></li>)}</ul>}</Card>{systemAdmin ? <><Card><h2 className="wk-admin-6">{t('settings.navGroups.systemAdministration')}</h2><ul className="wk-list wk-admin-8">{admins.map((item, index) => <li key={String(item.id ?? index)} className="wk-admin-9"><strong>{String(item.username ?? item.email ?? item.id)}</strong><span className="wk-admin-11">{item.is_active === false ? t('common.disabled') : t('mobileAdministration.status.active')}</span></li>)}</ul></Card><div data-testid="system-administration-panels" className="wk-admin-19">{systemAdminPanelKeys(systemAdmin).map((panel) => panel === 'system-global' ? <SystemGlobalSettingsPanel key={panel} client={client} initialSettings={settings} /> : panel === 'runtime-queues' ? <RuntimeQueuesPanel key={panel} client={client} payload={queues} loading={loading} /> : panel === 'platform-api-keys' ? <PlatformApiKeysPanel key={panel} client={client} initialKeys={apiKeys} /> : <SystemAuditLogPanel key={panel} client={client} payload={systemAudit} />)}</div></> : null}</div></main>;
}
...
const PAGER_BTN = 'wk-admin-pager-btn';
...
function MembersPager({ total, page, pageSize, onPage, onPageSize, t }: {
  ...
  return <div className="wk-admin-20">
    ...
    <Select className="wk-admin-22" value={pageSize} onChange={(event) => onPageSize(Number(event.target.value))}>
      {MEMBERS_PAGE_SIZE_OPTIONS.map((size) => <option key={size} value={size}>{t('mobileAdministration.pager.sizePerPage', { size })}</option>)}
    </Select>
    ...
    <span className="wk-admin-24">
      ...
      <Input className="wk-admin-25" type="text" inputMode="numeric" value={jump}
