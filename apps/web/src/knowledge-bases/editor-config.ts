...
export function defaultKnowledgeEditorConfig(): KnowledgeEditorConfig {
  return {
    ...
    storageBackendId: '', vectorStoreId: '',
...
export function hydrateKnowledgeEditorConfig(source: Record<string, unknown>): KnowledgeEditorConfig {
  ...
  const strategy = record(source.indexing_strategy);
  ...
  return {
    ...
    chunkingConfig: { chunkSize: numberValue(chunk.chunk_size, defaults.chunkingConfig.chunkSize), chunkOverlap: numberValue(chunk.chunk_overlap, defaults.chunkingConfig.chunkOverlap), strategy: stringValue(chunk.strategy, defaults.chunkingConfig.strategy), separators: Array.isArray(chunk.separators) ? chunk.separators.filter((v): v is string => typeof v === 'string') : defaults.chunkingConfig.separators, enableParentChild: boolValue(chunk.enable_parent_child, defaults.chunkingConfig.enableParentChild), parentChunkSize: numberValue(chunk.parent_chunk_size, defaults.chunkingConfig.parentChunkSize), childChunkSize: numberValue(chunk.child_chunk_size, defaults.chunkingConfig.childChunkSize), tokenLimit: numberValue(chunk.token_limit, defaults.chunkingConfig.tokenLimit), languages: Array.isArray(chunk.languages) ? chunk.languages.filter((v): v is string => typeof v === 'string') : [], tableMetadataInstructions: stringValue(chunk.table_metadata_instructions), parserEngineRules: rules },
    storageBackendId: stringValue(source.storage_backend_id), vectorStoreId: stringValue(source.vector_store_id),
    ...
    questionGenerationConfig: { enabled: boolValue(questions.enabled, defaults.questionGenerationConfig.enabled), questionCount: numberValue(questions.question_count, defaults.questionGenerationConfig.questionCount), customInstructions: stringValue(questions.custom_instructions) },
    autoTagConfig: { enabled: boolValue(autoTag.enabled, false), modelId: stringValue(autoTag.model_id), maxTags: numberValue(autoTag.max_tags, 3), skipIfTagged: boolValue(autoTag.skip_if_tagged, true) },
...
export function knowledgeEditorConfigPayload(config: KnowledgeEditorConfig, type: 'document' | 'faq'): Record<string, unknown> {
  ...
  const payload: Record<string, unknown> = {
    ...
    question_generation_config: { enabled: config.questionGenerationConfig.enabled, question_count: config.questionGenerationConfig.questionCount, custom_instructions: config.questionGenerationConfig.customInstructions },
    auto_tag_config: { enabled: config.autoTagConfig.enabled, model_id: config.autoTagConfig.modelId, max_tags: config.autoTagConfig.maxTags, skip_if_tagged: config.autoTagConfig.skipIfTagged },
  ...
  if (type === 'faq') payload.faq_config = { index_mode: config.faqConfig.indexMode, question_index_mode: config.faqConfig.questionIndexMode };
    ...
    payload.indexing_strategy = { vector_enabled: true, keyword_enabled: true, wiki_enabled: false, graph_enabled: config.nodeExtractConfig.enabled };
