...
export function KnowledgeBaseActivityPanel({ client, knowledgeBaseId, embedded = false }: Props) {
  ...
  const filterOptions = (values: string[], selectedValue: string, onSelect: (value: string) => void, open: boolean, setOpen: (value: boolean) => void, label: string) => <div className="wk-kb-activity-filter">
    <button type="button" className={`wk-kb-activity-filter-btn${selectedValue ? ' is-selected' : ''}`} aria-label={label} aria-expanded={open} onClick={() => setOpen(!open)}>⌄</button>
    {open ? <div className="wk-kb-activity-filter-menu" role="menu">
      {['', ...values].map((value) => <button key={value || '__all__'} type="button" role="menuitemradio" aria-checked={selectedValue === value} className={`wk-kb-activity-filter-item${selectedValue === value ? ' is-selected' : ''}`} onClick={() => { onSelect(value); setOpen(false); }}>{value ? (label === t('knowledgeEditor.activity.columns.action') ? actionLabel(value, t) : outcomeLabel(value, t)) : (label === t('knowledgeEditor.activity.columns.action') ? t('knowledgeEditor.activity.allActions') : t('knowledgeEditor.activity.allOutcomes'))}{selectedValue === value ? <span aria-hidden="true">✓</span> : null}</button>)}
    </div> : null}
  </div>;

  return <div className="wk-kb-activity" aria-label={t('knowledgeEditor.activity.title')}>
    <div className="wk-kb-activity-head">
      {embedded ? null : <div><h3 className="wk-kb-activity-title">{t('knowledgeEditor.activity.title')}</h3><p className="wk-kb-activity-desc">{t('knowledgeEditor.activity.description')}</p></div>}
    ...
    {entries.length > 0 ? <div className="wk-kb-activity-table-wrap"><table className="wk-kb-activity-table"><thead><tr className="wk-kb-activity-thead"><th><span className="wk-kb-activity-th-label">{t('knowledgeEditor.activity.columns.action')}{filterOptions(actionValues, filter.action, (value) => setFilter((current) => ({ ...current, action: value })), actionMenuOpen, setActionMenuOpen, t('knowledgeEditor.activity.columns.action'))}</span></th><th><span className="wk-kb-activity-th-label">{t('knowledgeEditor.activity.columns.outcome')}{filterOptions(outcomeValues, filter.outcome, (value) => setFilter((current) => ({ ...current, outcome: value })), outcomeMenuOpen, setOutcomeMenuOpen, t('knowledgeEditor.activity.columns.outcome'))}</span></th><th>{t('knowledgeEditor.activity.columns.target')}</th><th>{t('knowledgeEditor.activity.columns.actor')}</th><th>{t('knowledgeEditor.activity.columns.time')}</th></tr></thead><tbody>{entries.map((entry) => { const target = activityTargetSummary(entry); const time = activityDateTime(entry.created_at, locale); return <tr key={entry.id} className="wk-kb-activity-row" tabIndex={0} onClick={() => setSelected(entry)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelected(entry); } }}><td><span className={`wk-kb-activity-pill wk-kb-activity-pill--action ${activityActionTone(entry.action) === 'error' ? 'is-error' : activityActionTone(entry.action) === 'success' ? 'is-success' : activityActionTone(entry.action) === 'warning' ? 'is-warning' : ''}`}>{actionLabel(entry.action, t)}</span></td><td><span className={`wk-kb-activity-pill wk-kb-activity-pill--outcome ${activityOutcomeTone(entry.outcome) === 'error' ? 'is-error' : activityOutcomeTone(entry.outcome) === 'success' ? 'is-success' : activityOutcomeTone(entry.outcome) === 'warning' ? 'is-warning' : ''}`}>{outcomeLabel(entry.outcome, t)}</span></td><td><span className="wk-kb-activity-target">{target.subject || '—'}</span>{target.change ? <small className="wk-kb-activity-change">{target.change}</small> : null}</td><td className="wk-kb-activity-muted">{actorLabel(entry)}</td><td className="wk-kb-activity-muted"><span className="wk-kb-activity-time">{time.date}</span><span className="wk-kb-activity-time wk-kb-activity-time--clock">{time.time}</span></td></tr>; })}</tbody></table></div> : null}
    ...
    {nextCursor === undefined && entries.length > 0 && !loading ? <p className="wk-kb-activity-end" data-activity-end-hint="">{t('knowledgeEditor.activity.end')}</p> : null}
    <TDrawer footer={false} visible={selected !== null} header={selected ? actionLabel(selected.action, t) : ''} onClose={() => setSelected(null)} placement="right" size="640px">{selected ? <div className="wk-kb-activity-detail"><section><h4>{t('knowledgeEditor.activity.drawer.sectionSummary')}</h4><dl>{[['time', activityDateTime(selected.created_at, locale).date + ' ' + activityDateTime(selected.created_at, locale).time], ['action', actionLabel(selected.action, t)], ['outcome', outcomeLabel(selected.outcome, t)], ['target', targetLabel(selected, t)]].map(([key, value]) => <div key={key} className="wk-kb-activity-detail-row"><dt className="wk-kb-activity-detail-key">{key}</dt><dd>{value}</dd></div>)}</dl></section><section><h4>{t('knowledgeEditor.activity.drawer.sectionIdentifiers')}</h4><div className="wk-kb-activity-detail-row"><span className="wk-kb-activity-detail-key">id</span><code>{selected.id}</code></div></section><section><h4>{t('knowledgeEditor.activity.drawer.sectionTask')}</h4><pre className="wk-kb-activity-detail-pre">{JSON.stringify(selected.details ?? {}, null, 2)}</pre></section><section><h4>JSON</h4><pre className="wk-kb-activity-detail-pre wk-kb-activity-detail-pre--bordered">{JSON.stringify(selected, null, 2)}</pre></section></div> : null}</TDrawer>
  </div>;
}
