              ...
              <div className="settings-content">
                <div className="content-wrapper kb-editor-content">
                  ...
                  {sectionShell('models', editorSection === 'models', (
                    <div className="wk-kb-des-section">
                      <div><h3 className="wk-kb-des-title">{t('knowledgeEditor.models.title')}</h3><p className="wk-kb-des-desc wk-kb-des-desc--spaced">{t('knowledgeEditor.models.description')}</p></div>
                      <div className="wk-kb-des-field" data-guide="kb-create-llm"><label className="wk-kb-des-label">{t('knowledgeEditor.models.llmLabel')}<span className="wk-kb-des-required">*</span></label><p className="wk-kb-des-tip">{t('knowledgeEditor.models.llmDesc')}</p><ModelOptionSelect value={summaryModelId} options={chatModelOptions} addModelLabel={t('model.addModelInSettings')} onAddModel={() => navigate('/platform/settings?section=models&subsection=chat')} onChange={setSummaryModelId} /></div>
                      <div className="wk-kb-des-field" data-guide="kb-create-embedding"><label className="wk-kb-des-label">{t('knowledgeEditor.models.embeddingLabel')}{modelsEmbeddingRequired ? <span className="wk-kb-des-required">*</span> : null}</label><p className="wk-kb-des-tip">{t('knowledgeEditor.models.embeddingDesc')}</p>{modelsEmbeddingRequired && editingHasFiles ? <p className="wk-kb-des-tip" data-embedding-locked-tip="" style={{ margin: '0.5rem 0 0', fontSize: '0.85rem' }}>{t('knowledgeEditor.models.embeddingLocked')}</p> : null}<ModelOptionSelect value={embeddingModelId} options={embeddingModelOptions} disabled={modelsEmbeddingRequired && editingHasFiles} addModelLabel={t('model.addModelInSettings')} onAddModel={() => navigate('/platform/settings?section=models&subsection=embedding')} onChange={setEmbeddingModelId} /></div>
                    </div>
                  ))}

                  {/* VectorStore 绑定（留守段） */}
                  {sectionShell('vectorStore', editorSection === 'vectorStore', (
                    <div className="wk-kb-des-section"><h3 className="wk-kb-des-title">{t('knowledgeEditor.sidebar.vectorStore')}</h3><p className="wk-kb-des-desc">{t('kbSettings.vectorStore.description')}</p><label className="wk-kb-des-field">{t('kbSettings.vectorStore.engineLabel')}<TSelect value={editorConfig.vectorStoreId} disabled={Boolean(editingId) || editorOptions.loading} options={[{ value: '', label: t('kbSettings.vectorStore.systemDefault') }, ...editorOptions.vectorStores.map((store) => ({ value: store.id, label: `${store.name} · ${store.engine_type}` }))]} onChange={(value) => setEditorConfig((current) => ({ ...current, vectorStoreId: String(value) }))} /></label><p className="wk-kb-des-hint">{editingId ? t('kbSettings.vectorStore.immutableHint') : editorOptions.error ?? t('kbSettings.vectorStore.engineDesc')}</p></div>
                  ))}
                  ...
                  {type !== 'faq' && editorSection === 'parser' ? sectionShell('parser', true, (
                    <div className="wk-kb-des-section"><h3 className="wk-kb-des-title">{t('kbSettings.parser.title')}</h3><p className="wk-kb-des-desc">{t('kbSettings.parser.description')}</p><label className="wk-kb-des-field">{t('kbSettings.parser.title')}<TSelect value={editorConfig.chunkingConfig.parserEngineRules[0]?.engine ?? ''} disabled={editorOptions.loading} options={[{ value: '', label: t('kbSettings.parser.default') }, ...editorOptions.parserEngines.filter((engine) => engine.Available !== false).map((engine) => ({ value: engine.Name, label: engine.Name }))]} onChange={(value) => setEditorConfig((current) => ({ ...current, chunkingConfig: { ...current.chunkingConfig, parserEngineRules: String(value) ? [{ file_types: ['pdf'], engine: String(value) }] : [] } }))} /></label><p className="wk-kb-des-hint">{editorOptions.error ?? t('kbSettings.parser.goConfig')}</p></div>
                  )) : null}
                  ...
                  {type !== 'faq' && editorSection === 'storage' ? sectionShell('storage', true, (() => {
                    ...
                    return <div className="wk-kb-des-section"><div><h3 className="wk-kb-des-title">{t('knowledgeEditor.sidebar.storage')}</h3><p className="wk-kb-des-desc wk-kb-des-desc--spaced">{t('kbSettings.storage.selectDescription')}</p></div><div className="wk-kb-des-field"><label className="wk-kb-des-label">{t('kbSettings.storage.instanceLabel')}</label><p className="wk-kb-des-hint">{t('kbSettings.storage.instanceDesc')}</p><TSelect value={editorConfig.storageBackendId} disabled={storageLocked || editorOptions.loading} aria-label={t('kbSettings.storage.instanceLabel')} options={[...editorOptions.storageBackends.map((backend) => ({ value: backend.id, label: `${backend.name} · ${backend.provider.toUpperCase()}${backend.id === editorOptions.defaultStorageBackendId ? ` · ${t('kbSettings.storage.defaultTag')}` : ''}` })), ...(editorOptions.storageBackends.every((backend) => backend.id !== editorConfig.storageBackendId) && editorConfig.storageBackendId ? [{ value: editorConfig.storageBackendId, label: editorConfig.storageBackendId }] : [])]} onChange={(value) => { const backendId = String(value); const backend = editorOptions.storageBackends.find((candidate) => candidate.id === backendId); setEditorConfig((current) => ({ ...current, storageBackendId: backendId, storageProvider: backend?.provider ?? current.storageProvider })); }} />{storageLocked ? <p className="wk-kb-des-hint wk-kb-des-hint--plain" style={{ color: 'var(--color-warning-text, #b54708)' }} data-storage-migrate-hint="">{t('kbSettings.storage.migrateHint')}</p> : null}{!storageLocked && selectedHint ? <p className="wk-kb-des-hint" data-storage-instance-hint="">{selectedHint}</p> : null}<a data-storage-manage-instances="" href="/platform/settings?section=storage" className="wk-kb-des-manage-link" style={{ color: 'var(--color-brand)' }} onClick={(event) => { event.preventDefault(); setDialogOpen(false); navigate('/platform/settings?section=storage'); }}>{t('kbSettings.storage.manageInstances')}</a></div></div>;
                  })()) : null}

                  {/* 分块设置（留守段：ChunkingSettingsFields 与 knowledge-settings 共享） */}
                  {type !== 'faq' ? sectionShell('chunking', editorSection === 'chunking', (
                    <div className="wk-kb-des-section"><div><h3 className="wk-kb-des-title">{t('knowledgeEditor.chunking.title')}</h3><p className="wk-kb-des-desc wk-kb-des-desc--spaced">{t('knowledgeEditor.chunking.description')}</p></div><ChunkingSettingsFields splitting={editorConfig.chunkingConfig} onPatch={(patch) => setEditorConfig((current) => ({ ...current, chunkingConfig: { ...current.chunkingConfig, ...patch } }))} client={client} t={t} /></div>
                  )) : null}

                  {/* 多模态配置（留守段） */}
                  {type !== 'faq' ? sectionShell('multimodal', editorSection === 'multimodal', (
                    <div className="wk-kb-des-section"><div><h3 className="wk-kb-des-title">{t('knowledgeEditor.multimodal.title')}</h3><p className="wk-kb-des-desc wk-kb-des-desc--spaced">{t('knowledgeEditor.multimodal.description')}</p></div><label className="wk-kb-des-check"><TCheckbox checked={editorConfig.multimodalConfig.enabled} onChange={(value) => setEditorConfig((current) => ({ ...current, multimodalConfig: { ...current.multimodalConfig, enabled: Boolean(value) } }))} />{t('knowledgeEditor.advanced.multimodal.label')}</label>{editorConfig.multimodalConfig.enabled ? <><label className="wk-kb-des-field">{t('knowledgeEditor.advanced.multimodal.vllmLabel')}<TInput value={editorConfig.multimodalConfig.vllmModelId} onChange={(value) => setEditorConfig((current) => ({ ...current, multimodalConfig: { ...current.multimodalConfig, vllmModelId: String(value) } }))} placeholder={t('knowledgeEditor.advanced.multimodal.vllmPlaceholder')} /></label><label className="wk-kb-des-field">{t('knowledgeEditor.advanced.multimodal.customInstructionsLabel')}<TTextarea rows={4} maxlength={4000} value={editorConfig.multimodalConfig.customInstructions} onChange={(value) => setEditorConfig((current) => ({ ...current, multimodalConfig: { ...current.multimodalConfig, customInstructions: String(value) } }))} placeholder={t('knowledgeEditor.advanced.multimodal.customInstructionsPlaceholder')} /></label></> : null}</div>
                  )) : null}

                  {/* 音频处理（ASR）设置（留守段） */}
                  {type !== 'faq' ? sectionShell('asr', editorSection === 'asr', (
                    <div className="wk-kb-des-section"><div><h3 className="wk-kb-des-title">{t('knowledgeEditor.asr.title')}</h3><p className="wk-kb-des-desc wk-kb-des-desc--spaced">{t('knowledgeEditor.asr.description')}</p></div><label className="wk-kb-des-check"><TCheckbox checked={editorConfig.asrConfig.enabled} onChange={(value) => setEditorConfig((current) => ({ ...current, asrConfig: { ...current.asrConfig, enabled: Boolean(value) } }))} />{t('knowledgeEditor.asr.label')}</label>{editorConfig.asrConfig.enabled ? <label className="wk-kb-des-field">{t('knowledgeEditor.asr.modelLabel')}<TInput value={editorConfig.asrConfig.modelId} onChange={(value) => setEditorConfig((current) => ({ ...current, asrConfig: { ...current.asrConfig, modelId: String(value) } }))} placeholder={t('knowledgeEditor.asr.modelPlaceholder')} /></label> : null}</div>
                  )) : null}
                  ...
                  {type !== 'faq' ? sectionShell('advanced', editorSection === 'advanced', (
                    <div className="wk-kb-des-section"><div><h3 className="wk-kb-des-title">{t('knowledgeEditor.advanced.title')}</h3><p className="wk-kb-des-desc wk-kb-des-desc--spaced">{t('knowledgeEditor.advanced.description')}</p></div><label className="wk-kb-des-check"><TCheckbox checked={editorConfig.questionGenerationConfig.enabled} onChange={(value) => setEditorConfig((current) => ({ ...current, questionGenerationConfig: { ...current.questionGenerationConfig, enabled: Boolean(value) } }))} />{t('knowledgeEditor.advanced.questionGeneration.label')}</label>{editorConfig.questionGenerationConfig.enabled ? <label className="wk-kb-des-field">{t('knowledgeEditor.advanced.questionGeneration.countLabel')}<TInput value={String(editorConfig.questionGenerationConfig.questionCount)} onChange={(value) => setEditorConfig((current) => ({ ...current, questionGenerationConfig: { ...current.questionGenerationConfig, questionCount: Number(String(value)) } }))} /></label> : null}<label className="wk-kb-des-check"><TCheckbox checked={editorConfig.autoTagConfig.enabled} onChange={(value) => setEditorConfig((current) => ({ ...current, autoTagConfig: { ...current.autoTagConfig, enabled: Boolean(value) } }))} />{t('knowledgeEditor.advanced.autoTag.label')}</label><label className="wk-kb-des-field">{t('knowledgeEditor.advanced.tableMetadataInstructions.label')}<TTextarea rows={3} maxlength={4000} value={editorConfig.chunkingConfig.tableMetadataInstructions} placeholder={t('knowledgeEditor.advanced.tableMetadataInstructions.placeholder')} onChange={(value) => setEditorConfig((current) => ({ ...current, chunkingConfig: { ...current.chunkingConfig, tableMetadataInstructions: String(value) } }))} /><span className="kb-editor-desc-count" aria-live="polite" data-table-metadata-count="">{editorConfig.chunkingConfig.tableMetadataInstructions.length}/4000</span></label></div>
                  )) : null}

                  {/* 数据源管理（仅编辑模式，留守段） */}
                  {editingId && editorSection === 'datasource' ? sectionShell('datasource', true, (
                    <div className="wk-kb-des-scroll"><DataSourcesPage client={client} knowledgeBaseId={editingId} canManage={viewer.isAdmin} embedded /></div>
                  )) : null}
