...
import { Checkbox as WkCheckbox, Input as WkInput, Select as WkSelect, Textarea as WkTextarea } from '@weknora/ui';
import {
              ...
              <div className="settings-content">
                <div className="content-wrapper kb-editor-content">
                  ...
                  {sectionShell('models', editorSection === 'models', (
                    <div className="wk-kbl-1">
                      <div><h3 className="wk-kbl-2">{t('knowledgeEditor.models.title')}</h3><p className="wk-kbl-3">{t('knowledgeEditor.models.description')}</p></div>
                      <div className="wk-kbl-4" data-guide="kb-create-llm"><label className="wk-kbl-5">{t('knowledgeEditor.models.llmLabel')}<span className="wk-kbl-6">*</span></label><p className="wk-kbl-7">{t('knowledgeEditor.models.llmDesc')}</p><ModelOptionSelect value={summaryModelId} options={chatModelOptions} addModelLabel={t('model.addModelInSettings')} onAddModel={() => navigate('/platform/settings?section=models&subsection=chat')} onChange={setSummaryModelId} /></div>
                      <div className="wk-kbl-4" data-guide="kb-create-embedding"><label className="wk-kbl-5">{t('knowledgeEditor.models.embeddingLabel')}{modelsEmbeddingRequired ? <span className="wk-kbl-6">*</span> : null}</label><p className="wk-kbl-7">{t('knowledgeEditor.models.embeddingDesc')}</p>{modelsEmbeddingRequired && editingHasFiles ? <p className="wk-kbl-7" data-embedding-locked-tip="" style={{ margin: '0.5rem 0 0', fontSize: '0.85rem' }}>{t('knowledgeEditor.models.embeddingLocked')}</p> : null}<ModelOptionSelect value={embeddingModelId} options={embeddingModelOptions} disabled={modelsEmbeddingRequired && editingHasFiles} addModelLabel={t('model.addModelInSettings')} onAddModel={() => navigate('/platform/settings?section=models&subsection=embedding')} onChange={setEmbeddingModelId} /></div>
                    </div>
                  ))}

                  {/* VectorStore 绑定（留守段） */}
                  {sectionShell('vectorStore', editorSection === 'vectorStore', (
                    <div className="wk-kbl-1"><h3 className="wk-kbl-2">{t('knowledgeEditor.sidebar.vectorStore')}</h3><p className="wk-kbl-8">{t('kbSettings.vectorStore.description')}</p><label className="wk-kbl-4">{t('kbSettings.vectorStore.engineLabel')}<WkSelect value={editorConfig.vectorStoreId} disabled={Boolean(editingId) || editorOptions.loading} onChange={(event) => setEditorConfig((current) => ({ ...current, vectorStoreId: event.target.value }))}><option value="">{t('kbSettings.vectorStore.systemDefault')}</option>{editorOptions.vectorStores.map((store) => <option key={store.id} value={store.id}>{store.name} · {store.engine_type}</option>)}</WkSelect></label><p className="wk-kbl-9">{editingId ? t('kbSettings.vectorStore.immutableHint') : editorOptions.error ?? t('kbSettings.vectorStore.engineDesc')}</p></div>
                  ))}
                  ...
                  {type !== 'faq' && editorSection === 'parser' ? sectionShell('parser', true, (
                    <div className="wk-kbl-1"><h3 className="wk-kbl-2">{t('kbSettings.parser.title')}</h3><p className="wk-kbl-8">{t('kbSettings.parser.description')}</p><label className="wk-kbl-4">{t('kbSettings.parser.title')}<WkSelect value={editorConfig.chunkingConfig.parserEngineRules[0]?.engine ?? ''} disabled={editorOptions.loading} onChange={(event) => setEditorConfig((current) => ({ ...current, chunkingConfig: { ...current.chunkingConfig, parserEngineRules: event.target.value ? [{ file_types: ['pdf'], engine: event.target.value }] : [] } }))}><option value="">{t('kbSettings.parser.default')}</option>{editorOptions.parserEngines.filter((engine) => engine.Available !== false).map((engine) => <option key={engine.Name} value={engine.Name}>{engine.Name}</option>)}</WkSelect></label><p className="wk-kbl-9">{editorOptions.error ?? t('kbSettings.parser.goConfig')}</p></div>
                  )) : null}
                  ...
                  {type !== 'faq' ? sectionShell('multimodal', editorSection === 'multimodal', (
                    <div className="wk-kbl-1"><div><h3 className="wk-kbl-2">{t('knowledgeEditor.multimodal.title')}</h3><p className="wk-kbl-3">{t('knowledgeEditor.multimodal.description')}</p></div><label className="wk-kbl-12"><WkCheckbox checked={editorConfig.multimodalConfig.enabled} onChange={(event) => setEditorConfig((current) => ({ ...current, multimodalConfig: { ...current.multimodalConfig, enabled: event.target.checked } }))} />{t('knowledgeEditor.advanced.multimodal.label')}</label>{editorConfig.multimodalConfig.enabled ? <><label className="wk-kbl-4">{t('knowledgeEditor.advanced.multimodal.vllmLabel')}<WkInput value={editorConfig.multimodalConfig.vllmModelId} onChange={(event) => setEditorConfig((current) => ({ ...current, multimodalConfig: { ...current.multimodalConfig, vllmModelId: event.target.value } }))} placeholder={t('knowledgeEditor.advanced.multimodal.vllmPlaceholder')} /></label><label className="wk-kbl-4">{t('knowledgeEditor.advanced.multimodal.customInstructionsLabel')}<WkTextarea rows={4} maxLength={4000} value={editorConfig.multimodalConfig.customInstructions} onChange={(event) => setEditorConfig((current) => ({ ...current, multimodalConfig: { ...current.multimodalConfig, customInstructions: event.target.value } }))} placeholder={t('knowledgeEditor.advanced.multimodal.customInstructionsPlaceholder')} /></label></> : null}</div>
                  )) : null}

                  {/* 音频处理（ASR）设置（留守段） */}
                  {type !== 'faq' ? sectionShell('asr', editorSection === 'asr', (
                    <div className="wk-kbl-1"><div><h3 className="wk-kbl-2">{t('knowledgeEditor.asr.title')}</h3><p className="wk-kbl-3">{t('knowledgeEditor.asr.description')}</p></div><label className="wk-kbl-12"><WkCheckbox checked={editorConfig.asrConfig.enabled} onChange={(event) => setEditorConfig((current) => ({ ...current, asrConfig: { ...current.asrConfig, enabled: event.target.checked } }))} />{t('knowledgeEditor.asr.label')}</label>{editorConfig.asrConfig.enabled ? <label className="wk-kbl-4">{t('knowledgeEditor.asr.modelLabel')}<WkInput value={editorConfig.asrConfig.modelId} onChange={(event) => setEditorConfig((current) => ({ ...current, asrConfig: { ...current.asrConfig, modelId: event.target.value } }))} placeholder={t('knowledgeEditor.asr.modelPlaceholder')} /></label> : null}</div>
                  )) : null}
                  ...
                  {type !== 'faq' ? sectionShell('advanced', editorSection === 'advanced', (
                    <div className="wk-kbl-1"><div><h3 className="wk-kbl-2">{t('knowledgeEditor.advanced.title')}</h3><p className="wk-kbl-3">{t('knowledgeEditor.advanced.description')}</p></div><label className="wk-kbl-12"><WkCheckbox checked={editorConfig.questionGenerationConfig.enabled} onChange={(event) => setEditorConfig((current) => ({ ...current, questionGenerationConfig: { ...current.questionGenerationConfig, enabled: event.target.checked } }))} />{t('knowledgeEditor.advanced.questionGeneration.label')}</label>{editorConfig.questionGenerationConfig.enabled ? <label className="wk-kbl-4">{t('knowledgeEditor.advanced.questionGeneration.countLabel')}<WkInput type="number" min={1} max={10} value={editorConfig.questionGenerationConfig.questionCount} onChange={(event) => setEditorConfig((current) => ({ ...current, questionGenerationConfig: { ...current.questionGenerationConfig, questionCount: Number(event.target.value) } }))} /></label> : null}<label className="wk-kbl-12"><WkCheckbox checked={editorConfig.autoTagConfig.enabled} onChange={(event) => setEditorConfig((current) => ({ ...current, autoTagConfig: { ...current.autoTagConfig, enabled: event.target.checked } }))} />{t('knowledgeEditor.advanced.autoTag.label')}</label><label className="wk-kbl-4">{t('knowledgeEditor.advanced.tableMetadataInstructions.label')}<WkTextarea rows={3} maxLength={4000} value={editorConfig.chunkingConfig.tableMetadataInstructions} placeholder={t('knowledgeEditor.advanced.tableMetadataInstructions.placeholder')} onChange={(event) => setEditorConfig((current) => ({ ...current, chunkingConfig: { ...current.chunkingConfig, tableMetadataInstructions: event.target.value } }))} /><span className="kb-editor-desc-count wk-kbl-13 wk-kbl-count-end" aria-live="polite" data-table-metadata-count="">{editorConfig.chunkingConfig.tableMetadataInstructions.length}/4000</span></label></div>
                  )) : null}

                  {/* 数据源管理（仅编辑模式，留守段） */}
                  {editingId && editorSection === 'datasource' ? sectionShell('datasource', true, (
                    <div className="wk-kbl-14"><DataSourcesPage client={client} knowledgeBaseId={editingId} canManage={viewer.isAdmin} embedded /></div>
                  )) : null}
