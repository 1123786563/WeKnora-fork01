              ...
              <div className="settings-content">
                <div className="content-wrapper kb-editor-content">
                  ...
                  {type !== 'faq' && editorSection === 'parser' ? sectionShell('parser', true, (
                    <div className="grid gap-4"><h3 className="m-0 text-[20px] font-semibold leading-7 text-ink">{t('kbSettings.parser.title')}</h3><p className="m-0 text-sm leading-[22px] text-muted">{t('kbSettings.parser.description')}</p><label className="grid gap-1">{t('kbSettings.parser.title')}<WkSelect value={editorConfig.chunkingConfig.parserEngineRules[0]?.engine ?? ''} disabled={editorOptions.loading} onChange={(event) => setEditorConfig((current) => ({ ...current, chunkingConfig: { ...current.chunkingConfig, parserEngineRules: event.target.value ? [{ file_types: ['pdf'], engine: event.target.value }] : [] } }))}><option value="">{t('kbSettings.parser.default')}</option>{editorOptions.parserEngines.filter((engine) => engine.Available !== false).map((engine) => <option key={engine.Name} value={engine.Name}>{engine.Name}</option>)}</WkSelect></label><p className="m-0 text-xs leading-[18px] text-muted">{editorOptions.error ?? t('kbSettings.parser.goConfig')}</p></div>
                  )) : null}
