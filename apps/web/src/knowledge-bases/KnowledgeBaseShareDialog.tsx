...
export function KnowledgeBaseShareDialog({ client, knowledgeBaseId, knowledgeBaseName, open, onClose, onChanged, inline = false }: Props) {
  ...
  const shareFormActions = <div className="mt-[0.35rem] flex items-center gap-[0.6rem] border-t border-line-soft pt-4">{shares.length > 0 ? <Button type="button" onClick={() => setShowShareList(true)}>{t('organization.share.sharedTo')} ({shares.length})</Button> : null}<span className="flex-1" aria-hidden="true" /><Button type="button" onClick={onClose}>{t('common.cancel')}</Button>{inline ? <Button type="button" loading={busy} disabled={!organizationId} onClick={() => void share()}>{t('common.confirm')}</Button> : <Button type="submit" loading={busy} disabled={!organizationId}>{t('common.confirm')}</Button>}</div>;
