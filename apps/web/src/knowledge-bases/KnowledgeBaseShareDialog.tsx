...
function OrganizationPicker({ organizations, value, onChange, placeholder, t }: { organizations: Organization[]; value: string; onChange: (value: string) => void; placeholder: string; t: (key: string, vars?: Record<string, string | number>) => string }) {
  ...
  return <div className="wk-share-org-picker" ref={rootRef}><button type="button" className="wk-share-org-picker-trigger" role="combobox" aria-haspopup="listbox" aria-expanded={open} aria-controls="wk-share-org-picker-list" onClick={() => setOpen((current) => !current)} onKeyDown={(event) => { if ((event.key === 'ArrowDown' || event.key === 'ArrowUp') && organizations.length > 0) { event.preventDefault(); setOpen(true); setActiveIndex((current) => (current + (event.key === 'ArrowDown' ? 1 : -1) + organizations.length) % organizations.length); } else if (event.key === 'Enter' && open) { event.preventDefault(); choose(activeIndex); } else if (event.key === 'Escape') setOpen(false); }}>{selected ? <><span className="wk-share-org-avatar" aria-hidden="true">{initials(selected.name)}</span><span>{selected.name}</span></> : <span className="wk-muted">{placeholder}</span>}<span aria-hidden="true">⌄</span></button>{open ? <div id="wk-share-org-picker-list" className="wk-share-org-picker-menu" role="listbox" aria-label={placeholder}>{organizations.map((organization, index) => { const row = organization as Record<string, unknown>; const role = row.is_owner === true ? t('organization.owner') : typeof row.my_role === 'string' ? t(`organization.role.${row.my_role}`) : ''; return <button type="button" role="option" aria-selected={organization.id === value} className={`wk-share-org-option${organization.id === value ? ' is-selected' : ''}${index === activeIndex ? ' is-active' : ''}`} key={organization.id} onMouseEnter={() => setActiveIndex(index)} onClick={() => choose(index)}><span className="wk-share-org-avatar" aria-hidden="true">{initials(organization.name)}</span><span className="wk-share-org-body"><span className="wk-share-org-header"><strong>{organization.name}</strong>{role ? <span className="wk-share-role">{role}</span> : null}</span><span className="wk-share-org-meta"><span><KbIcon name="user" size={13} />{orgValue(organization, 'member_count')}</span><span><KbIcon name="layers" size={13} />{orgValue(organization, 'share_count')}</span><span><KbIcon name="usergroup" size={13} />{orgValue(organization, 'agent_share_count')}</span></span></span></button>; })}</div> : null}</div>;
}

function PermissionRadio({ value, onChange, t }: { value: 'viewer' | 'editor'; onChange: (value: 'viewer' | 'editor') => void; t: (key: string) => string }) {
  return <div className="wk-share-permission-radio" role="radiogroup" aria-label={t('organization.share.permission')}>
    {(['viewer', 'editor'] as const).map((item) => <button key={item} type="button" role="radio" aria-checked={value === item} className={value === item ? 'is-selected' : ''} onClick={() => onChange(item)}>{item === 'viewer' ? t('organization.share.permissionReadonly') : t('organization.share.permissionEditable')}</button>)}
  </div>;
}
