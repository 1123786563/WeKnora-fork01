...
export function KnowledgeBaseShareDialog({ client, knowledgeBaseId, knowledgeBaseName, open, onClose, onChanged }: Props) {
  ...
  return <Dialog open={open} title={`${t('organization.share.title')} · ${knowledgeBaseName}`} onClose={onClose}>
    ...
    {loading ? null : showShareList ? <>
      ...
      <form className="wk-share-form" onSubmit={(event) => void share(event)}><label>{t('organization.share.selectOrg')}<select className="wk-share-org-select" required value={organizationId} onChange={(event) => setOrganizationId(event.target.value)}><option value="">{t('organization.share.selectOrgPlaceholder')}</option>{available.map((org) => { const row = org as Record<string, unknown>; const role = typeof row.my_role === 'string' ? row.my_role : row.is_owner === true ? 'owner' : ''; return <option className="wk-share-org-option" data-role={role} key={org.id} value={org.id}>{org.name} · {orgValue(org, 'member_count')} · {orgValue(org, 'share_count')} · {orgValue(org, 'agent_share_count')}</option>; })}</select><div className="wk-share-org-option-preview" aria-live="polite">{organizationId ? (() => { const org = available.find((item) => item.id === organizationId); if (!org) return null; const row = org as Record<string, unknown>; const role = row.is_owner === true ? t('organization.owner') : typeof row.my_role === 'string' ? t(`organization.role.${row.my_role}`) : ''; return <div className="wk-share-org-option"><span className="wk-share-org-avatar" aria-hidden="true">{initials(org.name)}</span><span className="wk-share-org-body"><span className="wk-share-org-header"><strong>{org.name}</strong>{role ? <span className="wk-share-role">{role}</span> : null}</span><span className="wk-share-org-meta"><span><KbIcon name="user" size={13} />{orgValue(org, 'member_count')}</span><span><KbIcon name="layers" size={13} />{orgValue(org, 'share_count')}</span><span><KbIcon name="usergroup" size={13} />{orgValue(org, 'agent_share_count')}</span></span></span></div>; })() : null}</div></label><label>{t('organization.share.permission')}<select value={permission} onChange={(event) => setPermission(event.target.value as 'viewer' | 'editor')}><option value="viewer">{t('organization.share.permissionReadonly')}</option><option value="editor">{t('organization.share.permissionEditable')}</option></select></label><div className="wk-share-permission-tip"><KbIcon name="info-circle" size={14} /><span>{t('organization.share.permissionTip')}</span></div><div className="wk-share-form-actions"><Button type="button" onClick={onClose}>{t('common.cancel')}</Button><Button type="submit" loading={busy} disabled={!organizationId}>{t('common.confirm')}</Button></div></form>
