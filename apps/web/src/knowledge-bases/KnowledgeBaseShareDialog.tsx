...
export function KnowledgeBaseShareDialog({ client, knowledgeBaseId, knowledgeBaseName, open, onClose, onChanged, inline = false }: Props) {
  ...
  const shareFormActions = <div className="wk-kbs-13">{shares.length > 0 ? <TButton type="button" onClick={() => setShowShareList(true)}>{t('organization.share.sharedTo')} ({shares.length})</TButton> : null}<span className="wk-kbs-14" aria-hidden="true" /><TButton type="button" onClick={onClose}>{t('common.cancel')}</TButton>{inline ? <TButton type="button" loading={busy} disabled={!organizationId} onClick={() => void share()}>{t('common.confirm')}</TButton> : <TButton type="submit" loading={busy} disabled={!organizationId}>{t('common.confirm')}</TButton>}</div>;
