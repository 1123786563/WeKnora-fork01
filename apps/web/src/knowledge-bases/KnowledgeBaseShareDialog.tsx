...
export function KnowledgeBaseShareDialog({ client, knowledgeBaseId, knowledgeBaseName, open, onClose, onChanged, inline = false }: Props) {
  ...
  const shareFormFields = <>
    <label className="grid gap-[0.4rem] font-semibold">{t('organization.share.selectOrg')}<OrganizationPicker organizations={available} value={organizationId} onChange={setOrganizationId} placeholder={t('organization.share.selectOrgPlaceholder')} t={t} />{inline ? null : <select className="absolute box-border h-px w-full min-h-[2.45rem] rounded-control border border-line-control bg-surface px-[0.65rem] py-[0.55rem] text-ink [font:inherit] opacity-0 pointer-events-none" required value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} aria-hidden="true" tabIndex={-1}><option value="">{t('organization.share.selectOrgPlaceholder')}</option>{available.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select>}</label>
    <label className="grid gap-[0.4rem] font-semibold">{t('organization.share.permission')}<PermissionRadio value={permission} onChange={setPermission} t={t} /></label>
    <div className="flex items-start gap-[0.45rem] rounded-control bg-[#f4f6fa] px-[0.75rem] py-[0.7rem] text-[0.8rem] font-normal leading-[1.5] text-muted"><KbIcon name="info-circle" size={14} /><span>{t('organization.share.permissionTip')}</span></div>
  </>;
  const shareFormActions = <div className="mt-[0.35rem] flex items-center gap-[0.6rem] border-t border-line-soft pt-4">{shares.length > 0 ? <TButton type="button" onClick={() => setShowShareList(true)}>{t('organization.share.sharedTo')} ({shares.length})</TButton> : null}<span className="flex-1" aria-hidden="true" /><TButton type="button" onClick={onClose}>{t('common.cancel')}</TButton>{inline ? <TButton type="button" loading={busy} disabled={!organizationId} onClick={() => void share()}>{t('common.confirm')}</TButton> : <TButton type="submit" loading={busy} disabled={!organizationId}>{t('common.confirm')}</TButton>}</div>;
  const shareFormView = inline
    ? <div className="grid gap-[0.9rem] pt-[0.5rem]">{shareFormFields}{shareFormActions}</div>
    : <form className="grid gap-[0.9rem] pt-[0.5rem]" onSubmit={(event) => void share(event)}>{shareFormFields}{shareFormActions}</form>;
  const content = <>
