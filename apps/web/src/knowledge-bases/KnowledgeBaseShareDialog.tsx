...
export function KnowledgeBaseShareDialog({ client, knowledgeBaseId, knowledgeBaseName, open, onClose, onChanged }: Props) {
  ...
  async function unshare(item: Share) { const name = item.organization_name ?? item.organization_id ?? 'organization'; if (busyRef.current || !window.confirm(`Remove "${name}" from this shared space? Members will no longer have access to this knowledge base.`)) return; busyRef.current = true; setBusy(true); setError(null); try { await client.identity.organizations.knowledgeBaseShares.remove(knowledgeBaseId, item.id); setNotice('Sharing removed.'); await load(); onChanged?.(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to remove share'); } finally { busyRef.current = false; setBusy(false); } }
  ...
  return <Dialog open={open} title={`Share ${knowledgeBaseName}`} onClose={onClose}>
    ...
    {loading ? null : showShareList ? <>
      ...
      {shares.length === 0 ? <Status>No shares.</Status> : <ul className="wk-list">{shares.map((item) => <li key={item.id}><span>{item.organization_name ?? item.organization_id}</span><span>{item.permission === 'editor' ? 'Editable' : 'Read-only'}</span><Button type="button" disabled={busy} onClick={() => void unshare(item)}>Remove share</Button></li>)}</ul>}
    </> : <>
      <p className="wk-muted">Choose an organization and permission for this knowledge base.</p>
      <form className="wk-settings-editor" onSubmit={(event) => void share(event)}><label>Select Shared Space<select required value={organizationId} onChange={(event) => setOrganizationId(event.target.value)}><option value="">Select a shared space to share with</option>{available.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select></label><label>Permission<select value={permission} onChange={(event) => setPermission(event.target.value as 'viewer' | 'editor')}><option value="viewer">Read-only</option><option value="editor">Editable</option></select></label><Button type="submit" loading={busy} disabled={!organizationId}>Confirm share</Button></form>
