...
import { ConfigSettingsPanel } from './ConfigSettingsPanel.tsx';
...
export function SettingsPage({ client, tenantId }: { client: WeKnoraClient; tenantId: number }) {
  ...
  const configPanel = selectedKey === 'retrieval'
    ? <ConfigSettingsPanel client={client} section="retrieval" initialValue={payload} />
    : selectedKey === 'chathistory'
      ? <ConfigSettingsPanel client={client} section="chathistory" initialValue={((payload as Record<string, unknown> | null)?.config)} />
      : selectedKey === 'parser'
