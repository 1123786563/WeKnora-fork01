...
export function PlatformApiKeysPanel({ client, initialKeys }: { client: WeKnoraClient; initialKeys: ApiKey[] }) {
  ...
  return <section className="wk-platform-api-keys grid gap-4"><header className="wk-settings-panel-heading flex items-start justify-between gap-4 border-b border-[#eef1f5] pb-4 mb-4 max-[720px]:flex-col"><h2>{title}</h2><p>{copy.description}</p></header><Card className="wk-api-key-create"><strong className="text-sm font-semibold text-[#1f2733]">{copy.createTitle}</strong><div className="wk-api-key-create-row mt-3 flex gap-2"><input aria-label={copy.name} placeholder={copy.name} className="h-8 min-w-0 flex-1 rounded-[4px] border border-[#dcdcdc] bg-white px-2 text-[13px] text-black/90 outline-none focus:border-accent" value={name} onChange={(event) => setName(event.target.value)} disabled={creating} /><button type="button" className="cursor-pointer rounded-[4px] border border-[rgba(120,135,155,0.35)] bg-transparent px-3 py-[5px]" onClick={() => void create()} disabled={creating}>{creating ? copy.creating : copy.create}</button></div><div className="wk-api-key-capabilities mt-3 flex flex-wrap gap-x-4 gap-y-2">{capabilities.map((value) => <label className="text-xs" key={value}><input type="checkbox" checked={selected.includes(value)} onChange={() => toggle(value)} disabled={creating} />{capabilityLabels[value] ?? value}</label>)}</div></Card>{message ? <p className="wk-api-key-message m-0 text-[13px] text-[#c23434]" role="status">{message}</p> : null}{token ? <Card className="wk-api-key-token grid gap-2" role="alert"><strong className="text-sm font-semibold text-[#1f2733]">{copy.created}</strong><code className="bg-[rgba(120,135,155,0.1)] px-2 py-2 [overflow-wrap:anywhere]">{token}</code><div className="flex gap-2"><button type="button" className="cursor-pointer rounded-[4px] border border-[rgba(120,135,155,0.35)] bg-transparent px-3 py-[5px]" onClick={() => void copyToken()}>{copied ? copy.copied : copy.copy}</button><button type="button" className="cursor-pointer rounded-[4px] border border-[rgba(120,135,155,0.35)] bg-transparent px-3 py-[5px]" onClick={() => setToken(null)}>{copy.close}</button></div></Card> : null}<Card className="wk-api-key-list">{keys.length === 0 ? <Status>{copy.empty}</Status> : <div className="wk-api-key-table-wrap overflow-x-auto"><table className="w-full min-w-[680px] border-collapse text-xs [&_th]:border-b [&_th]:border-[rgba(120,135,155,0.18)] [&_th]:px-2 [&_th]:py-3 [&_th]:text-left [&_td]:border-b [&_td]:border-[rgba(120,135,155,0.18)] [&_td]:px-2 [&_td]:py-3 [&_td]:text-left [&_thead_th]:font-medium [&_thead_th]:text-[#5c6b83]"><thead><tr><th>{copy.nameHead}</th><th>{copy.keyHead}</th><th>{copy.permissionsHead}</th><th>{copy.lastUsedHead}</th><th>{copy.createdHead}</th><th>{copy.actionHead}</th></tr></thead><tbody>{keys.map((key) => { const keyCapabilities = key.capabilities ?? []; return <tr key={key.id}><th>{key.name}</th><td><code>{key.api_key}</code></td><td>{keyCapabilities.slice(0, 4).map((capability) => capabilityLabels[capability] ?? capability).join('、')}{keyCapabilities.length > 4 ? ` +${keyCapabilities.length - 4}` : ''}</td><td>{date(key.last_used_at, locale, copy.never)}</td><td>{date(key.created_at, locale, copy.never)}</td><td><button type="button" className="wk-api-key-revoke cursor-pointer rounded-[4px] border border-[rgba(120,135,155,0.35)] bg-transparent px-3 py-[5px]" onClick={() => void revoke(key)}>{copy.revoke}</button></td></tr>; })}</tbody></table></div>}</Card></section>;
}
