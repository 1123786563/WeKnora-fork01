...
export function McpToolsDirectory({ tools, serviceId, approvals, busy, policyError, onRetryPolicies, onPolicyChange }: Props) {
  ...
  return <div className="flex flex-col gap-[.65rem]">
    {tools.length > pageSize ? <label className="grid gap-1"><span className="wk-visually-hidden">{t('mcpMetadata.searchTools')}</span><input className="box-border w-full grid gap-1 border border-[#cbd5e1] rounded-[6px] p-[.45rem]!" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('mcpMetadata.searchTools')} /></label> : null}
    {visible.length ? <ul className="m-0 list-none border-t border-[#edf0f5] p-0">{visible.map((tool) => { const current = policy(tool.name); const isOpen = openTool === tool.name; return <li key={tool.name} className="min-w-0 border-b border-[#edf0f5] py-3.5 last:border-b-0 last:pb-0">
      ...
      {serviceId ? <div className="mt-2.5 flex flex-wrap gap-6 text-[12px] text-[#506078]">
        <label className="items-center cursor-pointer leading-5"><span>{t('mcpMetadata.enabled')}</span><span className="wk-switch [&_input:focus-visible+span]:[outline:2px_solid_#07c05f] [&_input:focus-visible+span]:outline-offset-2 [&_input:disabled+span]:cursor-not-allowed"><input type="checkbox" aria-label={`${tool.name} ${t('mcpMetadata.enabled')}`} disabled={busy} checked={current.enabled} onChange={(event) => onPolicyChange(tool.name, 'enabled', event.target.checked)} /><span className="wk-switch-knob" aria-hidden="true" /></span></label>
        <label className="items-center cursor-pointer leading-5"><span>{t('mcpMetadata.approval')}</span><span className="wk-switch [&_input:focus-visible+span]:[outline:2px_solid_#07c05f] [&_input:focus-visible+span]:outline-offset-2 [&_input:disabled+span]:cursor-not-allowed"><input type="checkbox" aria-label={`${tool.name} ${t('mcpMetadata.approval')}`} disabled={busy} checked={current.requireApproval} onChange={(event) => onPolicyChange(tool.name, 'requireApproval', event.target.checked)} /><span className="wk-switch-knob" aria-hidden="true" /></span></label>
      </div> : null}
