...
export function McpToolsDirectory({ tools, serviceId, approvals, busy, policyError, onRetryPolicies, onPolicyChange }: Props) {
  ...
  if (policyError) return <div className="flex flex-col gap-[.65rem]"><Status tone="error">{policyError}</Status><Button type="button" onClick={onRetryPolicies}>{t('mcpMetadata.retry')}</Button></div>;
  return <div className="flex flex-col gap-[.65rem]">
    {tools.length > pageSize ? <label className="grid gap-1"><span className="wk-visually-hidden sr-only">{t('mcpMetadata.searchTools')}</span><input className="box-border w-full grid gap-1 border border-[#cbd5e1] rounded-[6px] p-[.45rem]!" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('mcpMetadata.searchTools')} /></label> : null}
    {visible.length ? <ul className="m-0 list-none border-t border-[#edf0f5] p-0">{visible.map((tool) => { const current = policy(tool.name); const isOpen = openTool === tool.name; return <li key={tool.name} className="min-w-0 border-b border-[#edf0f5] py-3.5 last:border-b-0 last:pb-0">
      ...
      {serviceId ? <div className="mt-2.5 flex flex-wrap gap-6 text-[12px] text-[#506078]">
        <label className="items-center cursor-pointer leading-5"><span>{t('mcpMetadata.enabled')}</span><span className="wk-switch relative inline-flex shrink-0 cursor-pointer items-center [&_input:focus-visible+span]:[outline:2px_solid_#07c05f] [&_input:focus-visible+span]:outline-offset-2 [&_input:disabled+span]:cursor-not-allowed"><input className="peer absolute m-0 h-[18px] w-[34px] cursor-pointer opacity-0" type="checkbox" aria-label={`${tool.name} ${t('mcpMetadata.enabled')}`} disabled={busy} checked={current.enabled} onChange={(event) => onPolicyChange(tool.name, 'enabled', event.target.checked)} /><span className="wk-switch-knob h-[18px]! w-[34px]! rounded-full bg-[#cbd5e1]! [transition:background_.18s_ease]! after:absolute after:left-[2px] after:top-[2px] after:h-[14px] after:w-[14px] after:rounded-full after:bg-white after:content-[''] peer-checked:bg-primary! peer-checked:after:translate-x-[16px]" aria-hidden="true" /></span></label>
        <label className="items-center cursor-pointer leading-5"><span>{t('mcpMetadata.approval')}</span><span className="wk-switch relative inline-flex shrink-0 cursor-pointer items-center [&_input:focus-visible+span]:[outline:2px_solid_#07c05f] [&_input:focus-visible+span]:outline-offset-2 [&_input:disabled+span]:cursor-not-allowed"><input className="peer absolute m-0 h-[18px] w-[34px] cursor-pointer opacity-0" type="checkbox" aria-label={`${tool.name} ${t('mcpMetadata.approval')}`} disabled={busy} checked={current.requireApproval} onChange={(event) => onPolicyChange(tool.name, 'requireApproval', event.target.checked)} /><span className="wk-switch-knob h-[18px]! w-[34px]! rounded-full bg-[#cbd5e1]! [transition:background_.18s_ease]! after:absolute after:left-[2px] after:top-[2px] after:h-[14px] after:w-[14px] after:rounded-full after:bg-white after:content-[''] peer-checked:bg-primary! peer-checked:after:translate-x-[16px]" aria-hidden="true" /></span></label>
      </div> : null}
