...
export function MemoryWorkspacePanel({ client, initialConfig }: { client: WeKnoraClient; initialConfig: unknown }) {
  ...
  const autoFields = writeMode === 'auto' ? <>
    {setting(t('memoryWorkspaceSettings.extractModelLabel'), t('memoryWorkspaceSettings.extractModelDescription'), <ModelOptionSelect value={extractModelId} options={options(['chat', 'vllm'])} disabled={busy} onChange={(value) => void save({ extractModelId: value })} />)}
    {setting(t('memoryWorkspaceSettings.extractDelayLabel'), t('memoryWorkspaceSettings.extractDelayDescription'), <NumberInput min={5} max={3600} step={15} value={extractDelaySeconds} disabled={busy} onValueChange={(value) => setExtractDelaySeconds(Number(value))} onBlur={() => void save({})} />)}
    {setting(t('memoryWorkspaceSettings.extractMinIntervalLabel'), t('memoryWorkspaceSettings.extractMinIntervalDescription'), <NumberInput min={0} max={86400} step={60} value={extractMinIntervalSeconds} disabled={busy} onValueChange={(value) => setExtractMinIntervalSeconds(Number(value))} onBlur={() => void save({})} />)}
    {setting(t('memoryWorkspaceSettings.interestThresholdLabel'), t('memoryWorkspaceSettings.interestThresholdDescription'), <NumberInput min={1} max={20} step={1} value={interestThreshold} disabled={busy} onValueChange={(value) => setInterestThreshold(Number(value))} onBlur={() => void save({})} />)}
    {setting(t('memoryWorkspaceSettings.instructionsLabel'), t('memoryWorkspaceSettings.instructionsDescription'), <textarea className="h-auto min-h-20 w-full resize-y rounded-[3px] border border-[#dcdcdc] p-2 text-[13px] focus:border-[#07c05f] focus:outline-none focus:ring-2 focus:ring-[#07c05f]/20" maxLength={1000} rows={3} value={extractInstructions} disabled={busy} placeholder={t('memoryWorkspaceSettings.instructionsPlaceholder')} onChange={(event) => setExtractInstructions(event.target.value)} onBlur={() => void save({})} />)}
  </> : null;
  return <Card><div className="wk-settings-panel-heading"><div><h3>{t('memoryWorkspaceSettings.title')}</h3><p className="wk-muted">{t('memoryWorkspaceSettings.description')}</p></div></div><div className="wk-memory-workspace-intro mb-2 flex items-start gap-2.5 rounded-lg bg-[#f3f9f5] px-4 py-3.5 text-[13px] text-black/70"><strong className="shrink-0 font-medium text-black/90">{t('memoryWorkspaceSettings.introTitle')}</strong><span>{t('memoryWorkspaceSettings.introDescription')}</span></div>{error ? <Status tone="error">{error}</Status> : null}{notice ? <Status tone="success">{notice}</Status> : null}<div className="settings-group">{setting(t('memoryWorkspaceSettings.enableLabel'), t('memoryWorkspaceSettings.enableDescription'), <Switch checked={enabled} disabled={busy} onCheckedChange={(checked) => void save({ enabled: checked })} aria-label={t('memoryWorkspaceSettings.enableLabel')} />)}{enabled ? <>{setting(t('memoryWorkspaceSettings.writeModeLabel'), t('memoryWorkspaceSettings.writeModeDescription'), <ModelOptionSelect value={writeMode} options={[{ value: 'explicit_only', label: t('memoryWorkspaceSettings.writeModeExplicit') }, { value: 'auto', label: t('memoryWorkspaceSettings.writeModeAuto') }]} disabled={busy} onChange={(value) => void save({ writeMode: value })} />, writeMode === 'auto' ? t('memoryWorkspaceSettings.writeModeAutoHint') : t('memoryWorkspaceSettings.writeModeExplicitHint'))}{autoFields}{setting(t('memoryWorkspaceSettings.vectorRecallLabel'), t('memoryWorkspaceSettings.vectorRecallDescription'), <Switch checked={vectorRecall} disabled={busy} onCheckedChange={(checked) => void save({ vectorRecall: checked })} aria-label={t('memoryWorkspaceSettings.vectorRecallLabel')} />)}{vectorRecall ? setting(t('memoryWorkspaceSettings.embeddingModelLabel'), t('memoryWorkspaceSettings.embeddingModelDescription'), <ModelOptionSelect value={embeddingModelId} options={options(['embedding'])} disabled={busy} onChange={(value) => void save({ embeddingModelId: value })} />) : null}{setting(t('memoryWorkspaceSettings.conditioningLabel'), t('memoryWorkspaceSettings.conditioningDescription'), <Switch checked={retrievalConditioning} disabled={busy} onCheckedChange={(checked) => void save({ retrievalConditioning: checked })} aria-label={t('memoryWorkspaceSettings.conditioningLabel')} />)}{setting(t('memoryWorkspaceSettings.maxItemsLabel'), t('memoryWorkspaceSettings.maxItemsDescription'), <NumberInput min={10} max={2000} step={10} value={maxItems} disabled={busy} onValueChange={(value) => setMaxItems(Number(value))} onBlur={() => void save({})} />)}</> : null}</div></Card>;
}
