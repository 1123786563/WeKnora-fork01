...
test('tenant members keeps the Vue-style list header and count visible', () => { const html = renderToStaticMarkup(React.createElement(TenantMembersPanel, { client, tenantId: 1, role: 'viewer', initialMembers: { items: [member], total: 1 } })); assert.match(html, /members-list-header/); assert.match(html, /members-list-count-badge/); assert.match(html, />1</); assert.match(html, /Search by name or email/); });
