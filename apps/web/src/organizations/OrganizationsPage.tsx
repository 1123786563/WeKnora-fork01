            ...
            <div className="settings-container">
              ...
              <div className="settings-content">
                ...
                <div className="wk-org-5">
                  <TSelect
                    className="organization-settings-section-selector wk-org-6"
                      ...
                      <form onSubmit={submitBasic}>
                        ...
                        <div className={ORG_FORM_ITEM}>
                          ...
                          <div className="wk-org-7">
                            ...
                            <input id="organization-name" name="organization-name" className={ORG_FIELD + ' wk-org-94'} value={formName} onChange={(event) => setFormName(event.target.value)} required disabled={!settingsCanManage} />
                          </div>
                        </div>
                        <div className={ORG_FORM_ITEM}>
                          ...
                          <div className="wk-org-17">
                            ...
                            <div className="wk-org-30">
                              ...
                              <div className="wk-org-31">
                                ...
                                <TInput type="number" aria-label={t(locale, 'organization.settings.memberLimit')} className={ORG_FIELD + ' wk-org-101'} value={formMemberLimit === '' ? '' : String(formMemberLimit)} placeholder={t(locale, 'organization.settings.memberLimitPlaceholder')} onChange={(value) => { const raw = String(value); if (raw === '') { setFormMemberLimit(''); return; } const next = Math.min(10000, Math.max(0, Math.round(Number(raw)))); if (Number.isSafeInteger(next)) setFormMemberLimit(next); }} />
                    ...
                    <>
                              ...
                              {addMemberPopupOpen ? (
                                <div className="wk-org-49">
                                  ...
                                  <div className="wk-org-52">
                                    ...
                                    <TInput className={ORG_FIELD + ' wk-org-111'} aria-label={t(locale, 'organization.addMember.searchTenant')} value={memberInviteQuery} onChange={(value) => void searchMemberInviteCandidates(String(value))} placeholder={t(locale, 'organization.addMember.searchTenantPlaceholder')} />
                                  ...
                                  <div className="wk-org-52">
                                    ...
                                    <TSelect className={ORG_FIELD + ' wk-org-111'} aria-label={t(locale, 'organization.addMember.selectRole')} value={memberInviteRole} options={roleOptions.map(([value, labelKey]) => ({ value, label: t(locale, labelKey) }))} onChange={(value) => setMemberInviteRole(String(value) as 'admin' | 'editor' | 'viewer')} />
                                  </div>
                                  ...
                                  <tr key={member.id}>
                                    ...
                                    <td className={MEMBER_TD}>
                                      {settingsCanManage && !memberIsOwner ? (
                                        <TSelect className={'org-member-role-sel ' + ORG_FIELD + ' wk-org-118'} aria-label={t(locale, 'organization.members.columns.role')} value={member.role} options={roleOptions.map(([value, labelKey]) => ({ value, label: t(locale, labelKey) }))} onChange={(value) => void updateMemberRole(member, String(value) as 'admin' | 'editor' | 'viewer')} />
                                      ) : (
                      ...
                      {canRequestUpgrade ? <form onSubmit={submitUpgradeRequest} style={{ marginTop: '24px', borderTop: '1px dashed #e7e7ea', paddingTop: '16px' }}>
                        ...
                        <div className={ORG_FORM_ITEM}>
                          ...
                          <TSelect className={ORG_FIELD + ' wk-org-6'} value={upgradeRole} options={upgradeChoices.map((value) => ({ value, label: t(locale, 'organization.role.' + value) }))} onChange={(value) => setUpgradeRole(String(value) as 'admin' | 'editor' | 'viewer')} />
                        </div>
                        <div className={ORG_FORM_ITEM}>
                          <label className={ORG_FORM_LABEL} htmlFor="upgrade-note">{t(locale, 'organization.upgrade.reason')}</label>
                          <TTextarea id="upgrade-note" className={ORG_FIELD + ' wk-org-95'} rows={2} maxLength={500} value={upgradeNote} onChange={(value) => setUpgradeNote(clampApplicationNote(String(value)))} placeholder={t(locale, 'organization.upgrade.reasonPlaceholder')} />
                        </div>
                        ...
                        <>
                          ...
                          <div className="wk-org-78">
                            <div className={ORG_FORM_ITEM} style={{ marginBottom: '0' }}>
                              ...
                              <TSelect className={ORG_FIELD + ' wk-org-6'} aria-label={t(locale, 'organization.invite.requestRole')} value={requestRole} options={roleOptions.map(([value, labelKey]) => ({ value, label: t(locale, labelKey) }))} onChange={(value) => setRequestRole(String(value) as 'admin' | 'editor' | 'viewer')} />
                            </div>
                            <div className={ORG_FORM_ITEM} style={{ marginBottom: '0' }}>
                              <label className={ORG_FORM_LABEL} htmlFor="join-request-note">{t(locale, 'organization.invite.applicationNote')}</label>
                              <TTextarea id="join-request-note" className={ORG_FIELD + ' wk-org-95'} rows={2} maxLength={500} value={requestNote} onChange={(value) => setRequestNote(clampApplicationNote(String(value)))} placeholder={t(locale, 'organization.invite.messagePlaceholder')} />
                            </div>
                          </div>
                        </>
                      ) : (
