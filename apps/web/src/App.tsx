            ...
            <div className="kb-editor-content min-w-0">
              {editorSection === 'basic' ? <div className="grid gap-4">
                ...
                {type === 'document' ? <fieldset data-guide="kb-create-indexing" className="m-0 grid min-w-0 gap-2 border-0 p-0"><legend className="p-0 text-[15px] font-medium text-ink">{t('knowledgeEditor.indexing.title')}<span className="ml-1 text-[#e34d59]">*</span></legend><p className="m-0 text-xs leading-[18px] text-muted">{t('knowledgeEditor.indexing.description')}</p><div className={`grid gap-3 min-[720px]:grid-cols-2${Boolean(editingId) && editingHasFiles ? ' opacity-60' : ''}`}>
                  ...
                  <label className={`grid gap-[6px] rounded-[8px] border py-3 px-[14px] transition-colors ${indexingStrategy.vector_enabled ? 'border-[var(--color-brand)] bg-[color-mix(in_srgb,var(--color-brand)_8%,transparent)]' : 'border-line-soft bg-surface'}`}><span className="flex items-start gap-2"><Checkbox checked={indexingStrategy.vector_enabled} disabled={Boolean(editingId) && editingHasFiles} onChange={(event) => { if (Boolean(editingId) && editingHasFiles) return; setIndexingStrategy((current) => ({ ...current, vector_enabled: event.target.checked, keyword_enabled: event.target.checked })); }} /><span className="text-[14px] leading-[22px] text-ink">{t('knowledgeEditor.indexing.searchTitle')}</span></span><small className="pl-6 text-xs leading-[18px] text-muted">{t('knowledgeEditor.indexing.searchDesc')}</small></label>
                  <label className={`grid gap-[6px] rounded-[8px] border py-3 px-[14px] transition-colors ${indexingStrategy.wiki_enabled ? 'border-[var(--color-brand)] bg-[color-mix(in_srgb,var(--color-brand)_8%,transparent)]' : 'border-line-soft bg-surface'}`}><span className="flex items-start gap-2"><Checkbox checked={indexingStrategy.wiki_enabled} disabled={Boolean(editingId) && editingHasFiles} onChange={(event) => { if (Boolean(editingId) && editingHasFiles) return; setIndexingStrategy((current) => ({ ...current, wiki_enabled: event.target.checked })); }} />{/* Vue KnowledgeBaseEditorModal.vue:106-109 indexing-check-title + :1972-1984 indexing-new-badge */}
                    <span className="inline-flex items-center gap-[6px] text-[14px] leading-[22px] text-ink">{t('knowledgeEditor.indexing.wikiTitle')}<span className="kb-editor-new-badge inline-flex h-4 items-center rounded-[3px] bg-[var(--color-brand-light)] px-[6px] text-[10px] font-semibold leading-none tracking-[0.4px] text-accent">NEW</span></span></span><small className="pl-6 text-xs leading-[18px] text-muted">{t('knowledgeEditor.indexing.wikiDesc')}</small></label>
                ...
                <label className="grid gap-1"><span className="text-[15px] font-medium text-ink">{t('knowledgeEditor.basic.nameLabel')}<span className="ml-1 text-[#e34d59]">*</span></span> {/* Vue t-input :165-169 is maxlength-only; blank names are blocked by the JS save pipeline */}<Input data-guide="kb-create-name" value={name} onChange={(event) => setName(event.target.value)} placeholder={t('knowledgeEditor.basic.namePlaceholder')} maxLength={50} className="rounded-control border border-line-strong p-[0.55rem]" /></label>
                <label className="grid gap-1"><span className="text-[15px] font-medium text-ink">{t('knowledgeEditor.basic.descriptionLabel')}</span> <Textarea value={description} maxLength={200} onChange={(event) => setDescription(event.target.value)} placeholder={t('knowledgeEditor.basic.descriptionPlaceholder')} rows={3} />{/* Vue KnowledgeBaseEditorModal.vue:171-179 t-textarea maxlength=200 → .t-textarea__limit counter (12px/20px, placeholder gray, right-aligned) */}
