...
export function LoginPage({ client, onAuthenticated, apiBaseUrl, initialError, initialMode = 'login', inviteToken = '', onInviteAccepted }: LoginPageProps) {
  ...
  return <div className="[--auth-brand:#07C05F] [--auth-text-anti:#ffffff] [--auth-font:'PingFang_SC','Helvetica_Neue','Helvetica','Arial','Microsoft_YaHei',sans-serif] relative flex w-full min-h-screen overflow-hidden bg-[linear-gradient(225deg,#022c22_0%,#064e3b_15%,#065f46_25%,#047857_38%,#059669_50%,#07C05F_65%,#10B981_78%,#34D399_90%,#6EE7B7_100%)] [font-family:var(--auth-font)] before:pointer-events-none before:absolute before:inset-0 before:content-[''] before:bg-[radial-gradient(circle_at_20%_50%,rgba(255,255,255,0.06)_0%,transparent_50%),radial-gradient(circle_at_80%_50%,rgba(255,255,255,0.04)_0%,transparent_50%)]">
    ...
    <div className="relative box-border flex flex-[0_0_48%] items-end justify-center pt-[112px] pr-[50px] pb-[100px] pl-[30px] max-[1024px]:flex-[0_0_100%] max-[1024px]:pt-2.5 max-[1024px]:px-6 max-[1024px]:pb-[60px] max-[768px]:px-4 max-[768px]:pb-12">
      <div className="relative z-[2] mb-[60px] w-full max-w-[480px]">
        ...
        {!isRegister ? <div className="box-border w-full rounded-2xl border-0 bg-[rgba(255,255,255,0.97)] p-10 shadow-[0_10px_40px_rgba(0,0,0,0.15)] max-[768px]:p-5">
          <div className="mb-8 text-center">
            <h2 className="m-0 mb-1.5 text-2xl font-semibold text-[#1a1a1a]">{t('auth.login')}</h2>
            <p className="m-0 text-[13px] leading-[1.5] text-[#555]">{t('auth.subtitle')}</p>
            {registrationEnabled ? <p className="rounded-md bg-[#e9fbf0] px-3 py-2 mt-2 mb-0 text-[13px] text-[#07C05F]">{t('auth.loginHint')}</p> : null}
          </div>
          <form className="flex flex-col gap-[18px]" onSubmit={submit} aria-label="Login form">
            <label className="flex flex-col gap-1.5">
              ...
              <div className="auth-input-shell flex h-10 w-full items-center rounded-lg border border-[#dcdcdc] bg-white px-3 transition-colors focus-within:border-(--auth-brand) focus-within:shadow-[0_0_0_3px_rgba(7,192,95,0.1)]">
                <Input id="auth-email" aria-invalid={hasFieldError('email')} aria-describedby={hasFieldError('email') ? fieldErrorId('email') : undefined} className="box-border h-6 w-full rounded-none border-0 bg-transparent px-0 text-[15px] leading-6 text-[#1a1a1a] outline-none [font-family:var(--auth-font)] disabled:cursor-not-allowed disabled:bg-transparent" value={email} onChange={(event) => setEmail(event.target.value)} type="text" autoComplete="email" disabled={loading} placeholder={t('auth.emailPlaceholder')} />
              </div>
        ...
        {isRegister && (registrationEnabled || invite) ? <div className="box-border w-full rounded-2xl border-0 bg-[rgba(255,255,255,0.97)] p-10 shadow-[0_10px_40px_rgba(0,0,0,0.15)] max-[768px]:p-5">
          ...
          <form className="flex flex-col gap-[18px]" onSubmit={submit} aria-label="Register form">
            <label className="flex flex-col gap-1.5">
              ...
              <div className="auth-input-shell flex h-10 w-full items-center rounded-lg border border-[#dcdcdc] bg-white px-3 transition-colors focus-within:border-(--auth-brand) focus-within:shadow-[0_0_0_3px_rgba(7,192,95,0.1)]"><Input id="auth-username" aria-invalid={hasFieldError('username')} aria-describedby={hasFieldError('username') ? fieldErrorId('username') : undefined} className="box-border h-6 w-full rounded-none border-0 bg-transparent px-0 text-[15px] leading-6 text-[#1a1a1a] outline-none [font-family:var(--auth-font)] disabled:cursor-not-allowed disabled:bg-transparent" value={username} onChange={(event) => setUsername(event.target.value)} disabled={loading} placeholder={t('auth.usernamePlaceholder')} /></div>
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <div className="auth-input-shell flex h-10 w-full items-center rounded-lg border border-[#dcdcdc] bg-white px-3 transition-colors focus-within:border-(--auth-brand) focus-within:shadow-[0_0_0_3px_rgba(7,192,95,0.1)]"><Input id="auth-register-email" aria-invalid={hasFieldError('email')} aria-describedby={hasFieldError('email') ? fieldErrorId('email') : undefined} className="box-border h-6 w-full rounded-none border-0 bg-transparent px-0 text-[15px] leading-6 text-[#1a1a1a] outline-none [font-family:var(--auth-font)] disabled:cursor-not-allowed disabled:bg-transparent" value={email} onChange={(event) => setEmail(event.target.value)} type="text" autoComplete="email" disabled={loading} placeholder={t('auth.emailPlaceholder')} /></div>
              {hasFieldError('email') ? <span id={fieldErrorId('email')} role="alert">{fieldError('email')}</span> : null}
            </label>
            <label className="flex flex-col gap-1.5">
              <span className="text-sm font-medium text-[#1a1a1a]"><span style={{ color: '#d54941', marginRight: 4 }}>*</span>{t('auth.password')}</span>
              <div className="auth-input-shell flex h-10 w-full items-center rounded-lg border border-[#dcdcdc] bg-white px-3 transition-colors focus-within:border-(--auth-brand) focus-within:shadow-[0_0_0_3px_rgba(7,192,95,0.1)]"><Input className="box-border h-6 w-full rounded-none border-0 bg-transparent px-0 text-[15px] leading-6 text-[#1a1a1a] outline-none [font-family:var(--auth-font)] disabled:cursor-not-allowed disabled:bg-transparent" value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="new-password" disabled={loading} placeholder={t('auth.passwordPlaceholder')} /></div>
              {fieldError('password')}
            </label>
            <label className="flex flex-col gap-1.5">
              <span className="text-sm font-medium text-[#1a1a1a]"><span style={{ color: '#d54941', marginRight: 4 }}>*</span>{t('auth.confirmPassword')}</span>
              <div className="auth-input-shell flex h-10 w-full items-center rounded-lg border border-[#dcdcdc] bg-white px-3 transition-colors focus-within:border-(--auth-brand) focus-within:shadow-[0_0_0_3px_rgba(7,192,95,0.1)]"><Input className="box-border h-6 w-full rounded-none border-0 bg-transparent px-0 text-[15px] leading-6 text-[#1a1a1a] outline-none [font-family:var(--auth-font)] disabled:cursor-not-allowed disabled:bg-transparent" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} type="password" autoComplete="new-password" disabled={loading} placeholder={t('auth.confirmPasswordPlaceholder')} /></div>
