...
const messages: Record<string, string> = {
  ...
  'auth.loginError': '登录失败，请检查邮箱或密码', 'auth.registerSuccess': '注册成功，请登录', 'auth.registerFailed': '注册失败', 'auth.firstTime': '首次使用？', 'auth.backToLogin': '返回登录', 'auth.loginHint': '登录以继续使用；首次使用请在下方创建账户。', 'auth.oidcLogin': '使用单点登录', 'auth.oidcFailed': '单点登录失败',
