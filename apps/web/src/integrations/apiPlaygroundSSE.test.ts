...
import { consumeApiPlaygroundSSE, type ApiPlaygroundSSEProgress } from './apiPlaygroundSSE.ts';

// Ported from the Vue baseline frontend/src/views/integrations/apiPlaygroundSSE.test.ts
// (same fixtures, same terminal-state semantics). The consumer sits on the shared
// api-client SSE parser (packages/api-client/src/chat/stream.ts) like skill-install.
...
test('emits incremental progress with raw transcript and accumulated answer', async () => {
  const updates: ApiPlaygroundSSEProgress[] = [];
  const result = await consumeApiPlaygroundSSE(streamFromChunks([
    'data: {"response_type":"answer","content":"Hel","done":false}\n\n',
    'data: {"response_type":"answer","content":"lo","done":false}\n\n',
    'data: {"response_type":"complete","content":"","done":true}\n\n',
  ]).stream, (progress) => updates.push({ ...progress }));

  assert.equal(result.status, 'success');
  assert.equal(result.answer, 'Hello');
  assert.ok(updates.length >= 2, 'progress emitted per chunk');
  assert.equal(updates[updates.length - 1]!.answer, 'Hello');
  assert.match(updates[updates.length - 1]!.raw, /"complete"/);
});
