...
import { Alert, Button, Card, Input, Sheet, Status } from '@weknora/ui';
...
function ChannelCards({ tab, snapshot, agentId, canEdit, onOpen }: { tab: 'im' | 'embed'; snapshot: IntegrationSnapshot; agentId: string | null; canEdit: boolean; onOpen(channel: IntegrationChannel): void }) {
  ...
  return <section aria-label={`${integrationLabels[tab]} channels`}>
    <div className="wk-integration-toolbar"><span>{channels.length} channels</span>{canEdit ? <Button type="button" size="small" onClick={() => onOpen({ id: '', name: '', enabled: true, kind: tab })}>Add channel</Button> : null}</div>
...
export function IntegrationsPage({ route, onNavigate, canView = true, canEdit = false, load, initialData = emptySnapshot, onSave, runPlaygroundRequest }: IntegrationsPageProps) {
  ...
  return <main className="wk-page" aria-label="Integrations settings">
    ...
    <Card>
      ...
      {!canView ? <Alert tone="error">You do not have permission to view integration settings.</Alert> : loadState.status === 'loading' ? <Status>Loading integration settings…</Status> : loadState.status === 'error' ? <Alert tone="error">{loadState.message}<Button type="button" size="small" onClick={refresh}>Retry</Button></Alert> : <section aria-labelledby="integration-title"><p className="wk-eyebrow">{integrationLabels[route.tab]}</p><h2 id="integration-title">{active.title}</h2><p>{active.description}</p>{route.agentId ? <p className="wk-muted">Agent filter: {route.agentId}</p> : null}{route.tab === 'im' || route.tab === 'embed' ? <ChannelCards tab={route.tab} snapshot={snapshot} agentId={route.agentId} canEdit={canEdit} onOpen={(channel) => { trigger.current = document.activeElement as HTMLButtonElement; openDrawer(channel); }} /> : route.tab === 'api' ? <section aria-label="API playground"><Status>API base URL: {snapshot.apiBaseUrl || 'Not configured'}</Status>{snapshot.apiKeys?.length ? <ul aria-label="API keys">{snapshot.apiKeys.map((key) => <li key={key.id}>{key.name}: <code>{key.masked}</code></li>)}</ul> : <Status>No API keys configured.</Status>}<Button type="button" onClick={runPlayground} loading={playgroundState === 'loading'}>Run API playground</Button>{playgroundState === 'error' ? <Status tone="error">API playground request failed. Retry.</Status> : playgroundState === 'ready' ? <Status tone="success">API playground request completed.</Status> : null}</section> : <Status>Integration configuration remains on the existing backend contract.</Status>}</section>}
    </Card>
    <Sheet open={drawer !== null} title={drawer?.id ? `Edit ${drawer.name || 'channel'}` : 'Add channel'} onOpenChange={(open) => { if (!open) setDrawer(null); }}>
      {drawer ? <><DrawerTabs active={drawerTab} onChange={setDrawerTab} /><div id="integration-drawer-panel" role="tabpanel" aria-labelledby={`integration-drawer-tab-${drawerTab}`}><label>Channel name<Input value={drawer.name} disabled={!canEdit || saveState === 'saving'} onChange={(event) => setDrawer({ ...drawer, name: event.target.value })} /></label>{drawerTab === 'playground' ? <p>Use this panel to verify the configured API request.</p> : drawerTab === 'embed' ? <pre>{`<iframe src="/embed/${drawer.id || 'new'}" title="WeKnora" />`}</pre> : <p>{canEdit ? 'Changes are editable by administrators.' : 'Read-only access.'}</p>}{saveState === 'error' ? <Status tone="error">Unable to save this channel. Retry.</Status> : null}</div><div className="wk-drawer-actions"><Button type="button" variant="text" onClick={() => setDrawer(null)}>Cancel</Button>{canEdit ? <Button type="button" onClick={save} loading={saveState === 'saving'}>Save</Button> : null}</div></> : null}
    </Sheet>
  </main>;
}
