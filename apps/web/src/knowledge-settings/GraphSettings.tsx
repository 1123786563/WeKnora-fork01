...
// Alert tone→theme、正文进 message prop；Input onChange 改 (value) 签名；
// variant="text" 直译、theme="primary" → theme="primary"）；Status 无
// TDesign 对应，走 shared/wk-legacy（.wk-status 族，渲染不变）。布局不动。
import { Alert as TAlert, Button as TButton, Input as TInput } from 'tdesign-react';
...
export function GraphSettings({ graphExtract, modelId, client, embedded = false, canRunGraphExtract = false, graphDatabaseEngine, onChange, onOpenGraphGuide }: GraphSettingsProps) {
  ...
  return <div className={embedded ? 'wk-graph-settings wk-graph-settings-embedded' : 'wk-graph-settings'}>
    ...
    {system.status === 'ready' && !enabled ? <TAlert theme="warning" message={<><div>知识图谱数据库未启用，实体关系提取功能将无法使用</div>{!embedded && onOpenGraphGuide ? <TButton type="button" variant="text" onClick={onOpenGraphGuide}>如何启用知识图谱？</TButton> : null}</>} /> : null}
    {enabled ? <div className="wk-graph-form">
      ...
      {local.enabled ? <>
        ...
        <div className="wk-graph-field"><strong>示例文本</strong><span className="wk-muted">使用示例文本预览提取的图谱结构。</span><div className="wk-graph-inline wk-graph-stack">{shouldRenderGraphActions(canRunGraphExtract) ? <TButton type="button" loading={actions.text.status === 'loading'} disabled={!modelId} onClick={() => void action('text')}>生成随机文本</TButton> : null}<textarea rows={7} maxLength={5000} value={local.text} onChange={(event) => updateField('text', event.target.value)} /></div><ActionStatus state={actions.text} /></div>
        {local.nodes.length > 0 ? <div className="wk-graph-field"><strong>实体列表</strong><span className="wk-muted">管理实体名称和属性。</span>{local.nodes.map((node, index) => <div className="wk-graph-node" key={index}><div className="wk-graph-inline"><TInput aria-label={`Entity ${index + 1}`} placeholder="Entity name" value={node.name} onChange={(value) => update({ ...local, nodes: local.nodes.map((item, i) => i === index ? { ...item, name: String(value) } : item) })} /><TButton type="button" variant="text" onClick={() => update({ ...local, nodes: local.nodes.filter((_, i) => i !== index) })}>删除</TButton></div>{node.attributes.map((attribute, attributeIndex) => <div className="wk-graph-inline" key={attributeIndex}><TInput aria-label={`Entity ${index + 1} attribute ${attributeIndex + 1}`} value={attribute} onChange={(text) => update({ ...local, nodes: local.nodes.map((item, i) => i === index ? { ...item, attributes: item.attributes.map((value, ai) => ai === attributeIndex ? String(text) : value) } : item) })} /><TButton type="button" variant="text" onClick={() => update({ ...local, nodes: local.nodes.map((item, i) => i === index ? { ...item, attributes: item.attributes.filter((_, ai) => ai !== attributeIndex) } : item) })}>×</TButton></div>)}<TButton type="button" variant="text" onClick={() => update({ ...local, nodes: local.nodes.map((item, i) => i === index ? { ...item, attributes: [...item.attributes, ''] } : item) })}>添加属性</TButton></div>)}<TButton type="button" theme="primary" onClick={addNode}>添加实体</TButton></div> : null}
        {local.relations.length > 0 ? <div className="wk-graph-field"><strong>关系列表</strong><span className="wk-muted">使用关系类型连接实体。</span>{local.relations.map((relation, index) => <div className="wk-graph-inline" key={index}><select aria-label={`Relation ${index + 1} source`} value={relation.node1} onChange={(event) => update({ ...local, relations: local.relations.map((item, i) => i === index ? { ...item, node1: event.target.value } : item) })}><option value="">Select entity</option>{local.nodes.map((node) => <option key={node.name} value={node.name}>{node.name}</option>)}</select><span aria-hidden="true">→</span><TInput aria-label={`Relation ${index + 1} type`} value={relation.type} placeholder="Relation type" onChange={(value) => update({ ...local, relations: local.relations.map((item, i) => i === index ? { ...item, type: String(value) } : item) })} /><span aria-hidden="true">→</span><select aria-label={`Relation ${index + 1} target`} value={relation.node2} onChange={(event) => update({ ...local, relations: local.relations.map((item, i) => i === index ? { ...item, node2: event.target.value } : item) })}><option value="">Select entity</option>{local.nodes.map((node) => <option key={node.name} value={node.name}>{node.name}</option>)}</select><TButton type="button" variant="text" onClick={() => update({ ...local, relations: local.relations.filter((_, i) => i !== index) })}>删除</TButton></div>)}<TButton type="button" theme="primary" onClick={addRelation}>添加关系</TButton></div> : null}
        <div className="wk-graph-field"><strong>提取操作</strong><span className="wk-muted">保存配置前，可使用示例文本运行提取。</span><div className="wk-graph-inline">{shouldRenderGraphActions(canRunGraphExtract) ? <TButton type="button" theme="primary" loading={actions.relations.status === 'loading'} disabled={!modelId || !local.text.trim()} onClick={() => void action('relations')}>开始提取</TButton> : null}<TButton type="button" onClick={() => update(graphExample())}>默认示例</TButton><TButton type="button" onClick={() => update({ ...local, text: '', tags: [], nodes: [], relations: [] })}>清空示例</TButton></div><ActionStatus state={actions.relations} /></div>
      </> : null}
    </div> : null}
  </div>;
}
