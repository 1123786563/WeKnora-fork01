  ...
  return <div className={embedded ? 'wk-graph-settings wk-graph-settings-embedded' : 'wk-graph-settings'}>
    ...
    {enabled ? <div className="wk-graph-form">
      ...
      {local.enabled ? <>
        ...
        <div className="wk-graph-field"><strong>Extraction actions</strong><span className="wk-muted">Run extraction against the sample text before saving the configuration.</span><div className="wk-graph-inline">{shouldRenderGraphActions(canRunGraphExtract) ? <Button type="button" variant="primary" loading={actions.relations.status === 'loading'} disabled={!modelId || !local.text.trim()} onClick={() => void action('relations')}>Start extraction</Button> : null}<Button type="button" onClick={() => update(graphExample())}>Default example</Button><Button type="button" onClick={() => update({ ...local, text: '', tags: [], nodes: [], relations: [] })}>Clear example</Button></div><ActionStatus state={actions.relations} /></div>
      </> : null}
    </div> : null}
  </div>;
}
