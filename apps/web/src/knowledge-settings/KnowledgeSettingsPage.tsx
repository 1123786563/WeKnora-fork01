...
export function KnowledgeSettingsPage({ client, knowledgeBaseId }: { client: WeKnoraClient; knowledgeBaseId: string }) {
  ...
  return <main className="wk-page max-w-[1180px]! mx-auto box-border px-[1.25rem] py-12">
    ...
    <header className="wk-header mb-6 flex items-start justify-between gap-4"><div><p className="wk-eyebrow m-0 text-[0.78rem] font-bold uppercase tracking-[0.08em] text-primary">Knowledge base · {knowledgeBaseId}</p><h1>{t('knowledgeBase.settings.title')}</h1><p className="wk-muted text-muted">{t('knowledgeEditor.chunking.description')}</p></div><Button type="button" onClick={() => void load()} disabled={loading}>{t('knowledgeEditor.activity.refresh')}</Button></header>
