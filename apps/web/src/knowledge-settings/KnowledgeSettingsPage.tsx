...
export function KnowledgeSettingsPage({ client, knowledgeBaseId }: { client: WeKnoraClient; knowledgeBaseId: string }) {
  ...
  return <main className="wk-page max-w-[1180px]!">
    ...
    {loading || !form || !knowledgeBase ? <Card><Status>{t('common.loading')}</Status></Card> : <form onSubmit={save}>
      ...
      <Card className="wk-settings-section"><h2>{t('knowledgeEditor.indexing.title')}</h2><p className="wk-muted">{t('knowledgeEditor.indexing.description')}</p><div className="wk-toggle-grid grid grid-cols-2 gap-[0.25rem_1rem] max-[720px]:grid-cols-1">{([['vector_enabled', t('knowledgeEditor.indexing.searchTitle')], ['keyword_enabled', t('knowledgeEditor.indexing.searchTitle')], ['wiki_enabled', t('knowledgeEditor.indexing.wikiTitle')], ['graph_enabled', t('knowledgeEditor.indexing.graphTitle')]] as const).map(([key, label]) => <label key={key} className="wk-checkbox flex items-center gap-2 mt-[0.8rem] font-medium max-[720px]:mt-0 [&_input]:w-auto"><input type="checkbox" checked={form.indexing[key]} onChange={(event) => updateIndexing(key, event.target.checked)} /> {label}</label>)}</div><div className="wk-form-grid wk-form-grid--two"><label>{t('knowledgeEditor.models.embeddingLabel')}<input value={form.embeddingModelId} readOnly aria-describedby="model-binding-note" /></label><label>{t('knowledgeEditor.models.llmLabel')}<input value={form.summaryModelId} readOnly aria-describedby="model-binding-note" /></label></div><p id="model-binding-note" className="wk-muted">{t('knowledgeEditor.models.description')}</p></Card>
