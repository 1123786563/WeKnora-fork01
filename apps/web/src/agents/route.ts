export type AgentEditorSection = 'basic' | 'model' | 'prompts' | 'tools' | 'knowledge' | 'suggestions' | 'personalization' | 'subagents' | 'integration-im' | 'integration-embed';
...
export function parseAgentRoute(path: string): AgentRoute | null {
  ...
  return { editId: url.searchParams.get('edit'), section: aliases[raw] ?? (['basic', 'model', 'prompts', 'tools', 'knowledge', 'suggestions', 'personalization', 'subagents', 'integration-im', 'integration-embed'].includes(raw) ? raw as AgentEditorSection : 'basic'), highlight: url.searchParams.get('highlight'), sourceTenantId: url.searchParams.get('sourceTenantId') };
}
