...
export function WikiPage({ client, knowledgeBaseId, initialSlug }: { client: WeKnoraClient; knowledgeBaseId: string; initialSlug?: string }) {
  ...
  function choose(page: WikiPageModel) { setSelected(page); setTitle(page.title); setSlug(page.slug); setSummary(page.summary); setContent(page.content); setVersion(page.version); setSaveState(null); setEditing(false); }
  function newPage() { setSelected(null); setTitle(''); setSlug(''); setSummary(''); setContent(''); setVersion(1); setSaveState(null); setEditing(true); }
