...
final class DirectConnectionEditorForm extends ChangeNotifier {
  ...
  void selectProviderPreset(
    ...
    String weknoraDefaultName = 'WeKnora',
  }) {
    if (providerPreset == value) return;
    _providerPreset = value;
    _adapterKey = switch (value) {
      kOllamaAdapterKey => kOllamaAdapterKey,
      kWeKnoraAdapterKey => kWeKnoraAdapterKey,
      _ => kOpenAiCompatibleAdapterKey,
    };
    // WeKnora workspace API keys are sent as X-API-Key by the adapter, so the
    // API-key-header mode is the natural default (Bearer remains available for
    // JWT tokens).
    _authentication = value == kWeKnoraAdapterKey
    ...
    _updateTextFields(() {
      baseUrl.text = switch (value) {
        ...
        kWeKnoraAdapterKey => kWeKnoraDefaultBaseUrl,
