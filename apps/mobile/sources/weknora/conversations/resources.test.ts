...
test('queries every product resource and rejects cross tenant resources', async () => {
  ...
  const rows = {
    s1: { id: 's1', owner_tenant_id: 't1', owner_id: 'u1', members: ['u1'] },
...
test('fails closed when any ownership relationship is omitted', async () => {
  const identity = { origin: 'https://api.example', userId: 'u1', tenantId: 't1' };
  const selection = { spaceId: 's1', agentId: 'a1', targetId: 't1', workspaceRef: 'w1' };
  const complete = {
    s1: { id: 's1', owner_tenant_id: 't1', owner_id: 'u1', members: ['u1'] },
  ...
  for (const key of ['members', 'owner_id', 'space_id', 'workspace_ref', 'target_id']) {
    const rows = structuredClone(complete) as Record<string, Record<string, unknown>>;
    if (key === 'members') delete rows.s1.members;
    else if (key === 'owner_id') delete rows.a1.owner_id;
    else if (key === 'space_id') delete rows.t1.space_id;
    else if (key === 'workspace_ref') delete rows.t1.workspace_ref;
    else delete rows.w1.target_id;
    await assert.rejects(() => resolveProductSessionResources(session(rows), selection, identity, new AbortController().signal));
  }
});
