...
const base = () => ({ hydrating: false, credential: { kind: 'bearer', accessToken: 'test' }, tenantId: null, canCreateTenant: false, refreshWorkspaces: async () => {}, logout: async () => {}, client: { identity: { tenants: { invitations: { pendingCount: async () => ({pendingCount: 0}), listMine: async () => ({items: []}) } } } } });
