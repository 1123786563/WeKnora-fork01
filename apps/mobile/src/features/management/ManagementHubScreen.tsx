...
import { MOBILE_CAPABILITIES, projectMobileCapability, type MobileCapability } from './capabilities.ts';
...
export function ManagementHubScreen() {
  ...
  function open(capability: MobileCapability) {
    ...
    else if (capability.key === 'api-keys') router.push('/management/api-keys');
    else if (capability.key === 'organizations') router.push('/management/organizations');
  }
  return <SafeAreaView style={{ flex: 1, padding: 16 }}>
    ...
    <FlatList data={capabilities} keyExtractor={(item) => item.key} renderItem={({ item }) => <Pressable disabled={item.support === 'unsupported' || (!['configuration', 'identity', 'api-keys', 'organizations'].includes(item.key))} onPress={() => open(item)} style={{ borderBottomColor: '#eaecf0', borderBottomWidth: 1, paddingVertical: 12, opacity: item.support === 'unsupported' ? 0.55 : 1 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between' }}><Text style={{ fontWeight: '600' }}>{item.label}</Text><Text style={{ color: item.support === 'unsupported' ? '#b42318' : '#2864dc' }}>{item.support}</Text></View>{item.reason ? <Text style={{ color: '#667085', fontSize: 12, marginTop: 4 }}>{item.reason}</Text> : null}</Pressable>} />
  </SafeAreaView>;
}
