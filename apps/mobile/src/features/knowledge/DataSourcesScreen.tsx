...
export function DataSourcesScreen() {
  ...
  function updateDraft<K extends keyof NativeDataSourceDraft>(key: K, value: NativeDataSourceDraft[K]) { setDraft((current) => ({ ...current, [key]: value })); if (key === 'type') setGitlabProjects(value === 'gitlab' && gitlabProjects.length === 0 ? [{ projectId: '', ref: '', paths: '' }] : value === 'gitlab' ? gitlabProjects : []); setTestResult(null); }
  function openCreate() { const type = types[0]?.type ?? ''; setEditing(null); setTemporarySourceId(null); setResources([]); setSelectedResourceIds([]); setExpandedResourceIds([]); setDriveFolderToken(''); setGitlabProjects(type === 'gitlab' ? [{ projectId: '', ref: '', paths: '' }] : []); setRssFeedUrls(''); setRssAuthHeaders(''); setDraft({ ...EMPTY_DRAFT, type }); setError(''); }
  ...
  function openEdit(source: DataSource) { const ids = sourceResourceIds(source); const config = source.config && typeof source.config === 'object' && !Array.isArray(source.config) ? source.config as Record<string, unknown> : {}; const settings = config.settings && typeof config.settings === 'object' && !Array.isArray(config.settings) ? config.settings as Record<string, unknown> : {}; const projects = Array.isArray(settings.projects) ? settings.projects : []; setEditing(source); setTemporarySourceId(null); setResources([]); setSelectedResourceIds(ids); setDriveFolderToken(source.type === 'feishu_drive' || source.type === 'lark_drive' ? ids[0] ?? '' : ''); setRssFeedUrls(source.type === 'rss' && typeof settings.feed_urls === 'string' ? settings.feed_urls : ''); setRssAuthHeaders(''); setGitlabProjects(source.type === 'gitlab' ? projects.map((project) => { const row = project as Record<string, unknown>; return { projectId: typeof row.project_id === 'string' ? row.project_id : '', ref: typeof row.ref === 'string' ? row.ref : '', paths: Array.isArray(row.paths) ? row.paths.filter((path): path is string => typeof path === 'string').join('\n') : '' }; }) : []); setExpandedResourceIds([]); setDraft(nativeDataSourceDraftFrom(source)); setError(''); void loadResources(source); }
