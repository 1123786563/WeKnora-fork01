...
export function DataSourcesScreen() {
  ...
  async function showLogs(source: DataSource) {
    setLogsSource(source); setLogs([]); setLogsHasMore(false); setExpandedLogId(null); setLogsLoading(true); setError('');
    try { const next = await runtime.client.dataSources.logs(source.id, 50, 0); setLogs(next); setLogsHasMore(next.length === 50); }
  ...
  async function loadMoreLogs() {
    ...
    try { const next = await runtime.client.dataSources.logs(logsSource.id, 50, logs.length); setLogs((current) => [...current, ...next]); setLogsHasMore(next.length === 50); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to load more sync logs'); }
