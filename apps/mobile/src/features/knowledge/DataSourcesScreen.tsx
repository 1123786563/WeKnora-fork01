...
export function DataSourcesScreen() {
  ...
  function openCreate() { setEditing(null); setResources([]); setSelectedResourceIds([]); setExpandedResourceIds([]); setDriveFolderToken(''); setDraft({ ...EMPTY_DRAFT, type: types[0]?.type ?? '' }); setError(''); }
  ...
  function openEdit(source: DataSource) { const ids = sourceResourceIds(source); setEditing(source); setResources([]); setSelectedResourceIds(ids); setDriveFolderToken(source.type === 'feishu_drive' || source.type === 'lark_drive' ? ids[0] ?? '' : ''); setExpandedResourceIds([]); setDraft(nativeDataSourceDraftFrom(source)); setError(''); void loadResources(source); }
  ...
  async function loadDriveRoot() {
    ...
    if (!editing) { setError('Save the data source before loading Drive resources'); return; }
    setDriveFolderToken(token); setSelectedResourceIds([token]); setResourcesLoading(true); setError('');
    try {
      await runtime.client.dataSources.update(editing.id, { config: { resource_ids: [token] } });
      setResources(await runtime.client.dataSources.resources(editing.id));
