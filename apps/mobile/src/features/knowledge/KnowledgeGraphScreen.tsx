...
export function KnowledgeGraphScreen() {
  ...
  return <SafeAreaView style={{ flex: 1, padding: 16 }}>
    ...
    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8, gap: 8 }}><Text>{label("knowledgeBase.graph.depth")}</Text>{[1, 2, 3].map((value) => <Pressable key={value} onPress={() => setDepth(value)} style={{ paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, backgroundColor: depth === value ? '#dcfce7' : '#f2f4f7' }}><Text>{value}</Text></Pressable>)}</View>
