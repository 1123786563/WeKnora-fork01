...
| API bridge | The shared React entry reads `window.__WEKNORA_API_BASE__` during module evaluation. Wails' generated binding is asynchronous (`GetAPIBaseURL(): Promise<string>`) and may appear after WebView evaluation, so the desktop entry now waits up to Vue's 40 x 50ms window for an injected URL or binding before importing the shared renderer; it accepts only HTTP(S). | Repaired and covered by focused tests, including delayed binding, rejected Promise, and invalid-scheme Promise cases. |
...
```text
pnpm --filter @weknora/desktop-renderer test       9 pass, 0 fail
