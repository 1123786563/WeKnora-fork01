...
| API bridge | The shared React entry reads `window.__WEKNORA_API_BASE__` during module evaluation. The desktop entry now resolves `App.GetAPIBaseURL()` before importing the shared renderer and accepts only HTTP(S). | Repaired and covered by focused test. |
...
| Credential bridge | React has a desktop credential storage abstraction, but the current Go `App` has no `GetCredential`/`SetCredential`/`DeleteCredential` (or keychain equivalent) binding, and the Wails config in this branch still points at `../../frontend`. | `blocked-env` / integration gap. No claim of secure desktop credential persistence is made; no Go or Wails-config changes were made because they are outside the requested `apps/desktop/**` scope. |
