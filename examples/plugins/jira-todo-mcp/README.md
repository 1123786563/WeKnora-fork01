...
反向代理后运行），并自行评估会话存储与吊销需求。已知限制：授权页凭据提交
...
动态注册的客户端按 **24 小时 TTL** 惰性淘汰（过期注册即时失效并释放注册容量，
未认证方无法把 4096 的注册上限占满到重启）；`client_name` 上限 256 字节、
`redirect_uris` 上限 16 条 × 2KiB——注册表的聚合内存大小有界。
