...
CSV 第 21–24 行的 `/platform/apps*` 四行在当前 worktree 没有物理 Vue 文件，但已解析到历史 Vue 实现提交 `9b0c11c4`；映射与命令见 `artifacts/apps-vue-authority-20260916.md`。`apps/web/src/apps/AppsPages.tsx`、`model.ts` 与 focused test 存在。当前快照仍为 59 条数据行且没有显式 `row_id`，详见 `artifacts/route-inventory-parity-ledger-audit-20260916.md`。
