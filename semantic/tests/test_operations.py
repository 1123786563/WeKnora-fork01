...
def test_expired_claim_gets_new_fence_and_old_worker_cannot_transition(operation_store, apply_request):
    ...
    assert not operation_store.transition(operation.operation_id, "worker-old", first.lease_token, OperationPhase.RUNNING, OperationPhase.STAGED)
...
def test_cancel_and_publish_race_has_one_terminal_winner(operation_store, apply_request):
    ...
    assert operation_store.transition(operation.operation_id, "worker", claim.lease_token, OperationPhase.RUNNING, OperationPhase.STAGED)
    assert operation_store.transition(operation.operation_id, "worker", claim.lease_token, OperationPhase.STAGED, OperationPhase.PUBLISHING)
    ...
    def publish():
        barrier.wait()
        return "succeeded" if operation_store.transition(operation.operation_id, "worker", claim.lease_token, OperationPhase.PUBLISHING, OperationPhase.SUCCEEDED) else "lost"
    with ThreadPoolExecutor(max_workers=2) as pool:
...
def test_succeeded_operation_cannot_be_cancelled_and_terminal_clears_request(operation_store, apply_request):
    ...
    for expected, next_phase in ((OperationPhase.RUNNING, OperationPhase.STAGED), (OperationPhase.STAGED, OperationPhase.PUBLISHING), (OperationPhase.PUBLISHING, OperationPhase.SUCCEEDED)):
        assert operation_store.transition(operation.operation_id, "worker", claim.lease_token, expected, next_phase)
...
def test_c01_projection_preserves_internal_stage(operation_store, apply_request):
    ...
    assert staged.state == "running" and staged.stage == "staged"
